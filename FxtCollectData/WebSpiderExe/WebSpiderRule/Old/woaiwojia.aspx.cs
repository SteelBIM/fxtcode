...
{
    public partial class woaiwojia : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Regex reg = new Regex("<h3.*?>(?<ProjectName>.*?)&nbsp;");
            MatchCollection mcs = reg.Matches(content);
            if (mcs.Count >= 1)
            {
            ...
            reg = new Regex("<div class=\"list-info-r\">(?<Price>.*?)</div>");
            ...
            if (mcs.Count == 1)
            {
                ...
                reg = new Regex("<p>(?<UnitPrice>.*?)元/平米</p>");
                mcs = reg.Matches(Price);
                if (mcs.Count == 1)
                {
