...
namespace test20160519
{
    public partial class lianjia : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Regex reg = new Regex("data-el=\"xiaoqu\"><span class=\"\">(?<ProjectName>.*?)</a>");
            MatchCollection mcs = reg.Matches(content);
            if (mcs.Count == 1)
            {
            ...
            reg = new Regex("<div class=\"price-pre\">(?<UnitPrice>.*?)元/m²");
            mcs = reg.Matches(content);
            if (mcs.Count == 1)
            {
            ...
            if (mcs.Count == 1)
            {
                ...
                {
                    reg = new Regex("class=\"\">(?<BuildingArea>.*?)平米");
                    mcs = reg.Matches(HouseInfo.Substring(HouseInfo.IndexOf("平米") - 20, 25));
                    if (mcs.Count == 1)
                    {
            ...
            reg = new Regex("<div class=\"pic-panel\"><a target=\"_blank\" href=\"(?<SourceLink>.*?)\"");
            mcs = reg.Matches(content);
            if (mcs.Count == 1)
            {
