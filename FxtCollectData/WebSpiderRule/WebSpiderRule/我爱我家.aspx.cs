...
namespace WebSpiderRule
{
    public partial class 我爱我家 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (mcs.Count >= 1)
            {
                string HouserInfo = mcs[0].Groups["HouserInfo"].Value;
                if (HouserInfo.Contains("厅"))
                {
                    HouseTypeName=HouserInfo.Substring(HouserInfo.IndexOf("厅")-3,4);
                }

                reg = new Regex("建筑面积(?<BuildingArea>.*?)平");
                mcs = reg.Matches(HouserInfo);
                if (mcs.Count >= 1)
                {
                    BuildingArea = mcs[0].Groups["BuildingArea"].Value;
                }     
           
                if (HouserInfo.Contains("东") || HouserInfo.Contains("南") || HouserInfo.Contains("西") || HouserInfo.Contains("北"))
                {
                    reg = new Regex("平(?<FrontName>.*?)&");
                    mcs = reg.Matches(HouserInfo);
                    if (mcs.Count >= 1)
                    {
                ...
                if (HouserInfo.Contains("层") && HouserInfo.Contains("/"))
                {
