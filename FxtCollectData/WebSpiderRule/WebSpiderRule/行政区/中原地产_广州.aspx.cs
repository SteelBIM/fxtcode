...
{
    public partial class 中原地产_广州 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Regex reg = new Regex("<p class=\"house-name\">(?<ProjectInfo>.*?)</p>");
            MatchCollection mcs = reg.Matches(content);
            if (mcs.Count == 1)
            {
            ...
            reg = new Regex("<p class=\"txt-gray\">(?<HouseInfo>.*?)</p>");
            ...
            reg = new Regex("<p class=\"price-num\">(?<TotalPrice>.*?)万</p>");
            mcs = reg.Matches(content);
            if (mcs.Count == 1)
            {
            ...
            reg = new Regex("<p class=\"price-smallnum\">(?<UnitPrice>.*?)元/");
            mcs = reg.Matches(content);
            if (mcs.Count == 1)
            {
