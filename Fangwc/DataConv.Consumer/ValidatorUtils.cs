...
namespace DataConv.Consumer
{
    public class ValidatorUtils
    {
        ...
        private static readonly ILog logger = LogManager.GetLogger(typeof(ValidatorUtils));
        ...
        public static bool IsValidData(ProviderServiceRef.City item,
            DataCase data,
            int[] arrays,
            ...
            Dictionary<string, int> MoneyUnitInfoMap)
        {
            ...
            foreach (KeyValuePair<string, int> map in FrontMap)
            {
                if (!string.IsNullOrEmpty(data.FrontName) &&
                    map.Key.StartsWith(data.FrontName))
                {
