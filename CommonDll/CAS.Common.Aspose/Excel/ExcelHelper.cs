    ...
    public class ExcelHelper
    {
        ...
        public void GetNameValues(Dictionary<string, string> dicExcel, Dictionary<string, string> result = null, bool isNewDictionary = false, string[] notWriteToExcelArry = null)
        {
            ...
            if (rangeArray != null)
            {
                ...
                foreach (Range range in rangeArray)
                {
                    Cell cell = range[0, 0];
                    string name = "";
                    r1C1Formula = cell.R1C1Formula ?? "";
                    //如果自定义公式中包括[DBNum2][$-804]G/通用格式  就将他转化为英文的[DBNum2][$-804]General aspose.Cells的CalculateFormula才支持
                    if (r1C1Formula.ToLower().Contains("g/通用格式"))
                    {
                        r1C1Formula = r1C1Formula.ToLower().Replace("g/通用格式", "General");
                    }
                    if (r1C1Formula.ToLower().Contains("\"[dbnum2]\""))
                    {
                        r1C1Formula = r1C1Formula.ToLower().Replace("[dbnum2]", "[DBNum2][$-804]General");
                    }
                    //aspose无法执行NUMBERSTRING这个函数 因为这个函数只支持整数不支持小数 所以这个用ROUND代替获取出出整数值即可
                    if (r1C1Formula.ToLower().Contains("numberstring"))
                    {
                        r1C1Formula = r1C1Formula.ToLower().Replace("numberstring", "ROUND");
                    }
                    if (r1C1Formula != "")
                        cell.R1C1Formula = r1C1Formula.Replace("rmb", "RMB");
                    ...
                    if (range.Name.Contains("!"))
                        ...
                        name = range.Name;
                    #endregion
                    //if (name == "_c86" || name == "净值大写" || name == "显示净值" || name == "最终净值")
                    //    name = name;
                    if ((notWriteToExcelArry == null || !notWriteToExcelArry.Contains(range.Name)) && dicExcel.ContainsKey(name))
                    {
                        ...
                        {
                            PutExcelValue(cell, style, dicExcel[name]);
                        }
                    }
                }
                #endregion
                ...
                foreach (Range range in rangeArray)
                {
                    ...
                    Cell cell = range[0, 0];
                        ...
                        //else
                        //    name = range.Name;
                        //if (name == "_c86" || name == "评估总值大写" || name == "土地总价2大写" || name == "土地单价2")
                        //    name = name;
                        if (cell.R1C1Formula != null && cell.R1C1Formula.Contains("&"))
                        {
                            Regex regex = new Regex(@"(&)[^&&]+(&)");
                            foreach (Match m in regex.Matches(cell.R1C1Formula))
                            {
                                string key = m.Value.Replace("&", "");
                                if (key.Contains("(") && key.Contains(")") && key.Contains(","))//对于这样的公式处理：“大写：人民币"&TEXT(净值万元大写,"[DBNum2]")&"元整。",""”
                                ...
                                if (key.Contains("大写") && workbook.Worksheets.GetRangeByName(key)!=null)
                                {
                                    ...
                                    if (dicExcel.Keys.Contains(key))
                                        cell.R1C1Formula = cell.R1C1Formula.Replace("\"" + m.Value + "\"", dicExcel[key]).Replace("rmb","RMB");
                                }
                            }
                            workbook.CalculateFormula();
                        }
                    ...
                    //catch (Exception ex)
                    //{
                    //    throw new Exception(ex + range.Name);
                    //    rangeArray = null;
                    //}
                    //decimal d;
                    //string name = "";
                    ...
                    //if (name == "净值万元" || name == "净值大写" || name == "装修大写" || name == "最终净值")
                    //    name = name;
                    ...
                    //}
                    //else if (custom.IndexOf("dbnum1", StringComparison.Ordinal) >= 0 && decimal.TryParse(cell.StringValue, out d))
                    //{
                    //    AsposeHelper.AddItem(isNewDictionary ? result : dicExcel, name,
                    ...
                    //else
                    //{
                    //    AsposeHelper.AddItem(isNewDictionary ? result : dicExcel, name, cell.DisplayStringValue);
                    //}

                    //if (r1C1Formula.Contains("General"))
                    //{ cell.R1C1Formula = r1C1Formula.Replace("General", "G/通用格式"); }
                }
                #endregion
            }
        }
        ...
        public void SetPictureToExcel(Dictionary<string, string> dicExcelImg, string sheetName)
        {
            if ((Worksheet)workbook.Worksheets[sheetName] != null)
            {
                ...
                string name = sheetName.Replace("照片", "");
                if (dicExcelImg.Count > 0)
                {
                    foreach (KeyValuePair<string, string> kv in dicExcelImg)//由于图片的字典数据相对所有的名称管理器来说比较少所以遍历字段
                    {
