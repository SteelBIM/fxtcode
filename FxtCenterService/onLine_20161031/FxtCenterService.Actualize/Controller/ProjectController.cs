...
namespace FxtCenterService.Actualize
{
    public partial class DataController
    {
        ...
        /// <returns></returns>
        [OverflowAttrbute(ApiType.Project)]
        public static JsonDataContainExcuteTimeLog GetProjectDropDownList_MCAS_SDK(JObject funinfo, UserCheck company)
        {
            JsonDataContainExcuteTimeLog jsoncontainlog = new JsonDataContainExcuteTimeLog();
            ...
            string serialno = funinfo.Value<string>("serialno");
            key = HttpUtility.UrlDecode(key);
            Stopwatch swHuiDuPd = new Stopwatch();
            swHuiDuPd.Start();
            //使用RPC调用数据 20160816tring.IsNullOrEmpty(buildingname)
            if (FxtClientService.IfUseRpc(company.producttypecode, company.companyid))
            {
                swHuiDuPd.Stop();
                int huiDuPDMill = Convert.ToInt32(swHuiDuPd.Elapsed.TotalMilliseconds);
                Stopwatch swHuiDuRequest = new Stopwatch();
                swHuiDuRequest.Start();
                ...
                Stopwatch swHuiDuJson = new Stopwatch();
                swHuiDuJson.Start();
                jsoncontainlog.jsondata = projectDropdownListResponse.Projects.ToLowerJson();
                swHuiDuJson.Stop();

                log.requestparam = "灰度发布调用细化0104";
                //log.usercentertime = (int)swHuiDuPd.Elapsed.TotalMilliseconds; //灰度判断时间**********time1
                //log.cityauthoritytime = (int)swHuiDuRequest.Elapsed.TotalMilliseconds; //调用时间
                //log.overflowtime = (int)swHuiDuJson.Elapsed.TotalMilliseconds; //json解析时间************

                log.usercentertime = projectDropdownListResponse.ResponseMsg.Requestone; //程序初始化时间
                log.cityauthoritytime = projectDropdownListResponse.ResponseMsg.Requesttwo; //接口调用时间
                log.overflowtime = projectDropdownListResponse.ResponseMsg.Requestthree; //接口子调用时间


                log.sqlexecutetime = projectDropdownListResponse.ResponseMsg.ExecuteMill;//sql执行时间
                log.sqltime = projectDropdownListResponse.ResponseMsg.SqlMill - projectDropdownListResponse.ResponseMsg.ExecuteMill;//程序处理

                jsoncontainlog.executetimeLog = log;

                return jsoncontainlog;
            }
            else
            {
                List<Dictionary<string, object>> list = DatProjectBL.GetProjectDropDownList_MCAS_SDK(ref log, search, key, buildingname, items, serialno, company.producttypecode, company.companyid, priceorderby);
                jsoncontainlog.jsondata = list.ToJson();
                jsoncontainlog.executetimeLog = log;
                return jsoncontainlog;
            }
        }
        ...
        public static string GetProjectInfoById(JObject funinfo, UserCheck company)
        {
            ...
            if (FxtClientService.IfUseRpc(company.producttypecode, company.companyid))
            {
                ...
                projectInfoResponse.BuildingDate = (string.IsNullOrEmpty(projectInfoResponse.BuildingDate) ? "" : Convert.ToDateTime(projectInfoResponse.BuildingDate).GetDateTimeFormats('s')[0].ToString());
                projectInfoResponse.Enddate = (string.IsNullOrEmpty(projectInfoResponse.Enddate) ? "" : Convert.ToDateTime(projectInfoResponse.Enddate).GetDateTimeFormats('s')[0].ToString());
                projectInfoResponse.StartDate = (string.IsNullOrEmpty(projectInfoResponse.StartDate) ? "" : Convert.ToDateTime(projectInfoResponse.StartDate).GetDateTimeFormats('s')[0].ToString());
        ...
        public static string GetBuildingBaseInfoListList(JObject funinfo, UserCheck company)
        {
            ...
            if (FxtClientService.IfUseRpc(company.producttypecode, company.companyid))
            {
                ...
                var requestParam = new BuildingRequestParam()
                {
                    SerachParam = new SearchParam()
                    {
                        Page = search.PageIndex > 0 ? true : false,
                        PageIndex = search.PageIndex,
        ...
        /// <returns></returns>
        [OverflowAttrbute(ApiType.Building)]
        public static JsonDataContainExcuteTimeLog GetBuildingBaseInfoList_MCAS(JObject funinfo, UserCheck company)
        {
            JsonDataContainExcuteTimeLog jsoncontainlog = new JsonDataContainExcuteTimeLog();
            ExecuteTimeLog log = new ExecuteTimeLog();
            SearchBase search = DataCenterCommon.InitSearBase(funinfo);
            ...
            if (FxtClientService.IfUseRpc(company.producttypecode, company.companyid))
            {
                ...
                Stopwatch swHuiDuJson = new Stopwatch();
                swHuiDuJson.Start();
                jsoncontainlog.jsondata = buildingResponse.Buildings.ToLowerJson();
                swHuiDuJson.Stop();
                log.requestparam = "灰度发布2";
                log.usercentertime = (int)swHuiDu.Elapsed.TotalMilliseconds; //灰度判断时间
                log.cityauthoritytime = (int)swRequest.Elapsed.TotalMilliseconds; //调用时间
                log.overflowtime = (int)swHuiDuJson.Elapsed.TotalMilliseconds; //json解析时间

                log.sqlexecutetime = buildingResponse.ResponseMsg.ExecuteMill;
                log.sqltime = buildingResponse.ResponseMsg.SqlMill - buildingResponse.ResponseMsg.ExecuteMill;
            ...
            else
            {
                ...
                jsoncontainlog.jsondata = ds.OrderBy(obj => obj.buildingname, new FxtCenterService.Common.CNComparer()).ToList().ToJson();
                jsoncontainlog.executetimeLog = log;
                return jsoncontainlog;
            }
        }
        ...
        /// <returns></returns>
        [OverflowAttrbute(ApiType.Building)]
        public static JsonDataContainExcuteTimeLog GetHouseNoList_MCAS(JObject funinfo, UserCheck company)
        {
            JsonDataContainExcuteTimeLog jsoncontainlog = new JsonDataContainExcuteTimeLog();
            ExecuteTimeLog log = new ExecuteTimeLog();
            SearchBase search = DataCenterCommon.InitSearBase(funinfo);
            ...
            if (FxtClientService.IfUseRpc(company.producttypecode, company.companyid))
            {
                ...
                Stopwatch swHuiDuJson = new Stopwatch();
                swHuiDuJson.Start();
                jsoncontainlog.jsondata = floorHouseResponse.FloorHouses.Select(x => new { floorno = x.FloorNo, housecnt = x.HouseNoCount }).ToList().ToJson();
                swHuiDuJson.Stop();
                log.requestparam = "灰度发布2";
                log.usercentertime = (int)swHuiDu.Elapsed.TotalMilliseconds; //灰度判断时间
                log.cityauthoritytime = (int)swRequest.Elapsed.TotalMilliseconds; //调用时间
                log.overflowtime = (int)swHuiDuJson.Elapsed.TotalMilliseconds; //json解析时间
                log.sqlexecutetime = floorHouseResponse.ResponseMsg.ExecuteMill;
                log.sqltime = floorHouseResponse.ResponseMsg.SqlMill - floorHouseResponse.ResponseMsg.ExecuteMill;
                jsoncontainlog.executetimeLog = log;
                return jsoncontainlog;
            }
            key = HttpUtility.UrlDecode(key);
            ...
            else if(!string.IsNullOrEmpty(key))
            {
                ...
                {
                    if (dr[0].ToString().Contains(key))
                    {
                        dtCopy.Rows.Add(dr.ItemArray);
                    }                    
                }
                #endregion
                jsoncontainlog.jsondata = dtCopy.ToJson();
            }
            else
            {
                jsoncontainlog.jsondata = ds.Tables[0].ToJson();
            }
            jsoncontainlog.executetimeLog = log;
            return jsoncontainlog;
        }
        ...
        /// <returns></returns>
        [OverflowAttrbute(ApiType.House)]
        public static JsonDataContainExcuteTimeLog GetHouseDropDownList_MCAS(JObject funinfo, UserCheck company)
        {
            JsonDataContainExcuteTimeLog jsoncontainlog = new JsonDataContainExcuteTimeLog();
            ExecuteTimeLog log = new ExecuteTimeLog();
            SearchBase search = DataCenterCommon.InitSearBase(funinfo);
            ...
            if (FxtClientService.IfUseRpc(company.producttypecode, company.companyid))
            {
                ...
                {
                    jsoncontainlog.jsondata = OrderByHelper.OrderBy<DATHouseOrderBy>(list, "housename").ToJson();
                }
                else
                {
                    jsoncontainlog.jsondata = list.ToJson();
                ...
                log.cityauthoritytime = (int)swRequest.Elapsed.TotalMilliseconds; //调用时间
                log.overflowtime = (int)swHuiDuJson.Elapsed.TotalMilliseconds; //json解析时间
                log.sqlexecutetime = houseResponse.ResponseMsg.ExecuteMill;
                log.sqltime = houseResponse.ResponseMsg.SqlMill - houseResponse.ResponseMsg.ExecuteMill;
                jsoncontainlog.executetimeLog = log;
                return jsoncontainlog;
            }
            else
            {
                ...
                jsoncontainlog.jsondata = result.OrderBy(obj => obj.housename, new FxtCenterService.Common.CNComparer()).ToList().ToJson();
                jsoncontainlog.executetimeLog = log;
                return jsoncontainlog;
            }
        }
        ...
        public static string GetCaseListByCaseIds(JObject funinfo, UserCheck company)
        {
            ...
            if (FxtClientService.IfUseRpc(company.producttypecode, company.companyid))
            {
                ...
                var request = new CaseRequest()
                {
                    ...
                    ProjectName = string.IsNullOrEmpty(projectname) ? "" : projectname,
                    CaseIds = string.IsNullOrEmpty(caseIds) ? "" : caseIds,
                    CompanyCode = string.IsNullOrEmpty(company.companycode) ? "" : company.companycode
                };
        ...
        public static string ImportProjectAndBuildingAndHouse(JObject funinfo, UserCheck company)
        {
            try
            {
                ...
                try
                {
                    jobj = funinfo.Value<JObject>("data");
                }
                catch
                {
                    jobj = JObject.Parse(funinfo.Value<string>("data"));
                }
                ...
                else
                {
                    ...
                    nowProject.createtime = _projList[0].createtime;
                    int _projResult = 0;
                    //不是自己的信息&&自己不是25(插入子表)
        ...
        public static string GetProjectDetailInfo(JObject funinfo, UserCheck company)
        {
            ...
            int projectId = StringHelper.TryGetInt(funinfo.Value<string>("projectid"));
            int buildingid = StringHelper.TryGetInt(funinfo.Value<string>("buildingid"));
            DataSet ds = DatProjectBL.GetProjectDetailInfo(projectId, buildingid, search);
        ...
        /// <param name="company"></param>
        /// <returns></returns>
        public static string GetProjectOperatedRecord(JObject funinfo, UserCheck company)
        {
            int fxtcompanyid = company.parentshowdatacompanyid;
            int sysTypeCode = company.parentproducttypecode;
            int cityid = StringHelper.TryGetInt(funinfo.Value<string>("cityid"));
            int areaid = funinfo.Property("areaid") == null ? 0 : funinfo.Value<int>("areaid");
            int projectid = funinfo.Property("projectid") == null ? 0 : funinfo.Value<int>("projectid");
            DateTime startdate = StringHelper.TryGetDateTime(funinfo.Value<string>("startdate"));
            DateTime enddate = StringHelper.TryGetDateTime(funinfo.Value<string>("enddate"));

            return DatProjectBL.GetProjectOperatedRecord(fxtcompanyid, sysTypeCode, cityid, startdate, enddate, areaid, projectid);
        }

        public static string GetBuildingOperatedRecord(JObject funinfo, UserCheck company)
        {
            int fxtcompanyid = company.parentshowdatacompanyid;
            int sysTypeCode = company.parentproducttypecode;
            int cityid = StringHelper.TryGetInt(funinfo.Value<string>("cityid"));
            ...
            DateTime enddate = StringHelper.TryGetDateTime(funinfo.Value<string>("enddate"));
            int[] projectids =
                funinfo.Property("projectid") == null ? new int[] { } :
                funinfo.Value<string>("projectid").Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Select(o =>
                {
                    int defaultInt;
                    int.TryParse(o, out defaultInt);
                    return defaultInt;
                }).Where(o => o != 0).ToArray();
            string inProjectIDStr = string.Join(",", projectids);
            return DatProjectBL.GetBuildingOperatedRecord(fxtcompanyid, sysTypeCode, cityid, inProjectIDStr, startdate, enddate);
        }

        public static string GetHouseOperatedRecord(JObject funinfo, UserCheck company)
        {
            int fxtcompanyid = company.parentshowdatacompanyid;
            int sysTypeCode = company.parentproducttypecode;
            int cityid = StringHelper.TryGetInt(funinfo.Value<string>("cityid"));
            ...
            DateTime enddate = StringHelper.TryGetDateTime(funinfo.Value<string>("enddate"));
            int[] buildingids =
                funinfo.Property("buildingid") == null ? new int[] { } :
                funinfo.Value<string>("buildingid").Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Select(o =>
                {
                    int defaultInt;
                    int.TryParse(o, out defaultInt);
                    return defaultInt;
                }).Where(o => o != 0).ToArray();
            string inBuildingIDStr = string.Join(",", buildingids);
            return DatProjectBL.GetHouseOperatedRecord(fxtcompanyid, sysTypeCode, cityid, inBuildingIDStr, startdate, enddate);
        }
    }
}
