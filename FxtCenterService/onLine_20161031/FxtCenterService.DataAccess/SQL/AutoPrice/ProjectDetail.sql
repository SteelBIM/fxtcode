...
SELECT [ProjectId],[ProjectName] +'['+a.[AreaName]+']' ProjectName,p.IsEValue,p.Address,a.areaid, p.buildingdate,p.PinYinALL,p.PinYin,p.averageprice,p.[Weight],p.enddate  FROM @table_project p with(nolock),FxtDataCenter.dbo.SYS_Area a with(nolock)where p.[Valid]=1 and p.[CityId]=@cityid and p.[AreaID]=a.[AreaID] 
...
union 
SELECT [ProjectId],[ProjectName] +'['+a.[AreaName]+']' ProjectName,p.IsEValue,p.Address,a.areaid,p.buildingdate,p.PinYinALL,p.PinYin,p.averageprice,p.[Weight],p.enddate  FROM @table_project_sub p with(nolock),FxtDataCenter.dbo.SYS_Area a with(nolock)where p.[Valid]=1 and p.[CityId]=@cityid and p.[AreaID]=a.[AreaID] 
