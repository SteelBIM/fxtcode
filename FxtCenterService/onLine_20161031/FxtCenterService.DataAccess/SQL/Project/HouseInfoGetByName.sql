select top 1 * from (
select HouseId,BuildingId, HouseName, Valid,floorno,FxtCompanyId,'1' as Remark from @dat_house as tbl with(nolock) where 
...
union
select HouseId,BuildingId, HouseName, Valid,floorno,FxtCompanyId,'0' as Remark from @dat_house_sub with(nolock) 
