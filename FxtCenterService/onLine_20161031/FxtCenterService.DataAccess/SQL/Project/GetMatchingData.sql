...
if  @address is not null and  @address!=''
	...
	(select top 1 projectid,projectname,[address] from @table_project pro with(nolock) where [address]=@address   
	 and 
 (','+cast((select showcompanyid from FxtDataCenter.dbo.Privi_Company_ShowData pc with(nolock) where pc.fxtcompanyid=@fxtcompanyid and pc.cityid = pro.cityid and pc.TypeCode = @typecode)
