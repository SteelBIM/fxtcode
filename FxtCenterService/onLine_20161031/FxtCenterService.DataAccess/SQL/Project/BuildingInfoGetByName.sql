select top 1 * from (
select BuildingId,ProjectId, BuildingName, Valid,TotalFloor,FxtCompanyId,0 as Fxt_CompanyId from @dat_building as tbl with(nolock) where 
ProjectId=@projectid
and Cityid=@cityId 
and Valid=1
...
union
select BuildingId,ProjectId, BuildingName, Valid,TotalFloor, Fxt_CompanyId as FxtCompanyId,Fxt_CompanyId from @dat_building_sub with(nolock) 
