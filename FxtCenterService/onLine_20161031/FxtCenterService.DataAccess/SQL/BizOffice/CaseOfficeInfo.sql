...
left join (
	select T.address,T.ProjectId,T.AreaId,a.AreaName,T.SubAreaId,sa.SubAreaName,T.PurposeCode,T.BuildingType,T.UsableYear,T.StartDate,T.BuildingArea,T.CubageRate,T.GreenRate,T.BuildingNum,T.EndDate,T.OfficeArea,T.BizArea,T.IndustryArea,T.TrafficType,T.TrafficDetails,T.ParkingLevel,T.ParkingType,T.Details,T.East,T.west,T.south,T.north from (
