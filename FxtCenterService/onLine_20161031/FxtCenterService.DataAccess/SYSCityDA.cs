...
namespace FxtCenterService.DataAccess
{
    public class SYSCityDA : Base
    {
        ...
        /// <returns></returns>
        public static SYSCity GetSYSCityInfoByName(string cityname)
        {
            List<SqlParameter> parameters = new List<SqlParameter>();
            parameters.Add(SqlHelper.GetSqlParameter("@cityname", cityname, SqlDbType.VarChar));
            string strSql = "SELECT top 1 * FROM [FxtDataCenter].[dbo].[SYS_City] where CityName = @cityname or CityName = @cityname+'市'";
            return ExecuteToEntity<SYSCity>(strSql, System.Data.CommandType.Text, parameters);
        }
