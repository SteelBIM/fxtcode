...
namespace FxtCenterService.DataAccess
{
    public class DATProjectAvgPriceDA : Base
    {
        ...
        public static DataSet GetProjectPriceRank(string cityname, string areaname, string projectname, int FxtCompanyId, int systypecode)
        {
            if (string.IsNullOrEmpty(cityname) || string.IsNullOrEmpty(areaname) || string.IsNullOrEmpty(projectname)) return null;
            SYSCity city = SYSCityDA.GetSYSCityInfoByName(cityname);
            CityTable cityTb = CityTableDA.GetCityTable(city.cityid);
            if (null == cityTb) return null;            

            string strSql = SQL.Project.GetProjectPriceRankByProjectName;
            strSql = strSql.Replace("@projecttable", cityTb.ProjectTable);
            strSql = strSql.Replace("@projectavgtable", cityTb.ProjectTable.Replace("DAT_Project", "DAT_ProjectAvg"));
            strSql = strSql.Replace("@projectweighttable", cityTb.ProjectTable.Replace("DAT_Project", "DAT_WeightProject"));
            SqlCommand cmd = new SqlCommand(strSql);
            cmd.Parameters.Add(new SqlParameter("@CityName", cityname));
            cmd.Parameters.Add(new SqlParameter("@AreaName", areaname));
            cmd.Parameters.Add(new SqlParameter("@ProjectName", projectname));
            cmd.Parameters.Add(new SqlParameter("@FxtCompanyId", 25));
            cmd.Parameters.Add(new SqlParameter("@TypeCode", systypecode));
            cmd.CommandText = strSql;            
            DataSet ds = ExecuteDataSet(cmd);
            return ds;
        }
