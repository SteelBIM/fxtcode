...
namespace FxtCenterService.DataAccess
{
    public class DatProjectDA : Base
    {
        public static int Add(DATProject model)
        {
            ...
            {
                model.SetIgnoreFields(new string[] { "fxtcompanyid", "SaveDateTime" });
                model.SetPrimaryKeyIsIdentify(false);
            }
            else
            {
                model.SetIgnoreFields(new string[] { "fxt_companyid", "SaveDateTime" });
        ...
        /// </summary>
        /// <returns></returns>
        public static DataTable GetProjectDropDownList_MCAS_SDK(SearchBase search, string condition, string strkey, string buildingName, string param, string serialno, int priceorderby = 0)
        {
            string sql = SQL.Project.ProjectDropDownList_MCAS_SDK;
            CityTable cityTb = CityTableDA.GetCityTable(search.CityId);
            if (null == cityTb)
                return new DataTable();
            string projectTable = cityTb.ProjectTable;
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public static List<DATProject> GetSearchProjectListByKey(SearchBase search, int fxtcompanyid, int cityid, string key, bool searchBuilding)
        {
            if (!searchBuilding)
            {
                return GetSearchProjectListByKey(search, fxtcompanyid, cityid, key);
            }
            else
            {
                string sql = SQL.AutoPrice.ProjectListSpecial;
                List<SqlParameter> param = new List<SqlParameter>();
                CityTable cityTb = CityTableDA.GetCityTable(search.CityId);
                if (null == cityTb)
                    return new List<DATProject>();
        ...
        /// <param name="projectId"></param>
        /// <param name="distance">关联距离（米）</param>
        public static List<RelProject> GetRelProjectBylimit(SearchBase search, int projectId, int distance, decimal x, decimal minX, decimal maxX, decimal y, decimal minY, decimal maxY)
        {
            CityTable city = CityTableDA.GetCityTable(search.CityId);
            string sql = SQL.Project.RelProjectListByLimit;
            sql = sql.Replace("@projecttable", "fxtproject." + city.ProjectTable);
            List<SqlParameter> paramsList = new List<SqlParameter>();
            paramsList.Add(new SqlParameter("@CityId", search.CityId));
            paramsList.Add(new SqlParameter("@AreaId", search.AreaId));
            ...
            paramsList.Add(new SqlParameter("@x", x));
            paramsList.Add(new SqlParameter("@y", y));
            paramsList.Add(new SqlParameter("@minX", minX));
            paramsList.Add(new SqlParameter("@maxX", maxX));
            paramsList.Add(new SqlParameter("@minY", minY));
            paramsList.Add(new SqlParameter("@maxY", maxY));
            List<RelProject> entity = ExecuteToEntityList<RelProject>(sql, CommandType.Text, paramsList);
            return entity;
        }
        ...
        public static DataTable GetProjectOperatedRecord(int fxtcompanyid, int sysTypeCode, int cityid, DateTime startdate, DateTime enddate, int areaid, int projectid)
        {
            string where = string.Empty;
            string sql = SQL.Project.GetOperatedRecord;
            CityTable cityTb = CityTableDA.GetCityTable(cityid);
            if (null == cityTb)
                return new DataTable();
            string projectTable = cityTb.ProjectTable;
            if (projectid > 0)
            {
                where += string.Format(" and p.projectid = {0}", projectid);
            }
            if (areaid > 0)
            {
                where += string.Format(" and p.areaid = {0}", areaid);
            }
            sql = sql.Replace("@table_project", projectTable);
            sql = sql.Replace("$appendlimit", where);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandText = sql;
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@typecode", sysTypeCode, SqlDbType.Int));
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@startdate", startdate, SqlDbType.DateTime, 30));
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@enddate", enddate, SqlDbType.DateTime, 30));
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@fxtcompanyid", fxtcompanyid, SqlDbType.Int));
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@cityid", cityid, SqlDbType.Int));
            return ExecuteDataSet(cmd).Tables[0];
        }

        public static DataTable GetBuildingOperatedRecord(int fxtcompanyid, int sysTypeCode, int cityid, string inProjectIDStr, DateTime startdate, DateTime enddate)
        {
            string sql = SQL.Building.GetOperatedRecord;
            CityTable cityTb = CityTableDA.GetCityTable(cityid);
            if (null == cityTb)
                return new DataTable();
            string projectTable = cityTb.BuildingTable;
            sql = sql.Replace("@table_building", projectTable);
            if (!string.IsNullOrEmpty(inProjectIDStr))
            {
                sql = sql.Replace("$projectidlimit"," and tb.projectid in (" + inProjectIDStr + ")");
            }
            else
            {
                sql = sql.Replace("$projectidlimit", "");
            }
            SqlCommand cmd = new SqlCommand();
            cmd.CommandText = sql;
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@typecode", sysTypeCode, SqlDbType.Int));
        ...
        public static DataTable GetHouseOperatedRecord(int fxtcompanyid, int sysTypeCode, int cityid, string inBuildingIDStr, DateTime startdate, DateTime enddate)
        {
            string sql = SQL.House.GetOperatedRecord;
            CityTable cityTb = CityTableDA.GetCityTable(cityid);
            if (null == cityTb)
                return new DataTable();
            string projectTable = cityTb.HouseTable;
            sql = sql.Replace("@dat_house", projectTable);
            if (!string.IsNullOrEmpty(inBuildingIDStr))
            {
                sql = sql.Replace("$buildingidlimit", " and h.BuildingId in (" + inBuildingIDStr + ")");
            }
            else
            {
                sql = sql.Replace("$buildingidlimit", "");
            }
            SqlCommand cmd = new SqlCommand();
            cmd.CommandText = sql;
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@typecode", sysTypeCode, SqlDbType.Int));
