...
namespace FxtCenterService.Logic
{
    public class DatProjectBL
    {
        ...
        public static List<Dictionary<string, object>> GetProjectDropDownList_MCAS(SearchBase search, string strKey, string buildingName, int items, string serialno, int producttypecode, int companyid,int priceorderby = 0)
        {
            ...
            {
                List<ProjecForOrderBy> orderOperate = new List<ProjecForOrderBy>();
                int rowIndex = 0;
                foreach (DataRow item in o.Rows)
                {
                    orderOperate.Add(new ProjecForOrderBy()
                    {
                ...
                orderOperate = orderOperate.OrderBy(obj=>obj.projectname,new FxtCenterService.Common.CNComparer()).ToList();
                o.Columns.Add("projectsnameorder", typeof(int));
                for (int i = 0; i < orderOperate.Count; i++)
                {
                    dt.Rows[orderOperate[i].RowIndex]["projectsnameorder"] = i;
                }
        ...
        /// <returns></returns>
        public static List<Dictionary<string, object>> GetProjectDropDownList_MCAS_SDK(ref ExecuteTimeLog log,SearchBase search, string strKey, string buildingName, int items, string serialno, int producttypecode, int companyid, int priceorderby = 0)
        {
            List<Dictionary<string, object>> listResult = new List<Dictionary<string, object>>();
            ...
            TimeSpan tsInvoke = swRequest.Elapsed;
            int requestMill = (int)tsInvoke.TotalMilliseconds;
            log.sqlexecutetime = requestMill;

            #region 排序操作
            Stopwatch swRequestSort = new Stopwatch();
            swRequestSort.Start();
            DataView dv;
            DataTable dtCopy;
            Func<DataTable, DataView> tableSort = (o) =>
            ...
            TimeSpan tsInvokesort = swRequestSort.Elapsed;
            int requestMillsort = (int)tsInvokesort.TotalMilliseconds;
            log.sqltime = requestMillsort;
        ...
        /// <returns></returns>
        public static IEnumerable<object> GetSearchProjectListByKey(SearchBase search, int fxtcompanyid, int cityid, string key, bool searchBuilding)
        {
            var autoproject = DatProjectDA.GetSearchProjectListByKey(search, fxtcompanyid, cityid, key, searchBuilding).Select(o => new
            {
        ...
        /// <param name="search"></param>
        /// <returns></returns>
        public static DataSet GetProjectDetailInfo(int projectId, int buildingid, SearchBase search)
        {
            Stopwatch sw = new Stopwatch();
            sw.Start();

            DataSet result = DatProjectDA.GetProjectDetailInfo(projectId, buildingid, search);
        ...
        {
            decimal dlat = (decimal)FxtCenterService.Common.GeographyDistanceHelp.GetLatitudeRange(distance / 1000);
            decimal dlgt = Math.Abs((decimal)FxtCenterService.Common.GeographyDistanceHelp.GetLongitudeRange(distance / 1000, (double)y));
            var relProjects = DatProjectDA.GetRelProjectBylimit(search, projectId, distance, x, x - dlgt, x + dlgt, y, y - dlat, y + dlat);
        ...
        public static string GetProjectOperatedRecord(int fxtcompanyid, int sysTypeCode, int cityid, DateTime startdate, DateTime enddate, int areaid, int projectid)
        {
            var tb = DatProjectDA.GetProjectOperatedRecord(fxtcompanyid, sysTypeCode, cityid, startdate, enddate, areaid, projectid);
            List<object> records = new List<object>();
            foreach (DataRow item in tb.Rows)
            {
                records.Add(new
                {
                    cityid = item["cityid"].VInt(),
                    projectid = item["projectid"].VInt(),
                    projectname = item["projectname"].ToString(),
                    areaid = item["areaid"].VInt(),
                    statecode = item["opertedstate"]
                });
            }
            return JSONHelper.ObjectToJSON(records);
        }

        public static string GetBuildingOperatedRecord(int fxtcompanyid, int sysTypeCode, int cityid, string inProjectIDStr, DateTime startdate, DateTime enddate)
        {
            var tb = DatProjectDA.GetBuildingOperatedRecord(fxtcompanyid, sysTypeCode, cityid, inProjectIDStr, startdate, enddate);
            List<object> records = new List<object>();
            foreach (DataRow item in tb.Rows)
            {
                records.Add(new
                {
                    cityid = item["cityid"].VInt(),
                    buildingid = item["buildingid"].VInt(),
                    projectid = item["projectid"].VInt(),
        ...
        public static string GetHouseOperatedRecord(int fxtcompanyid, int sysTypeCode, int cityid, string inBuildingIDStr, DateTime startdate, DateTime enddate)
        {
            var tb = DatProjectDA.GetHouseOperatedRecord(fxtcompanyid, sysTypeCode, cityid, inBuildingIDStr, startdate, enddate);
            List<object> records = new List<object>();
            foreach (DataRow item in tb.Rows)
            {
                records.Add(new
