select top 1 * from (
select ProjectId, ProjectName, Valid,FxtCompanyId,0 as Fxt_CompanyId from @dat_project as tbl with(nolock) where
Cityid=@cityId 
and Valid=1
and ProjectName=@projectname  @where
...
union
select ProjectId, ProjectName, Valid, Fxt_CompanyId as FxtCompanyId,Fxt_CompanyId from @dat_project_sub with(nolock) 
where  CityId=@cityId and Valid=1 and ProjectName=@projectname @where and 
