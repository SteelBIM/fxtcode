...
(','+cast((select ShowCompanyId from dbo.privi_company_showdata with(nolock) where FxtCompanyId=@fxtcompanyid and Cityid=ps.cityid) as varchar)+',' like '%,' + cast(ps.Fxt_CompanyId as varchar) + ',%')
...
where 1=1 @valid and  
( ','+cast((select ShowCompanyId from dbo.privi_company_showdata with(nolock) where FxtCompanyId=@fxtcompanyid and Cityid=p.cityid) as varchar)+',' like '%,' + cast(p.Fxt_CompanyId as varchar) + ',%')
 @where