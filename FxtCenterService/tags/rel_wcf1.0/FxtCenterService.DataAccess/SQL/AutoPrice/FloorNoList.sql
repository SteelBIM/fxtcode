...
and hs.FxtCompanyId=@fxtcompanyid  and hs.CityId=h.CityId) @key
AND h.FxtCompanyId  IN (SELECT value FROM  dbo.SplitToTable((SELECT ShowCompanyId FROM dbo.Privi_Company_ShowData WHERE CityId=@cityid AND FxtCompanyId=@fxtcompanyid),',')) 
