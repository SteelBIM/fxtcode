select a.* from (
SELECT [ProjectId],[ProjectName] +'['+a.[AreaName]+']' ProjectName,isevalue  FROM @table_project p with(nolock),[dbo].SYS_Area a with(nolock)where p.[Valid]=1 and p.[CityId]=@cityid and p.[AreaID]=a.[AreaID] 
...
union 
SELECT [ProjectId],[ProjectName] +'['+a.[AreaName]+']' ProjectName,isevalue  FROM @table_project_sub p with(nolock),[dbo].SYS_Area a with(nolock)where p.[Valid]=1 and p.[CityId]=@cityid and p.[AreaID]=a.[AreaID] 
