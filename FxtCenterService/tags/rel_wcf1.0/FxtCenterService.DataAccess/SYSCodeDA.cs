    ...
    public class SYSCodeDA : BaseDA
    {
        ...
        public static List<CAS.Entity.SurveyDBEntity.SYSCode> GetSYSCodeList(int code)
        {
            ...
            if (code > 0)
            {
                ...
                parameters.Add(SqlHelper.GetSqlParameter("@id", code, SqlDbType.Int));
                sql += where;
            }
        ...
        {
            List<SqlParameter> parameters = new List<SqlParameter>();
            string sql = SQL.SQLName.SysCode.SYSCodeListByDicType;
            if (type > 0)
            {
                string where = " and dictype=@id";
                parameters.Add(SqlHelper.GetSqlParameter("@id", type, SqlDbType.Int));
                sql += where;
            }
            return ExecuteToEntityList<CAS.Entity.SurveyDBEntity.SYSCode>(sql, System.Data.CommandType.Text, parameters);
        }
