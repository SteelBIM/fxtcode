...
namespace FxtCenterService.Actualize
{
    public partial class DataController
    {
        ...
        public static string GetSearchProjectListByKey(JObject funinfo, UserCheck company)
        {
            ...
            var list = DatProjectBL.GetSearchProjectListByKey(search, company.parentshowdatacompanyid, search.CityId, key);
            return list.ToJson();
        }
        /// <summary>
        /// 获取楼盘详细信息
        ...
        public static string GetProjectDropDownList_MCAS(JObject funinfo, UserCheck company)
        {
            ...
            //LogHelper.Info("获取楼盘下拉列表MCAS");
            //LogHelper.Info(list.ToJson());
            return list.ToJson();
        }
        /// <summary>
        /// 获取楼盘下拉列表
        ...
        public static string GetBuildingBaseInfoList_MCAS(JObject funinfo, UserCheck company)
        {
            ...
            //LogHelper.Info(search.FxtCompanyId + ":" + search.SysTypeCode +":" + company.producttypecode);
            ...
            //LogHelper.Info("获取楼栋下拉列表forMCAS总共：" + ts2.TotalMilliseconds + "ms.");
            return result.ToJson();
        }
        /// <summary>
        /// 获取楼栋下拉列表
        ...
        public static string GetHouseNoList(JObject funinfo, UserCheck company)
        {
            ...
            //try
            //{
            //    //File.AppendAllText(dailyReportFullPath, "进行GetHouseNoList方法" + Environment.NewLine);
            //    if (FxtClientService.IfUseRpc(company.producttypecode, company.companyid))
            //    {
            //        //File.AppendAllText(dailyReportFullPath, "进行GetHouseNoList灰度发布方法" + Environment.NewLine);
            ...
            //        //File.AppendAllText(dailyReportFullPath, "进行GetHouseNoList灰度发布调用开始" + Environment.NewLine);
            //        FxtClientService.GetFloorHouseNoList(request, out floorHouseResponse);
            //        //File.AppendAllText(dailyReportFullPath, "进行GetHouseNoList灰度发布调用结束" + Environment.NewLine);
            //        //if (!floorHouseResponse.ResponseMsg.Success)
            //        //{
            //        //    LogHelper.Info(floorHouseResponse.ResponseMsg.Msg);
            //        //}
            //        //else
            //        //{
            //        //    LogHelper.Info("获取楼层列表成功");
            //        //}
            //        //File.AppendAllText(dailyReportFullPath, "GetHouseNoList灰度发布返回结果：" + floorHouseResponse.FloorHouses.Select(x => new { floorno = x.FloorNo, housecnt = x.HouseNoCount }).ToList().ToJson() + Environment.NewLine);
            //        return floorHouseResponse.FloorHouses.Select(x => new { floorno = x.FloorNo, housecnt = x.HouseNoCount }).ToList().ToJson();
            //    }
            //}
            //catch (System.Exception ex)
            //{
            //    //File.AppendAllText(dailyReportFullPath, "进行GetHouseNoList灰度发布错误:"+ex.Message + Environment.NewLine);
            //    //LogHelper.Info(ex.ToString());
            //    return "";
            //}
            //File.AppendAllText(dailyReportFullPath, "调用GetHouseNoList原来方法" + Environment.NewLine);
            key = HttpUtility.UrlDecode(key);
