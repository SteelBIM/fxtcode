...
namespace FxtCenterService.Logic
{
    public class SYSAreaBL
    {
        ...
        /// <returns></returns>
        public static List<SYSArea> GetSYSAreaList(SearchBase search, int[] areaid, int[] zipcode)
        {
            string areaidstr = (areaid == null || areaid.Length == 0) ? null : string.Join(",", areaid.Select(i => i.ToString()).ToArray());
            string zipcodestr = (zipcode == null || zipcode.Length == 0) ? null : string.Join(",", zipcode.Select(i => i.ToString()).ToArray());
