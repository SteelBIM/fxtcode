...
declare @tbl_prolist table(projectid int,cityid int,distince decimal(18,2))--符合条件的楼盘
...
if( @@x <= 0 or  @@y <= 0)
	begin
		...
		','+cast((select ShowCompanyId from FxtDataCenter.dbo.Privi_Company_ShowData with(nolock) where FxtCompanyId=@fxtcompanyid and Cityid=@cityid and TypeCode = @typecode) as varchar)+',' like '%,' + cast(p.FxtCompanyId as varchar) + ',%'
		union
...
if (@max_x > 0 and @min_x > 0 and @max_y> 0 and @min_y > 0)
	...
	--查询周边5公里范围内的楼盘
	insert into @tbl_prolist (projectid,cityid,distince)
	select projectid,cityid,distince from ( 
	select p.projectid,p.cityid
	...
	select p.projectid,p.cityid,p.x,p.y from @projecttable p with(nolock)
	where 1=1 
	...
	','+cast((select ShowCompanyId from FxtDataCenter.dbo.Privi_Company_ShowData with(nolock) where FxtCompanyId=@fxtcompanyid and Cityid=@cityid and TypeCode = @typecode) as varchar)+',' like '%,' + cast(p.FxtCompanyId as varchar) + ',%'
	union
	select p.projectid,p.cityid,p.x,p.y from @projectsubtable p with(nolock)
	where 1=1 
	and p.x >= @min_x and p.x <= @max_x and p.y >= @min_y and p.y <= @max_y --边为5公里的正方形之内
	and p.valid =1 
	and 
	...
	if(select count(1) from @tbl_prolist)>0
		begin
			...
			inner join @tbl_prolist t on t.projectid=c.projectid and t.cityid=c.cityid
			where c.purposecode=1002001 and c.buildingtypecode=@buildingtypecode
			and ','+cast((select ShowCompanyId from FxtDataCenter.dbo.Privi_Company_ShowData with(nolock) where FxtCompanyId=@fxtcompanyid and Cityid=@cityid and TypeCode = @typecode) as varchar)+',' like '%,' + cast(fxtcompanyid as varchar) + ',%'
			and (case 
			...
			and (REPLACE(buildingdate,'年','')) > @buildingstartdate and (REPLACE(buildingdate,'年','')) < @buildingenddate--竣工时间偏差3年
	...
	if(select count(1) from @tbl_caselist)<10 --无数据
		begin
			...
			--楼盘所在行政区楼盘
			insert into @tbl_prolist (projectid,cityid,distince)		
			select p.projectid,cityid,0 from (
			select p.projectid,p.cityid from @projecttable p with(nolock)
			where 1=1 
			and p.areaid=@areaid
			and p.projectid not in (select projectid from @projectsubtable ps with(nolock) where p.ProjectId=ps.ProjectId 
			and 
			...
			','+cast((select ShowCompanyId from FxtDataCenter.dbo.Privi_Company_ShowData with(nolock) where FxtCompanyId=@fxtcompanyid and Cityid=@cityid and TypeCode = @typecode) as varchar)+',' like '%,' + cast(p.FxtCompanyId as varchar) + ',%'
			union
			select p.projectid,p.cityid from @projectsubtable p with(nolock)
			where 1=1 
			and p.areaid=@areaid
			and p.valid =1 
			and 
