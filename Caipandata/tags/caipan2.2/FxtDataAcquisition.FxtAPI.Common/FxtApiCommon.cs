...
namespace FxtDataAcquisition.FxtAPI.Common
{
    public class FxtApiCommon
    {
        ...
        public static void GetNowApiInfo(int nowApiCode,List<UserCenter_Apps> appList, out string appUrl, out string appPwd, out string appKey)
        {
            appUrl = ""; appPwd = ""; appKey = "";
            if (appList != null)
            {
                UserCenter_Apps appInfo = appList.Where(obj => obj.AppId == nowApiCode).FirstOrDefault();
                if (appInfo != null)
                {
