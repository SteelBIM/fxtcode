/*
    作者:李晓东
    摘要:新建 押品检测 2014.03.04 
         2014.03.17 修改人:李晓东
                    修改:CDMap根据条件读取展现地图
         2014.03.26 修改人:贺黎亮
                    修改:
                    1.SetMpTypeText添加:押品面积,原估价值,现估价值,担保金额,原抵押率,现抵押率 类型
                    2.GetChecked, GetPCA方法提取到最外层
                    3.地图显示，添加:押品面积,原估价值,现估价值,担保金额,原抵押率,现抵押率 显示字段
         2014.03.31 修改人:贺黎亮
                    修改:
                    1.押品分类查询 表格显示为1个，图表显示为7个，
                     新增字段 押品总量 	押品面积 	原估价值 	现估价值 	担保金额 	原平均抵押率 	现
平均抵押率
                    2.数据为空或0不显示图表,没有数据显示暂无数据
        2014.04.1 
                    2.贷款时间 改为  放贷日期.去除按时间查询条件输入框
                    3.添加按 放贷款日期查询选项
         2014.04.24 修改人:贺黎亮
                    1.去除、现估价值，现平均抵押率字段页面显示
        2014.05.06 修改人:李晓东
                    删除:GetChecked
        2014.05.07 修改人:李晓东
                    修改:loadDetials 押品监测明细查询
*/
// 百度地图API功能
    ...
    new BMap.Geocoder().getPoint(name, function (point) {
        if (point) {
            ...
            ComplexCustomOverlay.prototype.initialize = function (map) {
                ...
                '<li id="q2">' +
                '    <ul>' +
                '        <li class="south bar" style="height:' + Math.ceil((this._data.Count / this._count) 
* 100) + 'px;"></li>' +
                '    </ul>' +
                '</li>' +
                '</ul>';
    ...
    var loadMap = function () {
        ...
        $('div.tab-content>div.form-inline>button', _maps).click(function () {
            ...
            CDMap(roc.text, function (mp) {
                ...
                $.extendAjax({
                    url: '/CollateralDetect/GetPCACount',
                    type: 'post',
                    data: sendData
                }, function (rdata) {
        ...
        loadClass = function () {
            ...
            $("ul.nav-pills>li", _class).click(function () {
                ...
                $.extendAjax({
                    url: '/CollateralDetect/GetCFCount',
                    type: 'post',
                    data: sendData
                }, function (rdata) {
                    ...
                    if (rdata.type == 1) {
                        ...
                        for (var i = 1; i <= 5; i++) {
                            ...
                            if (i == 1) {
                                $.tmpl("<tr><td>${label}</td><td>${data}</td><td>${buildingarea}</td><td>${loanamount}</td><td>${oldrate}</td><td>${guaranteeprice}</td></tr>", _data)
        ...
        loadDetials = function () {
            ...
            $("button", _detials).click(function () {
                ...
                $.extendAjax({
                    url: '/CollateralDetect/GetCollDetials',
                    type: 'post',
                    data: sendData
                }, function (rdata) {
