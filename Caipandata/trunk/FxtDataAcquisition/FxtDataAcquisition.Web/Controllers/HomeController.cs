namespace FxtDataAcquisition.Web.Controllers
{
    ...
    public class HomeController : BaseController
    {
        ...
        public ActionResult SelectCity(int provinceId, LoginUser userInfo)
        {
            AjaxResult result = new AjaxResult("");

            var provinceList = _unitOfWork.CityService.GetProvinceCityListBy(userInfo.UserName, userInfo.SignName, userInfo.AppList);
            //城市
            var cl = provinceList.Where(m => m.ProvinceId == provinceId).FirstOrDefault();

            var cityList = provinceList.Count > 0 && cl != null ? cl.CityList : new List<FxtApi_SYSCity>();
        ...
        public ActionResult SelectMenuTotalCount(LoginUser userInfo)
        {
            AjaxResult result = new AjaxResult("");
