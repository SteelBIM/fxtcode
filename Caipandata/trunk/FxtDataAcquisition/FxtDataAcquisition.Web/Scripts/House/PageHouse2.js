...
var HouseObj = {
    ...
    Floors: [],                //物理层
    //加载初始化
    InitLoad: function (buildingId, cityId, fxtcompanyId, totalFloor, pAveragePrice, averagePrice, priceDetail, proWeight, floors) {
    ...
    */
    AddFloorNo: function (floorNum) {
        if (ValidFloorNum(floorNum)) {
            ...
            var fno = this.GetFloorIndex(num);
            if (fno) {
                alert("楼层号已经存在");
                return false;
            }
            ...
            if (floorNum > 0) {
                ...
                FoeachAddFloorNo(floorNum, lastFloor);//添加楼层、面积、用途。。。。。。

                if (lastFloor < 0) lastFloor = 0;
                for (var i = 0; i < floorNum - lastFloor; i++) {
                    this.Floors.push(GetInt32(lastFloor) + GetInt32(i) + 1);
                }
            } else if (floorNum < 0) {
                //第一层
                var frstFloor = this.Floors[0];
                if (!frstFloor) {
                    frstFloor = 0;
                }
                FoeachAddFloorNo(floorNum, frstFloor);//添加楼层、面积、用途。。。。。。

                frstFloor = frstFloor >= 0 ? -1 : frstFloor - 1;

                for (var i = floorNum; i <= frstFloor; i++) {
                    this.Floors.push(GetInt32(i));
                }
            }
            ...
            this.TotalFloor = this.Floors.length;//重置总楼层数
   ...
   */
    DelFloorNo: function (floorNum) {
        if (ValidFloorNum(floorNum)) {
            ...
            if (!fno) {
                ...
                //最后一层
                var lastFloor = this.Floors[this.Floors.length - 1];
                FoeachDelFloorNo(floorNum, fno, lastFloor);//删除楼层、面积、用途。。。。。。
    ...
    */
    DelHouseName: function (floor, unitNo, houseName) {
        ...
        floor = HouseObj.GetFloorNo(floor);
        HouseAttr = {
            FloorNo: floor,
       ...
       */
        ...
        floor = HouseObj.GetFloorIndex(floor);
   ...
   */
    ...
    Submit: function (thisobj) {
        ...
        */
        ...
        //        //window.location.href = "/House/HouseDetails?buildingId=" + HouseObj.BuildingId;
        //    },
    ...
    AddHouseDetails: function (buildingid, allotId, fxtcompanyId, cityId, id) {
        var url = "/House/AddHouseDetails?BuildingId=" + buildingid + "&AllotId=" + allotId + "&FxtcompanyId=" + fxtcompanyId + "&CityID=" + cityId + "&Id=" + id;
        $.fancybox({
            'href': url,
            'width': 900,
            'height': 320,
    ...
    GetFloorIndex: function (floorNo) {
        for (var i = 0; i < this.Floors.length; i++) {
            if (floorNo == this.Floors[i]) {
                return i + 1;
            }
        }
    },
    GetFloorNo: function (floorIndex) {
        return this.Floors[floorIndex - 1];
        //for (var i = 0; i < this.Floors.length - 1; i++) {
        //    if (floorIndex-1 == i) {
        //        return i + 1;
        //    }
...
function JsonData(obj, valid) {
    ...
    for (var key in obj) {
        var json = {
            ...
            "UnitNo": obj[key]["UnitNo"], "RoomNo": obj[key]["RoomNo"], "CityID": obj[key]["CityID"],
...
function ValidFloorUnitNo(unitNo, houseName) {
    ...
    var flag = true;
    for (var i = 2; i < $(thead).length; i++) {
        if ((unitNo.toUpperCase()) + (houseName) == GetTrim($(thead[i]).text()).toUpperCase())//转换小写
        {
            flag = false;
            break;
        }
...
function GetThIndex(unitNo, houseName) {
    ...
    var index = 0;
    for (var i = 2; i < $(thead).length; i++) {
        if (unitNo.toUpperCase() + houseName == GetTrim($(thead[i]).text()).toUpperCase()) {
            index = i;
            break;
        }
    }
...
function FoeachAddFloorNo(floorNum, totalFloor) {
    for (var a = 0; a < tb_arr.length; a++) {
        ...
        if (floorNum > 0) {
            ...
            lastFloor = floorNum - totalFloor;

            for (var i = index ; i <= lastFloor; i++) {
                if (i == 0) continue;
                var tr = "<tr>";
                for (var j = 0; j < $(thead).length; j++) {
                    if (j == 0 || j == 1) {
                        tr += "<td>";
                        tr += GetInt32(totalFloor) + GetInt32(i);
                        tr += "</td>";
                    } else {
                        var no = $(thead).eq(j);
                        var unitno = GetTrim(no.attr("unitno"));
                        var roomno = GetTrim(no.attr("roomno"));
                        var houName = GetTrim(unitno + (GetInt32(totalFloor) + i + roomno));
                        tr += "<td>";
                        if (a == 0) {//房号项
            ...
            index = GetInt32(floorNum);
            lastFloor = totalFloor >= 0 ? -1 : totalFloor-1;
            //totalFloor = 0;
            for (var i = lastFloor; i >= index; i--) {
                if (i == 0) { continue; }
                var tr = "<tr>";
                for (var j = 0; j < $(thead).length; j++) {
                    if (j == 0 || j == 1) {
                        ...
                        var unitno = GetTrim(no.attr("unitno"));
                        var roomno = GetTrim(no.attr("roomno"));
                        var houName = GetTrim('-'+unitno + ( -i + roomno));
                        tr += "<td>";
                        if (a == 0) {//房号项
                            tr += houName;//房号=单元+楼层+室号(A101,A201,A301)
                            /*
                     ...
                     */
                            HouseAttr = {
                                FloorNo: GetTrim( + i),      //物理楼层
                                NominalFloor: GetTrim( + i), //名义层（实际层）
                                HouseName: houName,                     //房号名称
                                BuildingId: HouseObj.BuildingId,        //楼栋ID
                                CityID: HouseObj.CityID,                //城市ID
                                FxtCompanyId: HouseObj.FxtCompanyId,    //评估机构ID
                ...
                tr += "/</tr>";
                $(tobj).find("tbody").prepend(tr);
            }
        }


        //var length = GetInt32(floorNum) - GetInt32(totalFloor);
...
attr:房号、面积、用途、附属房屋类型、附属房屋面积、户型、
     ...
     面积确认、通风采光、装修、是否有厨房、阳台数、洗手间数
*/
function FoeachDelFloorNo(floorNum, floorIndex, totalFloor) {
    for (var a = 0; a < tb_arr.length; a++) {
        var tobj = GetCurrTableByTId("#tab_" + tb_arr[a]);//获取table
        var thead = GetTableHead(tobj);//获取table的单元列(包含列:物理层和实际层这两列)
        for (var j = 2; j < $(thead).length; j++) {
            var tr = $(tobj).find("tbody>tr").eq(GetInt32(floorIndex - 1));
            if (a == 0)//房号项
            {
...
function FoeachAddHouse(unitNo, houseName) {
    ...
    if (ValidFloorUnitNo(unitNo, houseName)) {
        for (var a = 0; a < tb_arr.length; a++) {
            var tobj = GetCurrTableByTId("#tab_" + tb_arr[a]);//获取table
            $(tobj).find("thead>tr").append("<th fanghao='" + houseName + "' unitno = '" + unitNo + "' roomno = '" + houseName + "'>" + (unitNo + houseName) + " </th>");//添加单元
            tobj = GetCurrTableByTId("#tab_" + tb_arr[a]);//重新获单元信息
            var thead = GetTableHead(tobj);//获取table的单元列(包含列:物理层和实际层这两列)
            for (var i = 0; i < HouseObj.Floors.length; i++) {
                //for (var i = 0; i < HouseObj.TotalFloor; i++) {
                var tr = "";
                if (a == 0) {//房号项 房号=单元+楼层+室号(A101,A201,A301)
                    var houName = unitNo + (HouseObj.Floors[i]) + houseName;
                    tr += "<td>";
                    tr += houName;
                    tr += "</td>";
                    /*
                    ...
                    */
                    HouseAttr = {
                        FloorNo: GetTrim(HouseObj.Floors[i]),      //物理楼层
                        NominalFloor: GetTrim(HouseObj.Floors[i]), //名义层（实际层）
                        HouseName: houName,                     //房号名称
                        BuildingId: HouseObj.BuildingId,        //楼栋ID
                        CityID: HouseObj.CityID,                //城市ID
                        FxtCompanyId: HouseObj.FxtCompanyId,    //评估机构ID
                        ...
                        Toilet: "",                             //洗手间数
                        NoiseCode: "",                           //噪音情况
                        UnitNo: GetTrim(GetTrim(unitNo)),                         //单元    
                        RoomNo: GetTrim((houseName)),                         //室号  
                        Valid: 1                                //是否有效
                    };
                    /*
                        新增房号
        ...
        for (var a = 0; a < tb_arr.length; a++) {
            ...
            for (var i = 0; i < HouseObj.Floors.length; i++) {
                if (a == 0)//房号项
                {
                    ...
                    */
                            HouseAttr = {
                                FloorNo: GetInt32(HouseObj.GetFloorNo(i + 1)),                     //物理楼层
...
function SaveUpdate(houseName, tobj, i, j, tableModule, newVal) {
    ...
    if (add_arr[houseName] != null && add_arr[houseName] != undefined) {
        ...
        if (update_arr[houseName] == null && update_arr[houseName] == undefined) {
            var no = $(tobj).find("thead>tr").eq(0).find("th").eq(j);
            var unitno = GetTrim(no.attr("unitno"));
            var roomno = GetTrim(no.attr("roomno"));
            HouseAttr = {
                FloorNo: HouseObj.GetFloorNo(i),//物理楼层
                UnitNo: GetTrim(unitno),//单元
                RoomNo: GetTrim(roomno),//单元
                HouseName: houseName,                   //房号名称
                BuildingId: HouseObj.BuildingId,        //楼栋ID
                CityID: HouseObj.CityID,                //城市ID
...
function loadFloor(floorNum) {
    ...
    for (var i = 0; i <= HouseObj.Floors.length - 1; i++) {
        louceng_str += "<option value='" + HouseObj.Floors[i] + "'>" + HouseObj.Floors[i] + "</option>";
        if (i == parseInt(HouseObj.Floors.length / 2)) {
            PriceFloorNo_str += "<option value='" + HouseObj.Floors[i] + "' selected='selected'>" + HouseObj.Floors[i] + "</option>";
        } else {
            PriceFloorNo_str += "<option value='" + HouseObj.Floors[i] + "'>" + HouseObj.Floors[i] + "</option>";
        }
    }
