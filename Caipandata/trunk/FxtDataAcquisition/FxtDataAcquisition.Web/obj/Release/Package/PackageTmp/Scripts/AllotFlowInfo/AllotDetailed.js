    ...
    DA_Confirm("提示", title, function (ok) {
        if (ok) {
            ...
            $.extendAjax(
                        ...
                        function (data) {
                            if (!data.result) {
                                DA_Alert(data.message, function () { });
                            }
                            else {
                                alert("操作成功!");
    ...
    $.extendAjax(
                ...
                function (data) {
                    if (!data.result) {
                        ...
                        $("#photoCount").html(data.data.photocount);
                        var dingwei = "未定";
                        ...
                        if (data.data.buildinglist != null) {
                            if (data.data.buildinglist.length > 0) {
                                $("#buildingInfo").show();
                            }
                            for (var i = 0; i < data.data.buildinglist.length; i++) {
                                var obj = data.data.buildinglist[i];
                            ...
                            if (data.data.buildinglist.length > 0) {
                                GetHouse(data.data.buildinglist[0].buildingid);
                            }
                        }
                    }
                },
...
function OpenProjectInfo(projectId, allotId) {
    var url = Url_AllotFlowInfo_EditProject_ByProjectIdAndAllotId._StringFormat(allotId);
    $.fancybox({
        'href': url,
        'width': 930,
        'height': 600,
        ...
        'type': 'iframe',
        'onClosed': function () {
            location.href = "/AllotFlowInfo/AllotDetailed?Id=" + allotId;
...
function OpenHouseInfo(buildingId, houseId, allotId) {
    var url = "/AllotFlowInfo/EditHouse?buildingId=" + buildingId + "&Id=" + houseId + "&allotId=" + allotId;
    $.fancybox({
        'href': url,
        'width': 930,
        'height': 600,
