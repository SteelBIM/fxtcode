...
{
    public class 石家庄 : NewDataSpider, INewDataRum
    {
        protected DataClassesDataContext db2 = new DataClassesDataContext();
        public void start()
        {

            网站爬取配置 obj = SpiderWebConfigManager.根据城市获取新浪二手房爬取配置("石家庄");
            if (obj != null)
            {
                NewDataRum newDataRum = new NewDataRum("石家庄", obj.域名, obj.列表页链接, obj.详细页面爬取频率, obj.列表页面爬取频率);
