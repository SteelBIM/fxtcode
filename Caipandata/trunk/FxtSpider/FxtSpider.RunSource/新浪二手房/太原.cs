...
{
    public class 太原 : NewDataSpider, INewDataRum
    {
        protected DataClassesDataContext db2 = new DataClassesDataContext();
        public void start()
        {

            网站爬取配置 obj = SpiderWebConfigManager.根据城市获取新浪二手房爬取配置("太原");
            if (obj != null)
            {
                NewDataRum newDataRum = new NewDataRum("太原", obj.域名, obj.列表页链接, obj.详细页面爬取频率, obj.列表页面爬取频率);
