...
namespace FxtSpider.RunSource.满堂红地产网
{
    public class NewDataSpider : INewDataRum
    {
        ...
        protected static string 网站名称 = WebsiteManager.满堂红地产网;
        protected static int 网站ID = WebsiteManager.满堂红地产网_ID;
        ...
        protected static 网站表 WebObj = WebsiteManager.GetWebById(WebsiteManager.满堂红地产网_ID);
        #endregion
        #region(出售房源_页面需要爬取的各字段正则)
        ///// <summary>
        ///// 楼盘名
        ///// </summary>
        //protected RegexInfo regex_lpm = new RegexInfo("地址：<a[^<>]+>[^<>]+</a>\\-\\-<a[^<>]+>[^<>]*</a>\\-\\-<a[^<>]+>([^<>]+)</a>", "$1");
        //protected RegexInfo regex_lpm2 = new RegexInfo(">楼盘：</span>([^<>]+)<", "$1");
        ///// <summary>
        ///// 行政区
        ///// </summary>
        //protected RegexInfo regex_xzq = new RegexInfo(">地址：(</span>[^<>]*|)<a[^<>]+>([^<>]+)</a>", "$2");
        ///// <summary>
        ///// 片区
        ///// </summary>
        //protected RegexInfo regex_pq = new RegexInfo(">地址：(</span>[^<>]*|)<a[^<>]+>[^<>]+</a>\\-\\-<a[^<>]+>([^<>]+)</a>", "$2");
        ///// <summary>
        ///// 面积
        ///// </summary>
        //protected RegexInfo regex_mj = new RegexInfo(">户  型：[^<>]*\\&nbsp\\;([^<>]+)平方<", "$1");//("型：</i>([^<>]+[^\\d]+)\\d+m&sup2;</p>", "$1");
        //protected RegexInfo regex_mj2 = new RegexInfo(">[^<>]*\\&nbsp\\;[^<>]*\\&nbsp\\;([^<>]+)平方<", "$1");//("型：</i>([^<>]+[^\\d]+)\\d+m&sup2;</p>", "$1");
        ///// <summary>
        ///// 单价
        ///// </summary>
        //protected RegexInfo regex_dj = new RegexInfo(">单  价：([^<>]+)元/平方<", "$1");
        ///// <summary>
        ///// 结构
        ///// </summary>
        ...
        ///// 总价
        ///// </summary>
        //protected RegexInfo regex_zj = new RegexInfo(">(售  价：|参考价\\:)<[^<>]+>([^<>]+)</[^<>]+>万", "$2");
        ///// <summary>
        ///// 所在楼层
        ///// </summary>
        //protected RegexInfo regex_szlc = new RegexInfo(">[^<>]*楼层：第(\\d+)层[^<>]+<", "$1");
        ///// <summary>
        ///// 总楼层
        ///// </summary>
        //protected RegexInfo regex_zlc = new RegexInfo(">[^<>]*楼层：第[^<>]+层（共([^<>]+)层）<", "$1");
        ///// <summary>
        ///// 户型
        ///// </summary>
        //protected RegexInfo regex_hx = new RegexInfo(">户  型：([^<>]+)\\&nbsp\\;", "$1");
        //protected RegexInfo regex_hx2 = new RegexInfo(">[^<>]*&nbsp;([^<>]+)\\&nbsp\\;[^<>]*平方<", "$1");
        ///// <summary>
        ///// 朝向
        ///// </summary>
        //protected RegexInfo regex_cx = new RegexInfo(">朝向：(</span>|)([^<>]+)\\&nbsp\\;<", "$2");
        ///// <summary>
        ///// 装修
        ///// </summary>
        //protected RegexInfo regex_zx = new RegexInfo(">装修：(</span>|)([^<>]+)<", "$2");
        ///// <summary>
        ///// 建筑年代
        ///// </summary>
        //protected RegexInfo regex_jznd = new RegexInfo(">楼龄：(</span>|)([^<>]+)年<", "$2");
        ///// <summary>
        ///// 信息(备注)
        ///// </summary>
        //protected RegexInfo regex_title = new RegexInfo("<div class=\"maintitle\"><[^<>]+>([^<>]+)</[^<>]+>", "$1");
        //protected RegexInfo regex_title2 = new RegexInfo("<div class=\"shop_fyxxy_bt\">([^<>]+)<", "$1");
        ///// <summary>
        ///// 电话
        ///// </summary>
        //protected RegexInfo regex_phone = new RegexInfo("<div [^<>]*class=\"tel_info\"[^<>]*><span [^<>]*class=\"big_tel\"[^<>]*>([^<>]+)<", "$1");
        //protected RegexInfo regex_phone2 = new RegexInfo(">咨询电话\\:<[^<>]+>([^<>]+)<", "$1");
        ///// <summary>
        ///// URL
        ///// </summary>
        //protected RegexInfo regex_infUrl = new RegexInfo("<div class=\"title\"[^<>]*><a href=\"([^\"]+)\"[^<>]*>", "$1");
        ///// <summary>
        ///// 下一页正则
        ///// </summary>
        ...
        ///// 地址
        ///// </summary>
        //protected RegexInfo regex_address = new RegexInfo(">地址：(</span>[^<>]*|)<a[^<>]+>([^<>]*)</a>\\-\\-<a[^<>]+>([^<>]*)</a>\\-\\-<a[^<>]+>([^<>]*)</a>(\\-\\-<a[^<>]+>([^<>]*)</a>|)<", "$2$3$4$6");
        ///// <summary>
        ///// 信息的发布时间
        ///// </summary>
        ...
        //RegexInfo 总页数正则 = new RegexInfo("<[^<>]+>([^<>]+)</[^<>]+><[^<>]+>下一页</[^<>]+>", "$1");
        ...
        public void start()
        {
            List<VIEW_网站爬取配置_城市表_网站表> list = SpiderWebConfigManager.获取满堂红地产网下所有城市爬取配置();
        ...
        public void SpiderHouse(string hostUrl, string pageListIndexUrl, int rate, int pageCheckRate)
        {
            ...
            //stest.Append(SpiderRegexInfoHelp.GetRegexInfoXmlByObj(总页数正则, "总条数正则", "总条数正则"));
            ...
            try
            {
                ...
                Dictionary<string, List<string>> 根页面正则字典集合结果 = SpiderHelp.GetHtmlByRegex(pageListIndexUrl, "gb2312", 根页面正则字典集合, WebObj, CityId,keepAlive:true);
                int pageCount = 根页面正则字典集合结果["*总页数"].Count < 1 ? 0 : Convert.ToInt32(根页面正则字典集合结果["*总页数"][0]);
                string 根页面分页链接参数 = pageListIndexUrl + "&p={0}";
                string 根页面下一页链接 = pageListIndexUrl;
                int 当前总页数 = pageCount;
                int 当前页码 = 1;

                log.Debug(string.Format("满堂红地产网SpiderHouse()--获取总页数{0},hostUrl:{1}, pageListUrl:{2}, cityName{3}", pageCount, hostUrl, pageListIndexUrl, CityName));
                while (!string.IsNullOrEmpty(根页面下一页链接))
                {
                    ...
                    log.Debug(string.Format("满堂红地产网SpiderHouse()--获取根页面下一页链接{0},hostUrl:{1}, pageListUrl:{2}, cityName{3}", nowPageList, hostUrl, pageListIndexUrl, CityName));
                    ...
                    {
                        根页面下一页链接 = string.Format(根页面分页链接参数, ((当前页码 - 1) * pageSize).ToString());
                    }
                    if (isNowPageStop)
                    {
                        break;
                ...
                log.Debug(string.Format("满堂红地产网SpiderHouse()--获取根页面下信息吸取完成,{0},hostUrl:{1}, pageListUrl:{2}, cityName{3}", 根页面下一页链接, hostUrl, pageListIndexUrl, CityName));

            }
            catch (Exception ex)
            {
                log.Error(string.Format("满堂红地产网SpiderHouse()异常,hostUrl:{0}, pageListIndexUrl:{1}, cityName{2}", hostUrl, pageListIndexUrl, CityName), ex);
            }
            log.Debug(string.Format("满堂红地产网SpiderHouse()--获取{0}页面下详细信息Url吸取完成,详细页面url内容正则析取中-,hostUrl:{1}, pageListUrl:{2}", CityName, hostUrl, pageListIndexUrl));
        ...
        {
            //url = "http://bj.esf.loupan.com/14970583.html";
            try
            {
                //设置各字段规则(正则)
                ...
                //根据规则获取数据
                dicRegexInfo_List = SpiderHelp.GetHtmlByRegex(url, "gb2312", dicRegexInfo, WebObj, CityId, keepAlive: true);
                ...
                //计算建筑年代
                if (!string.IsNullOrEmpty(value_jznd) && value_jznd.IsInteger())
                {
                    value_jznd = nowDate.AddYears(0 - Convert.ToInt32(value_jznd)).ToString("yyyy");
                }
                //计算单价
                if (string.IsNullOrEmpty(value_dj) && value_mj.CheckDecimal() && value_zj.CheckDecimal() && Convert.ToDecimal(value_mj) > 0)
                {
                    value_dj = Convert.ToInt32((Convert.ToDecimal(value_zj) / Convert.ToDecimal(value_mj))*10000).ToString();
                }
                //将数据添加到字典 用于excel
        ...
        public void SpiderHouseByPageListUrl(string hostUrl, string pageListUrl, int rate, int pageCheckRate, out string 下一页链接)
        {
            ...
            try
            {
                ...
                Dictionary<string, List<string>> dicRegexItem_List = SpiderHelp.GetHtmlByRegex(url_sz, "gb2312", dicRegexItem, WebObj, CityId, keepAlive: true);
