...
namespace FxtSpider.RunSource.楼盘网
{
    public class NewDataSpider : INewDataRum
    {
        ...
        protected static string 网站名称 = WebsiteManager.楼盘网;
        protected static int 网站ID = WebsiteManager.楼盘网_ID;
        ...
        protected static 网站表 WebObj = WebsiteManager.GetWebById(WebsiteManager.楼盘网_ID);
        #endregion
        #region(出售房源_页面需要爬取的各字段正则)
        ///// <summary>
        ///// 楼盘名
        ///// </summary>
        //protected RegexInfo regex_lpm = new RegexInfo("置：</[^<>]+>[^<>]*<a[^<>]+>[^<>]*</a>([^<>]+)</[^<>]+>", "$1");
        ///// <summary>
        ///// 行政区
        ///// </summary>
        //protected RegexInfo regex_xzq = new RegexInfo("位置：</[^<>]+>[^<>]*<a[^<>]+>([^<>]+)</a>", "$1");
        ///// <summary>
        ///// 片区
        ///// </summary>
        ...
        ///// 面积
        ///// </summary>
        //protected RegexInfo regex_mj = new RegexInfo("户型：</[^<>]+>([^<>]*[^\\d]+|)(\\d+)m\\&sup2\\;", "$2");//("型：</i>([^<>]+[^\\d]+)\\d+m&sup2;</p>", "$1");
        ///// <summary>
        ///// 单价
        ///// </summary>
        //protected RegexInfo regex_dj = new RegexInfo("价：</[^<>]+><b>[^<>]*</b>[^<>]*万[^<>]*\\(([^<>]+)元/m2", "$1");
        ///// <summary>
        ///// 结构
        ///// </summary>
        ...
        ///// 用途
        ///// </summary>
        //protected RegexInfo regex_yt = new RegexInfo("类型：</[^<>]+>([^<>\\&\\d]+)(\\d*/\\d*层|)", "$1");
        ///// <summary>
        ///// 总价
        ///// </summary>
        //protected RegexInfo regex_zj = new RegexInfo("售价：</[^<>]+><b>(\\d+)</b>[^<>]*万", "$1");
        ///// <summary>
        ///// 所在楼层
        ///// </summary>
        //protected RegexInfo regex_szlc = new RegexInfo("类型：</[^<>]+>[^<>\\d]*(\\d+)/\\d+层</", "$1");
        ///// <summary>
        ///// 总楼层
        ///// </summary>
        //protected RegexInfo regex_zlc = new RegexInfo("类型：</[^<>]+>[^<>\\d]*\\d+/(\\d+)层</", "$1");
        ///// <summary>
        ///// 户型
        ///// </summary>
        //protected RegexInfo regex_hx = new RegexInfo("户型：</[^<>]+>([^<>]+)\\d+m\\&sup2\\;</[^<>]+>", "$1");
        ///// <summary>
        ///// 朝向
        ///// </summary>
        //protected RegexInfo regex_cx = new RegexInfo("装修：</[^<>]+>[^<>]+([^<>\\s]+)\\s*</[^<>]+>", "$1");
        ///// <summary>
        ///// 装修
        ///// </summary>
        //protected RegexInfo regex_zx = new RegexInfo("装修：</[^<>]+>\\s*([^<>\\s]+)", "$1");
        ///// <summary>
        ///// 建筑年代
        ///// </summary>
        //protected RegexInfo regex_jznd = new RegexInfo("产权：</[^<>]+>[^<>]+[^\\d]+(\\d+)年建</[^<>]+>", "$1");
        ///// <summary>
        ///// 信息(备注)
        ///// </summary>
        //protected RegexInfo regex_title = new RegexInfo("class=\"by\"><p>([^<>]+)<", "$1");
        ///// <summary>
        ///// 电话
        ///// </summary>
        //protected RegexInfo regex_phone = new RegexInfo("class=\"call\"><p>([^<>]+)<", "$1");
        ///// <summary>
        ///// URL
        ///// </summary>
        //protected RegexInfo regex_infUrl = new RegexInfo("<a [^<>]*onclick=\"goUrl\\(([^\"]+)\\,this\\)\">[^<>]+</a>", "/tmp/$1.html");
        //protected RegexInfo regex_infUrl2 = new RegexInfo("</span><h2><a [^<>]*href=\"/([^\"]+)\"[^<>]*>[^<>]+</a>", "/$1");
        ///// <summary>
        ///// 下一页正则
        ///// </summary>
        ...
        ///// 信息的发布时间
        ///// </summary>
        //protected RegexInfo regex_datetime = new RegexInfo("发布时间:<span>([^<>]+)</span>", "$1");
        ///// <summary>
        ///// 中介公司
        ///// </summary>
        ...
        //protected RegexInfo 总页数正则 = new RegexInfo("\\d+/(\\d+)</span>页", "$1");
        #endregion
        ...
        protected RegexInfo regex_infUrl2 = SpiderRegexInfoHelp.GetRegexInfoByXmlName("regex_infUrl2", 网站名称);
        /// <summary>
        /// 下一页正则
        /// </summary>
        ...
        public void start()
        {
            List<VIEW_网站爬取配置_城市表_网站表> list = SpiderWebConfigManager.获取楼盘网下所有城市爬取配置();
        ...
        public void SpiderHouse(string hostUrl, string pageListIndexUrl, int rate, int pageCheckRate)
        {
            ...
            //stest.Append(SpiderRegexInfoHelp.GetRegexInfoXmlByObj(regex_infUrl2, "regex_infUrl2", "url")); ;
            ...
            try
            {
                ...
                string 根页面分页链接参数 = pageListIndexUrl.Replace("/p1/", "/p{0}/");
                string 根页面下一页链接 = pageListIndexUrl;
                int 当前总页数 = pageCount;
                int 当前页码 = 1;

                log.Debug(string.Format("楼盘网SpiderHouse()--获取总页数{0},hostUrl:{1}, pageListUrl:{2}, cityName{3}", pageCount, hostUrl, pageListIndexUrl, CityName));
                while (!string.IsNullOrEmpty(根页面下一页链接))
                {
                    ...
                    log.Debug(string.Format("楼盘网SpiderHouse()--获取根页面下一页链接{0},hostUrl:{1}, pageListUrl:{2}, cityName{3}", nowPageList, hostUrl, pageListIndexUrl, CityName));
                ...
                log.Debug(string.Format("楼盘网SpiderHouse()--获取根页面下信息吸取完成,{0},hostUrl:{1}, pageListUrl:{2}, cityName{3}", 根页面下一页链接, hostUrl, pageListIndexUrl, CityName));

            }
            catch (Exception ex)
            {
                log.Error(string.Format("楼盘网SpiderHouse()异常,hostUrl:{0}, pageListIndexUrl:{1}, cityName{2}", hostUrl, pageListIndexUrl, CityName), ex);
            }
            log.Debug(string.Format("楼盘网SpiderHouse()--获取{0}页面下详细信息Url吸取完成,详细页面url内容正则析取中-,hostUrl:{1}, pageListUrl:{2}", CityName, hostUrl, pageListIndexUrl));
        ...
        {
            //url = "http://bj.esf.loupan.com/14969114.html";
            try
            {
                //设置各字段规则(正则)
                ...
                string value_jznd = dicRegexInfo_List["regex_jznd"].Count < 1 ? "" : dicRegexInfo_List["regex_jznd"][0].TrimBlank();
                string value_cx = dicRegexInfo_List["regex_cx"].Count < 1 ? "" : dicRegexInfo_List["regex_cx"][0].TrimBlank();
                ...
                string value_zx = dicRegexInfo_List["regex_zx"].Count < 1 ? "" : dicRegexInfo_List["regex_zx"][0].TrimBlank();
