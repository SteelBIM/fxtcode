...
namespace FxtSpider.RunSource.黄石信息港
{
    public class NewDataSpider : INewDataRum
    {
        ...
        protected static string 网站名称 = WebsiteManager.黄石信息港;
        protected static int 网站ID = WebsiteManager.黄石信息港_ID;
        ...
        protected static 网站表 WebObj = WebsiteManager.GetWebById(WebsiteManager.黄石信息港_ID);
        #endregion
        #region(出售房源_页面需要爬取的各字段正则)
        ///// <summary>
        ///// 楼盘名
        ///// </summary>
        //protected RegexInfo regex_lpm = new RegexInfo("<td >小区：<a href=\"[^\"]*\">([^<>]+)</a></td>", "$1");

        ///// <summary>
        ///// 行政区
        ///// </summary>
        //protected RegexInfo regex_xzq = new RegexInfo(">区域：([^<>]*)</td>", "$1");
        ///// <summary>
        ///// 片区
        ///// </summary>
        //protected RegexInfo regex_pq = new RegexInfo(">地段：([^<>]*)</td>", "$1");
        ///// <summary>
        ///// 面积
        ///// </summary>
        ...
        ///// 单价
        ///// </summary>
        //protected RegexInfo regex_dj = new RegexInfo("单价：<font[^<>]*>([^<>]+)</font>元", "$1");
        ///// <summary>
        ///// 结构
        ///// </summary>
        ...
        ///// 用途
        ///// </summary>
        //protected RegexInfo regex_yt = new RegexInfo(">类型：([^<>]*)<", "$1");
        ///// <summary>
        ///// 总价
        ///// </summary>
        //protected RegexInfo regex_zj = new RegexInfo("价格：</em><span[^<>]*>([^<>]+)</span>万元", "$1");
        ///// <summary>
        ///// 所在楼层
        ///// </summary>
        //protected RegexInfo regex_szlc = new RegexInfo(">楼层：(\\d*)/\\d+<", "$1");
        ///// <summary>
        ///// 总楼层
        ///// </summary>
        //protected RegexInfo regex_zlc = new RegexInfo(">楼层：\\d*/(\\d+)<", "$1");
        ///// <summary>
        ///// 户型
        ///// </summary>
        //protected RegexInfo regex_hx = new RegexInfo(">房型：([^<>]+)<", "$1");     
        ///// <summary>
        ///// 朝向
        ///// </summary>
        ...
        ///// 建筑年代
        ///// </summary>
        //protected RegexInfo regex_jznd = new RegexInfo(">房龄：(\\d+)年<", "$1");
        ///// <summary>
        ///// 信息(备注)
        ///// </summary>
        //protected RegexInfo regex_title = new RegexInfo("<div class=\"right\">(?:(?!<h3>).)*<h3>([^<>]*)</h3>", "$1");
        ///// <summary>
        ///// 电话
        ///// </summary>
        //protected RegexInfo regex_phone = new RegexInfo("<p class=\"clearfix\"><span class=\"tel\"[^<>]*>(\\d*)<", "$1");       
        ///// <summary>
        ///// URL
        ///// </summary>
        //protected RegexInfo regex_infUrl = new RegexInfo("<li class=\"item\"><div class=\"pic\"><a href=\"([^\"]+)\" target=_blank><img", "$1");
        ///// <summary>
        ///// 下一页正则
        ///// </summary>
        ...
        ///// 地址
        ///// </summary>
        //protected RegexInfo regex_address =new RegexInfo("<td >地址：([^<>]+)</td>", "$1");
        ///// <summary>
        ///// 信息的发布时间
        ///// </summary>
        ...
        ///// 中介公司
        ///// </summary>
        //protected RegexInfo regex_comName = new RegexInfo("所属公司：</em><a href=\"[^\"]*\">([^<>]*)</a>", "$1");
        ///// <summary>
        ///// 门店地址
        ///// </summary>
        //protected RegexInfo regex_comArea = new RegexInfo("服务区域：</em><a href=\"[^\"]*\">([^<>]*)</a>", "$1");

        //protected RegexInfo 总页数正则 = new RegexInfo("<a href=\"[^\"]*PageNo=(\\d+)\\&[^\"]*\" title=\"[^\"]*\">末页</a>", "$1");
        ...
        public void start()
        {
            List<VIEW_网站爬取配置_城市表_网站表> list = SpiderWebConfigManager.获取黄石信息港下所有城市爬取配置();
        ...
        public void SpiderHouse(string hostUrl, string pageListIndexUrl, int rate, int pageCheckRate)
        {
            ...
            try
            {
                ...
                while (!string.IsNullOrEmpty(根页面下一页链接))
                {
                    ...
                    log.Debug(string.Format("黄石信息港SpiderHouse()--获取根页面下一页链接{0},hostUrl:{1}, pageListUrl:{2}, cityName{3}", nowPageList, hostUrl, pageListIndexUrl, CityName));
                ...
                log.Debug(string.Format("黄石信息港SpiderHouse()--获取根页面下信息吸取完成,{0},hostUrl:{1}, pageListUrl:{2}, cityName{3}", 根页面下一页链接, hostUrl, pageListIndexUrl, CityName));

            }
            catch (Exception ex)
            {
                log.Error(string.Format("黄石信息港SpiderHouse()异常,hostUrl:{0}, pageListIndexUrl:{1}, cityName{2}", hostUrl, pageListIndexUrl, CityName), ex);
            }
            log.Debug(string.Format("黄石信息港SpiderHouse()--获取{0}页面下详细信息Url吸取完成,详细页面url内容正则析取中-,hostUrl:{1}, pageListUrl:{2}", CityName, hostUrl, pageListIndexUrl));
        ...
        public void GetHouseByUrl(string url)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(value_lpm.TrimBlank())&&!string.IsNullOrEmpty(value_address.TrimBlank()))
                {
                    value_lpm = value_address;
                }
                //如果所在楼层和总楼层顺序颠倒
        ...
        public void SpiderHouseByPageListUrl(string hostUrl, string pageListUrl, int rate, int pageCheckRate, out string 下一页链接)
        {
            ...
            try
            {
                ...
                {
                    下一页链接 = hostUrl + "/house/chushou.html" + 下一页链接;
                }
        ...
        /// <returns></returns>
        public bool CheckPurpose(string value_yt)
        {
            string[] strings = new string[] { "公寓", "普通住宅", "别墅" };
            if (string.IsNullOrEmpty(value_yt.TrimBlank()))
            {
                return true;
            }
            value_yt = value_yt.Trim();
            if (!strings.Contains(value_yt))
            {
                return false;
            }
            return true;
