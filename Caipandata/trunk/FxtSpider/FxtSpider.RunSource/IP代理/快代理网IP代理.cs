...
{
    public class 快代理网IP代理
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(快代理网IP代理));
        public void Run()
        {
            //ip列表页ip信息
            RegexInfo regex_ipInfo = new RegexInfo("(<tr><td>[^<>]+</td><td>[^<>]+</td><td>[^<>]+</td><td>[^<>]+</td><td>[^<>]+</td><td>[^<>]+</td><td>[^<>]+</td></tr>)", "$1");
            //ip列表页ip
            RegexInfo regex_ip = new RegexInfo("<tr><td>([^<>]+)</td><td>([^<>]+)</td><td>[^<>]+</td><td>[^<>]+</td><td>[^<>]+</td><td>[^<>]+</td><td>[^<>]+</td></tr>", "$1:$2");
            //ip列表页ip区域
            RegexInfo regex_area = new RegexInfo("<tr><td>[^<>]+</td><td>[^<>]+</td><td>[^<>]+</td><td>[^<>]+</td><td>([^<>]+)</td><td>[^<>]+</td><td>[^<>]+</td></tr>", "$1");
            //ip列表页下一页
            RegexInfo regex_nextPage = new RegexInfo("<a href=\"[^\"]+\" class=\"active\">[^<>]+</a></li><li><a href=\"([^\"]+)\">[^<>]+</a>", "$1");
            ...
            string configPath = AppDomain.CurrentDomain.BaseDirectory + "IP代理/Config/快代理网IP代理ProxyIpConfig.txt";
            ...
            sr.Close();
            sr.Dispose();
            urlList.Add("http://www.kuaidaili.com/free/intr/1/");
            //但从当前页面中获取爬取ip分类列表
            RegexInfo regex_urllist = new RegexInfo("", "$1");
            RegexInfo regex_urllist_pagecount = new RegexInfo("", "$1");
            ...
            string urllist_nextPage_para = "";//下一页
            string urllist_nextPage = "";//下一页
            ...
            begin_nextpage:
                ...
                List<string> urlList2 = new List<string>(); //dicValueList3["regex_urllist"];//所有ip列表集合
                urlList.AddRange(urlList2);
                //获取ip列表列表页的最大页数
                urllist_max_index = 0;//dicValueList3["regex_urllist_pagecount"].Count < 1 ? 0 : Convert.ToInt32(dicValueList3["regex_urllist_pagecount"][0].TrimBlank());
                ...
                for (int i = 0; i < urlList.Count(); i++)
                {
                    ...
                    string urlHost = urlInfo.Split('$').Length < 2 ? "http://www.kuaidaili.com" : urlInfo.Split('$')[1];
                requestpage:
                    //根据ip列表页url爬取ip信息
