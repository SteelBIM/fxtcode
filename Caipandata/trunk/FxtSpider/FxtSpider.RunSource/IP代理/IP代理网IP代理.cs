...
{
    public class IP代理网IP代理
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(IP代理网IP代理));
        public void Run()
        {
            //ip列表页ip信息
            ...
            string configPath = AppDomain.CurrentDomain.BaseDirectory + "IP代理/Config/IP代理网IP代理ProxyIpConfig.txt";
            ...
            sr.Dispose();
            //但从当前页面中获取爬取ip分类列表
            RegexInfo regex_urllist = new RegexInfo("\\] <a href=\"([^\"]+)\"[^<>]*><h2>[^<>]*</h2><", "$1");
            RegexInfo regex_urllist_pagecount = new RegexInfo("> 页次：\\d+/(\\d+)页 共\\d+条记录 \\d+条/页 转第 <", "$1");
            ...
            string urllist_nextPage_para = "http://www.ip-daili.com/xw/index.asp?id=4&page={0}";//下一页
            ...
            begin_nextpage:
                ...
                for (int i = 0; i < urlList.Count(); i++)
                {
                    ...
                    string urlHost = urlInfo.Split('$').Length < 2 ? "http://www.ip-daili.com" : urlInfo.Split('$')[1];
                requestpage:
                    //根据ip列表页url爬取ip信息
