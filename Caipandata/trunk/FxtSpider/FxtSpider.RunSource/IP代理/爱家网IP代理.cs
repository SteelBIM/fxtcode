...
{
    public class 爱家网IP代理
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(爱家网IP代理));
        public void Run()
        {
            //ip列表页ip信息
            RegexInfo regex_ipInfo = new RegexInfo("([^<>]+\\@[^<>]+<br />)", "$1");
            //ip列表页ip
            RegexInfo regex_ip = new RegexInfo("([^<>]+)\\@[^<>]+<br />", "$1");
            //ip列表页ip区域
            RegexInfo regex_area = new RegexInfo("[^<>]+\\#([^<>]+)<br />", "$1");
            //ip列表页下一页
            RegexInfo regex_nextPage = new RegexInfo("<a href=\"javascript:ContentPageHref\\(''\\,[^\\,]*\\,[^\\,]*\\,'([^\"']+)'\\)\\;\"[^<>]*>下一页</a>", "$1");
            ...
            string configPath = AppDomain.CurrentDomain.BaseDirectory + "IP代理/Config/爱家网IP代理ProxyIpConfig.txt";
            ...
            sr.Dispose();
            //但从当前页面中获取爬取ip分类列表
            RegexInfo regex_urllist = new RegexInfo("><h\\d*><a href='([^\"']+)'[^<>]*target='_blank'[^<>]*>[^<>]+</a></h\\d*>", "$1");
            RegexInfo regex_urllist_pagecount = new RegexInfo(">第(\\d+)/(\\d+)页", "$1");
            ...
            string urllist_nextPage_para = "http://www.ajshw.net/news/?list_2_{0}.html";//下一页
            ...
            begin_nextpage:
                ...
                {
                    string urlInfo = urlList[i].Replace("../","/");
                    string url = urlInfo.Split('$')[0];
                    string urlHost = urlInfo.Split('$').Length < 2 ? "http://www.ajshw.net/" : urlInfo.Split('$')[1];
                    int ipPageIndex = 1;
                requestpage:
                    ...
                    {
                        ipPageIndex = ipPageIndex + 1;
                        nextPage = nextPage.Replace("../../../", "/");
                        nextPage = nextPage.Replace("[page]", ipPageIndex.ToString());
