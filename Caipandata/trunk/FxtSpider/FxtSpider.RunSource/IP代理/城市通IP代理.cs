...
{
    public class 城市通IP代理
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(城市通IP代理));
        public void Run()
        {
            //ip列表页ip信息
            RegexInfo regex_ipInfo = new RegexInfo("([^<>]+\\@[^<>]+(<br />|</span>))", "$1");
            //ip列表页ip
            RegexInfo regex_ip = new RegexInfo("([^<>]+)\\@[^<>]+(<br />|</span>)", "$1");
            //ip列表页ip区域
            RegexInfo regex_area = new RegexInfo("[^<>]+\\#([^<>]+)(<br />|</span>)", "$1");
            ...
            string configPath = AppDomain.CurrentDomain.BaseDirectory + "IP代理/Config/故意网IP代理ProxyIpConfig.txt";
            ...
            sr.Dispose();
            //但从当前页面中获取爬取ip分类列表
            RegexInfo regex_urllist = new RegexInfo("<dt class=\"clearfix\"><a [^<>]*href=\"([^\"]+)\"[^<>]*>[^<>]+</a></dt>", "$1");
            RegexInfo regex_urllist_pagecount = new RegexInfo("<li><a href='list_\\d+_(\\d+)\\.html'>末页</a></li>", "$1");
            ...
            string urllist_nextPage_para = "http://www.mesk.cn/ip/china/list_70_{0}.html";//下一页
            ...
            begin_nextpage:
                ...
                for (int i = 0; i < urlList.Count(); i++)
                {
                    ...
                    string urlHost = urlInfo.Split('$').Length < 2 ? "http://www.mesk.cn" : urlInfo.Split('$')[1];
                requestpage:
