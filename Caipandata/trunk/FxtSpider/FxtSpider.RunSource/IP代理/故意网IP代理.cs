...
{
    public class 故意网IP代理
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(故意网IP代理));
        public void Run()
        {
            //ip列表页ip信息
            ...
            sr.Dispose();
            //但从当前页面中获取爬取ip分类列表
            RegexInfo regex_urllist = new RegexInfo("国内代理</a>\\]</em>[^<>]*<a href=\"([^\"]+)\"[^<>]*>", "$1");
            ...
            string urllist_nextPage_para = "http://www.gyupu.co/forum.php?mod=forumdisplay&fid=320&typeid=267&typeid=267&filter=typeid&t=4589926&page={0}";//下一页
            ...
            begin_nextpage:
                ...
                for (int i = 0; i < urlList.Count(); i++)
                {
                    ...
                    string urlHost = urlInfo.Split('$').Length < 2 ? "http://www.gyupu.co/" : urlInfo.Split('$')[1];
                requestpage:
