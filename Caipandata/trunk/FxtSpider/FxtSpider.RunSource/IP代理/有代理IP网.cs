...
namespace FxtSpider.RunSource.IP代理
{
    public class 有代理IP网
    {
        ...
        public void Run()
        {
            ...
            RegexInfo regex_nextPage = new RegexInfo("<a href='([^\'\"]+)'>下一页</a>", "$1");
            ...
            string configPath = AppDomain.CurrentDomain.BaseDirectory + "IP代理/Config/有代理ProxyIpConfig.txt";
            ...
            sr.Dispose();
            //但从当前页面中获取爬取列表
            RegexInfo regex_urllist = new RegexInfo("<li><a href=\"([^\"]+)\"[^<>]*><[^<>]+>【国内代理】", "$1");
            RegexInfo regex_urllist_pagecount = new RegexInfo("共[^<>]*<strong>([^<>]+)</strong>页", "$1");
            ...
            string urllist_nextPage_para = "http://www.youdaili.cn/Daili/guonei/list_{0}.html";//下一页
            ...
            begin_nextpage:
                ...
                urllist_max_index = dicValueList3["regex_urllist_pagecount"].Count < 1 ? 0 : Convert.ToInt32(dicValueList3["regex_urllist_pagecount"][0]);
                ...
                foreach (string urlInfo in urlList)
                {
                    ...
                    string urlHost = urlInfo.Split('$').Length < 2 ? "http://www.youdaili.cn/Daili/guonei/" : urlInfo.Split('$')[1];
                    requestpage:
