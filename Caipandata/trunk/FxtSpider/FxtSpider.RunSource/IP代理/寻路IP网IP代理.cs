...
{
    public class 寻路IP网IP代理
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(寻路IP网IP代理));
        public void Run()
        {
            //ip列表页ip信息
            ...
            string configPath = AppDomain.CurrentDomain.BaseDirectory + "IP代理/Config/寻路IP网IP代理ProxyIpConfig.txt";
            ...
            sr.Dispose();
            //但从当前页面中获取爬取ip分类列表
            RegexInfo regex_urllist = new RegexInfo("<li><span class=\"rt\">[^<>]*</span>·<a href=\"([^\"]+)\"[^<>]*>[^<>]+</a></li>", "$1");
            RegexInfo regex_urllist_pagecount = new RegexInfo("<div id=\"pages\"[^<>]*><a[^<>]*>([^<>]+)条</a>", "$1");
            ...
            string urllist_nextPage_para = "http://www.xunluw.com/DomesticIP/{0}.html";//下一页
            string urllist_nextPage = "http://www.xunluw.com/DomesticIP/"; //string.Format(urllist_nextPage_para, urllist_index);//下一页
            ...
            begin_nextpage:
                ...
                for (int i = 0; i < urlList.Count(); i++)
                {
                    ...
                    string urlHost = urlInfo.Split('$').Length < 2 ? "http://www.xunluw.com" : urlInfo.Split('$')[1];
                requestpage:
                    //根据ip列表页url爬取ip信息
