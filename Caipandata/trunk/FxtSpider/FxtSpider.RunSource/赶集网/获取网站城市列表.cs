...
namespace FxtSpider.RunSource.赶集网
{
    public class 获取网站城市列表 : INewDataRum
    {
        public void start()
        {
            RegexInfo cityRegexInfo = new RegexInfo("<div class=\"all-city\">((?:(?!</div>).)*)</div>", "$1");
            ...
            Dictionary<string, List<string>> dicCitylistText = SpiderHelp.GetHtmlByRegex("http://www.ganji.com/index.htm", "utf-8", cityRegexDic,WebsiteManager.GetWebById(WebsiteManager.赶集网_ID),CityId);
            ...
            foreach (string cityInfoStr in cityList)
            {
                ...
                if (city != null && SpiderWebConfigManager.get网站爬取配置_by城市ID_网站ID(city.ID, WebsiteManager.赶集网_ID) == null)
                {
                    execStr = string.Format(execStr, WebsiteManager.赶集网, city.城市名称, cityUrl.TrimEnd('/'), cityUrl.TrimEnd('/') + "/fang5/",
                        "", "4000", "2000");
        ...
        {
            List<城市表> list = CityManager.Get网站爬取配置城市_by网站Id(WebsiteManager.赶集网_ID);
        ...
        public void 导出任务计划配置文件(string[] 城市名称)
        {
            ...
            {
                sb.Append("赶集网.").Append(str).Append(" ");
                sb2.Append(str).Append(".");
            }
            string cityResult = sb.ToString().TrimEnd(' ');
            string filaName = "任务计划_赶集网(" + sb2.ToString().TrimEnd('.') + ").xml";
