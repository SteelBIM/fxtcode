...
namespace FxtCenterService.Actualize
{
    public partial class DataController
    {
        ...
        public static string GetSYSAreaList(JObject funinfo, UserCheck company)
        {
            ...
            string zipcodeStr = funinfo.Property("zipcode") == null ? string.Empty : funinfo.Property("zipcode").Value.ToString();

            List<int> areaIDs = new List<int>();
            string areaName = string.Empty;
            if (funinfo.Property("key") != null)
            {
                string key = funinfo.Property("key").Value.ToString();
                if (System.Text.RegularExpressions.Regex.IsMatch(key, "^[\u4e00-\u9fa5]+$"))
                {
                    areaName = key;
                }
                else
                {
                    areaIDs = key.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Select(o => StringHelper.TryGetInt(o)).Where(o => o != 0).ToList();
                    areaids = areaids == null? areaIDs.ToArray() : areaids.Concat(areaIDs).Distinct().ToArray();
                }
            }
            ...
            List<SYSArea> arealist = SYSAreaBL.GetSYSAreaList(search, areaids, zipcodeStr, areaName);
        ...
        public static string GetProvinceList(JObject funinfo, UserCheck company)
        {
            ...
            if (funinfo.Property("key") != null)
            {
                ...
                {
                    provinceIDs = key.Split(new char[]{','},StringSplitOptions.RemoveEmptyEntries).Select(o=>StringHelper.TryGetInt(o)).Where(o=>o!=0).ToList();
                }
            }
            List<SYSProvince> provincelist = SYSAreaBL.GetProvinceList(provinceName, provinceIDs);
        ...
        public static string GetSYSCityList(JObject funinfo, UserCheck company)
        {
            int provinceid = funinfo.ToString().Contains("provinceid")
                ...
                : 0;
            string zipcode = funinfo.ToString().Contains("zipcode") ? funinfo.Value<object>("zipcode").ToString(): string.Empty;
            ...
            if (funinfo.Property("key") != null)
            {
                ...
                {
                    cityIDs = key.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Select(o => StringHelper.TryGetInt(o)).Where(o => o != 0).ToList();
                }
            }
            ...
            List<SYSCity> citylist = SYSAreaBL.GetSYSCityList(provinceid, zipcode,cityName,cityIDs);
