...
{
    public class OfficeProject : IOfficeProject
    {
        #region access
        ...
        public IQueryable<DatProjectOffice> GetProjectOffices(DatProjectOffice projectOffice, int pageIndex, int pageSize, out int totalCount, bool self = true)
        {
            string ptable, ctable, btable, comId;
            Access(projectOffice.CityId, projectOffice.FxtCompanyId, out ptable, out ctable, out btable, out comId);
            if (string.IsNullOrEmpty(comId)) comId = projectOffice.FxtCompanyId.ToString();
            if (self) comId = projectOffice.FxtCompanyId.ToString();

            var where = string.Empty;
            if (!(new[] { 0, -1 }).Contains(projectOffice.AreaId)) where += " and p.AreaId = @AreaId";
            if (!(new[] { 0, -1 }).Contains(projectOffice.SubAreaId)) where += "  and p.SubAreaId = @SubAreaId";
            if (!string.IsNullOrEmpty(projectOffice.ProjectName)) where += " and p.ProjectName like @ProjectName";
            if (!string.IsNullOrEmpty(projectOffice.OtherName)) where += "  and p.OtherName like @OtherName";
            if (!(new[] { 0, -1 }).Contains(projectOffice.OfficeType ?? -1)) where += "  and p.OfficeType = @OfficeType";
            if (!(new[] { 0, -1 }).Contains(projectOffice.RentSaleType ?? -1)) where += "  and p.RentSaleType = @RentSaleType";
...
LEFT JOIN FxtDataCenter.dbo.SYS_Code c3 WITH (NOLOCK) ON p.OfficeType = c3.Code
...
WHERE p.valid = 1
	...
	AND p.FxtCompanyId = @fxtCompanyId" + where;
            ...
            {
                totalCount = conn.Query<int>(totalCountSql, new { projectOffice.FxtCompanyId, projectOffice.SubAreaId, projectOffice.CityId, projectOffice.AreaId, ProjectName = "%" + projectOffice.ProjectName + "%", OtherName = "%" + projectOffice.OtherName + "%", projectOffice.OfficeType, projectOffice.RentSaleType }).FirstOrDefault();
                return conn.Query<DatProjectOffice>(pagenatedSql, new { projectOffice.FxtCompanyId, projectOffice.SubAreaId, projectOffice.CityId, projectOffice.AreaId, ProjectName = "%" + projectOffice.ProjectName + "%", OtherName = "%" + projectOffice.OtherName + "%", projectOffice.OfficeType, projectOffice.RentSaleType }).AsQueryable();
            }
        }

        public IQueryable<DatProjectOffice> GetOfficeProjects(int cityId, int fxtCompanyId, int areaId = -1, int subAreaId = -1, int projectId = -1, bool self = true)
        {
            string ptable, ctable, btable, comId;
...
WHERE p.valid = 1
    ...
    and p.fxtcompanyId = @fxtCompanyId" + where;
            ...
            using (IDbConnection conn = DapperAdapter.OpenConnection(ConfigurationHelper.FxtDataOffice))
            {
                return conn.Query<DatProjectOffice>(strSql, new { cityId = cityId, AreaId = areaId, SubAreaId = subAreaId, ProjectId = projectId, fxtCompanyId = fxtCompanyId }).AsQueryable();
            }
        }

        public DatProjectOffice GetProjectNameById(long id, int fxtCompanyId)
        {
            var strSql = @"
SELECT p.*
	,sa.SubAreaName
...
WHERE p.valid = 1
	...
	AND p.FxtCompanyId = @fxtCompanyId";
            ...
            using (IDbConnection conn = DapperAdapter.OpenConnection(ConfigurationHelper.FxtDataOffice))
            {
                return conn.Query<DatProjectOffice>(strSql, new { ProjectId = id, FxtCompanyId = fxtCompanyId }).FirstOrDefault();
            }
        }

        public bool IsExistProjectOffice(int areaId, long projectId, string projectName, int cityId, int fxtCompanyId)
        {
            string ptable, ctable, btable, comId;
            Access(cityId, fxtCompanyId, out ptable, out ctable, out btable, out comId);
            if (string.IsNullOrEmpty(comId)) comId = fxtCompanyId.ToString();
            ...
            var strSql = @"
SELECT projectId FROM FxtData_Office.dbo.Dat_Project_Office p WITH (NOLOCK)
WHERE NOT EXISTS (
		SELECT ProjectId
		FROM FxtData_Office.dbo.Dat_Project_Office_sub p1 WITH (NOLOCK)
		WHERE p1.areaId = p.areaId
...
where p.valid = 1
	...
	and p.FxtCompanyId = @fxtCompanyId" + strWhere;
        ...
        public int UpdateProjectOffice(DatProjectOffice projectOffice, int currentCompanyId)
        {
            var strSqlMainUpdate = @"UPDATE FxtData_Office.dbo.Dat_Project_Office WITH (ROWLOCK)
                                    SET AreaId = @AreaId,SubAreaId = @SubAreaId,CorrelationType = @CorrelationType,PurposeCode = @PurposeCode,ProjectName = @ProjectName,OtherName = @OtherName,Address = @Address,FieldNo = @FieldNo,LandArea = @LandArea,StartDate = @StartDate,StartEndDate=@StartEndDate,UsableYear = @UsableYear,BuildingArea = @BuildingArea,CubageRate = @CubageRate,GreenRate = @GreenRate,BuildingNum = @BuildingNum,BuildingType = @BuildingType,OfficeType = @OfficeType,EndDate = @EndDate,SaleDate = @SaleDate,OfficeArea = @OfficeArea,BizArea = @BizArea,IndustryArea = @IndustryArea,ManagerPrice = @ManagerPrice,ManagerTel = @ManagerTel,TrafficType = @TrafficType,TrafficDetails = @TrafficDetails,ParkingLevel = @ParkingLevel,ParkingType = @ParkingType,ParkingPrice = @ParkingPrice,RentSaleType = @RentSaleType,AirConditionType = @AirConditionType,Details = @Details,ZZProjectId = @ZZProjectId,East = @East,south = @south,west = @west,north = @north,SaveDateTime = @SaveDateTime,SaveUser = @SaveUser,PinYin = @PinYin,PinYinAll = @PinYinAll,X = @X,Y = @Y,Remarks = @Remarks,AveragePrice = @AveragePrice,Weight = @Weight
                                    WHERE ProjectId = @ProjectId and (FxtCompanyId = @FxtCompanyId or @fxtcompanyid = " + ConfigurationHelper.FxtCompanyId + ")";

            var strSqlSubAdd = @"INSERT INTO FxtData_Office.dbo.Dat_Project_Office_Sub (ProjectId,CityId,AreaId,SubAreaId,CorrelationType,PurposeCode,ProjectName,OtherName,Address,FieldNo,LandArea,StartDate,StartEndDate,UsableYear,BuildingArea,CubageRate,GreenRate,BuildingNum,BuildingType,OfficeType,EndDate,SaleDate,OfficeArea,BizArea,IndustryArea,ManagerPrice,ManagerTel,TrafficType,TrafficDetails,ParkingLevel,ParkingType,ParkingPrice,RentSaleType,AirConditionType,Details,ZZProjectId,East,south,west,north,Creator,CreateTime,SaveDateTime,SaveUser,PinYin,PinYinAll,FxtCompanyId,X,Y,Remarks,AveragePrice,Weight)
                                VALUES (@ProjectId,@CityId,@AreaId,@SubAreaId,@CorrelationType,@PurposeCode,@ProjectName,@OtherName,@Address,@FieldNo,@LandArea,@StartDate,StartEndDate,@UsableYear,@BuildingArea,@CubageRate,@GreenRate,@BuildingNum,@BuildingType,@OfficeType,@EndDate,@SaleDate,@OfficeArea,@BizArea,@IndustryArea,@ManagerPrice,@ManagerTel,@TrafficType,@TrafficDetails,@ParkingLevel,@ParkingType,@ParkingPrice,@RentSaleType,@AirConditionType,@Details,@ZZProjectId,@East,@south,@west,@north,@Creator,@CreateTime,@SaveDateTime,@SaveUser,@PinYin,@PinYinAll,@FxtCompanyId,@X,@Y,@Remarks,@AveragePrice,@Weight)";

            var strSqlSubUpdate = @"UPDATE FxtData_Office.dbo.Dat_Project_Office_Sub WITH (ROWLOCK)
                                    SET AreaId = @AreaId,SubAreaId = @SubAreaId,CorrelationType = @CorrelationType,PurposeCode = @PurposeCode,ProjectName = @ProjectName,OtherName = @OtherName,Address = @Address,FieldNo = @FieldNo,LandArea = @LandArea,StartDate = @StartDate,StartEndDate=@StartEndDate,UsableYear = @UsableYear,BuildingArea = @BuildingArea,CubageRate = @CubageRate,GreenRate = @GreenRate,BuildingNum = @BuildingNum,BuildingType = @BuildingType,OfficeType = @OfficeType,EndDate = @EndDate,SaleDate = @SaleDate,OfficeArea = @OfficeArea,BizArea = @BizArea,IndustryArea = @IndustryArea,ManagerPrice = @ManagerPrice,ManagerTel = @ManagerTel,TrafficType = @TrafficType,TrafficDetails = @TrafficDetails,ParkingLevel = @ParkingLevel,ParkingType = @ParkingType,ParkingPrice = @ParkingPrice,RentSaleType = @RentSaleType,AirConditionType = @AirConditionType,Details = @Details,ZZProjectId = @ZZProjectId,East = @East,south = @south,west = @west,north = @north,SaveDateTime = @SaveDateTime,SaveUser = @SaveUser,PinYin = @PinYin,PinYinAll = @PinYinAll,X = @X,Y = @Y,Remarks = @Remarks,AveragePrice = @AveragePrice,Weight = @Weight
                                    WHERE projectid = @projectid and fxtCompanyId = @fxtCompanyId";

            using (IDbConnection conn = DapperAdapter.OpenConnection(ConfigurationHelper.FxtDataOffice))
            {
                projectOffice.FxtCompanyId = currentCompanyId;
                var mainreturn = conn.Execute(strSqlMainUpdate, projectOffice);

                if (mainreturn == 0)
                {
                    if (conn.Execute(strSqlSubUpdate, projectOffice) == 0)
                    {
                        return conn.Execute(strSqlSubAdd, projectOffice);
                    }
                    return 1;
                }
                return 1;
        ...
        public int AddProjectOffice(DatProjectOffice projectOffice)
        {
            var strSql = @"INSERT INTO FxtData_Office.dbo.Dat_Project_Office (CityId,AreaId,SubAreaId,CorrelationType,PurposeCode,ProjectName,OtherName,Address,FieldNo,LandArea,StartDate,StartEndDate,UsableYear,BuildingArea,CubageRate,GreenRate,BuildingNum,BuildingType,OfficeType,EndDate,SaleDate,OfficeArea,BizArea,IndustryArea,ManagerPrice,ManagerTel,TrafficType,TrafficDetails,ParkingLevel,ParkingType,ParkingPrice,RentSaleType,AirConditionType,Details,ZZProjectId,East,south,west,north,Creator,CreateTime,SaveDateTime,SaveUser,PinYin,PinYinAll,FxtCompanyId,X,Y,Remarks,AveragePrice,Weight)
                            VALUES (@CityId,@AreaId,@SubAreaId,@CorrelationType,@PurposeCode,@ProjectName,@OtherName,@Address,@FieldNo,@LandArea,@StartDate,@StartEndDate,@UsableYear,@BuildingArea,@CubageRate,@GreenRate,@BuildingNum,@BuildingType,@OfficeType,@EndDate,@SaleDate,@OfficeArea,@BizArea,@IndustryArea,@ManagerPrice,@ManagerTel,@TrafficType,@TrafficDetails,@ParkingLevel,@ParkingType,@ParkingPrice,@RentSaleType,@AirConditionType,@Details,@ZZProjectId,@East,@south,@west,@north,@Creator,@CreateTime,@SaveDateTime,@SaveUser,@PinYin,@PinYinAll,@FxtCompanyId,@X,@Y,@Remarks,@AveragePrice,@Weight)";
            using (IDbConnection conn = DapperAdapter.OpenConnection(ConfigurationHelper.FxtDataOffice))
            {
                return conn.Execute(strSql, projectOffice);
            }
        }

        public int DeleteProjectOffice(DatProjectOffice projectOffice, int currentCompanyId)
        {
            var strSqlMainDelete = @"UPDATE FxtData_Office.dbo.Dat_Project_Office WITH (ROWLOCK) SET valid = 0,SaveDateTime = @SaveDateTime,SaveUser = @SaveUser
                                    WHERE projectid = @projectid and (FxtCompanyId = @fxtcompanyid or @fxtcompanyid = @MainCompanyId)";
            var strSqlSubDelete = @"UPDATE FxtData_Office.dbo.Dat_Project_Office_Sub WITH (ROWLOCK) SET valid = 0,SaveDateTime = @SaveDateTime,SaveUser = @SaveUser
                                    WHERE projectid = @projectid and fxtCompanyId = @fxtCompanyId";
            var strSqlSubAdd = @"INSERT INTO FxtData_Office.dbo.Dat_Project_Office_Sub (ProjectId,CityId,AreaId,SubAreaId,CorrelationType,PurposeCode,ProjectName,OtherName,Address,FieldNo,LandArea,StartDate,StartEndDate,UsableYear,BuildingArea,CubageRate,GreenRate,BuildingNum,BuildingType,OfficeType,EndDate,SaleDate,OfficeArea,BizArea,IndustryArea,ManagerPrice,ManagerTel,TrafficType,TrafficDetails,ParkingLevel,ParkingType,ParkingPrice,RentSaleType,AirConditionType,Details,ZZProjectId,East,south,west,north,Creator,CreateTime,SaveDateTime,SaveUser,PinYin,PinYinAll,FxtCompanyId,X,Y,Remarks,AveragePrice,Valid,Weight)
                                 VALUES (@ProjectId,@CityId,@AreaId,@SubAreaId,@CorrelationType,@PurposeCode,@ProjectName,@OtherName,@Address,@FieldNo,@LandArea,@StartDate,@StartEndDate,@UsableYear,@BuildingArea,@CubageRate,@GreenRate,@BuildingNum,@BuildingType,@OfficeType,@EndDate,@SaleDate,@OfficeArea,@BizArea,@IndustryArea,@ManagerPrice,@ManagerTel,@TrafficType,@TrafficDetails,@ParkingLevel,@ParkingType,@ParkingPrice,@RentSaleType,@AirConditionType,@Details,@ZZProjectId,@East,@south,@west,@north,@Creator,@CreateTime,@SaveDateTime,@SaveUser,@PinYin,@PinYinAll,@FxtCompanyId,@X,@Y,@Remarks,@AveragePrice,@Valid,@Weight)";

            using (IDbConnection conn = DapperAdapter.OpenConnection(ConfigurationHelper.FxtDataOffice))
            {
                projectOffice.FxtCompanyId = currentCompanyId;
                var mainreturn = conn.Execute(strSqlMainDelete, new { projectid = projectOffice.ProjectId, fxtcompanyid = projectOffice.FxtCompanyId, SaveDateTime = projectOffice.SaveDateTime, SaveUser = projectOffice.SaveUser, MainCompanyId = ConfigurationHelper.FxtCompanyId });

                if (mainreturn == 0)
                {
                    if (conn.Execute(strSqlSubDelete, projectOffice) == 0)
                    {
                        projectOffice.Valid = 0;
                        return conn.Execute(strSqlSubAdd, projectOffice);
                    }
                    return 1;
                }
        ...
        public IQueryable<LnkPPhoto> GetOfficeProjectPhotoes(LnkPPhoto lnkPPhoto, bool self = true)
        {
            string ptable, ctable, btable, comId;
...
FROM FxtData_Office.dbo.LNK_P_Photo p WITH (NOLOCK)
LEFT JOIN FxtDataCenter.dbo.SYS_Code c WITH (NOLOCK) ON p.PhotoTypeCode = c.Code
WHERE not exists (
		SELECT id
		FROM FxtData_Office.dbo.LNK_P_Photo_Sub p1 WITH (NOLOCK)
...
UNION
SELECT p.*,c.CodeName AS PhotoTypeName
FROM FxtData_Office.dbo.LNK_P_Photo_Sub p WITH (NOLOCK)
LEFT JOIN FxtDataCenter.dbo.SYS_Code c WITH (NOLOCK) ON p.PhotoTypeCode = c.Code
WHERE p.valid = 1
	AND p.CityId = @CityId
	AND p.FxtCompanyId = @fxtCompanyId
...
ORDER BY 1 DESC";
        ...
        public LnkPPhoto GetOfficeProjectPhoto(int id, int fxtCompanyId)
        {
            var strSql = @"
SELECT p.*
...
ORDER BY 1 DESC";
        ...
        public int AddOfficeProjectPhoto(LnkPPhoto lnkPPhoto)
        {
            var strSql = @"INSERT INTO FxtData_Office.dbo.LNK_P_Photo (projectid,phototypecode,path,photodate,photoname,cityid,fxtcompanyid,saveuser,savedate)
        ...
        public int UpdateOfficeProjectPhoto(LnkPPhoto lnkPPhoto, int currentCompanyId)
        {
            var strSqlMainUpdate = @"UPDATE FxtData_Office.dbo.LNK_P_Photo
            ...
            var strSqlSubUpdate = @"UPDATE FxtData_Office.dbo.LNK_P_Photo_Sub
                                    SET phototypecode = @phototypecode,path = @path,photoname = @photoname,saveuser = @saveuser,savedate = @savedate
                                    WHERE id = @id and fxtCompanyId = @fxtCompanyId";

            var strSqlSubAdd = @"INSERT INTO FxtData_Office.dbo.LNK_P_Photo_Sub (id,projectid,phototypecode,path,photodate,photoname,cityid,fxtcompanyid,saveuser,savedate)
        ...
        public int DeleteOfficeProjectPhoto(LnkPPhoto lnkPPhoto, int currentCompanyId)
        {
            var strSqlMainDelete = @"UPDATE FxtData_Office.dbo.LNK_P_Photo WITH (ROWLOCK) SET valid = 0,SaveUser = @SaveUser,SaveDate = @SaveDate
                                    WHERE id = @id and (FxtCompanyId = @fxtcompanyid or @fxtcompanyid = @MainCompanyId)";

            var strSqlSubDelete = @"UPDATE FxtData_Office.dbo.LNK_P_Photo_Sub WITH (ROWLOCK) SET valid = 0,SaveUser = @SaveUser,SaveDate = @SaveDate
                                    WHERE id = @id and fxtCompanyId = @fxtCompanyId";

            var strSqlSubAdd = @"INSERT INTO FxtData_Office.dbo.LNK_P_Photo_Sub (projectid,phototypecode,path,photodate,photoname,cityid,fxtcompanyid,saveuser,savedate,Valid)
...
//        public string GetProjectNameByProjectId(long projectId, int cityId, int fxtCompanyId)
//        {
//            string ptable, ctable, btable, comId;
//            Access(cityId, fxtCompanyId, out ptable, out ctable, out btable, out comId);
//            if (string.IsNullOrEmpty(comId)) comId = fxtCompanyId.ToString();
...
//            var strSql = @"
//SELECT ProjectName FROM FxtData_Office.dbo.Dat_Project_Office p WITH (NOLOCK)
//WHERE NOT EXISTS (
//		SELECT ProjectId
//		FROM FxtData_Office.dbo.Dat_Project_Office_sub p1 WITH (NOLOCK)
//		WHERE p1.areaId = p.areaId
...
//        public int GetProjectFxtCompanyIdByProjectId(long projectId, int cityId, int fxtCompanyId)
//        {
//            string ptable, ctable, btable, comId;
//            Access(cityId, fxtCompanyId, out ptable, out ctable, out btable, out comId);
//            if (string.IsNullOrEmpty(comId)) comId = fxtCompanyId.ToString();
...
//            var strSql = @"
//SELECT FxtCompanyId FROM FxtData_Office.dbo.Dat_Project_Office p WITH (NOLOCK)
//WHERE NOT EXISTS (
//		SELECT ProjectId
//		FROM FxtData_Office.dbo.Dat_Project_Office_sub p1 WITH (NOLOCK)
//		WHERE p1.areaId = p.areaId
...
//        }
        ...
        public DataTable ProjectSelfDefineExport(DatProjectOffice projectOffice, List<string> projectAttr, int CityId, int FxtCompanyId, bool self = true)
        {
            try
            {
                ...
                if (!(new[] { 0, -1 }).Contains(projectOffice.AreaId))
                {
                    where += " and p.AreaId = @AreaId";
                    paramet.Add(new SqlParameter("@AreaId", projectOffice.AreaId));
                }
                if (!(new[] { 0, -1 }).Contains(projectOffice.SubAreaId))
                {
                    where += "  and p.SubAreaId = @SubAreaId";
                    paramet.Add(new SqlParameter("@SubAreaId", projectOffice.SubAreaId));
                }
                if (!string.IsNullOrEmpty(projectOffice.ProjectName))
                {
                    where += " and p.ProjectName like @ProjectName";
                    paramet.Add(new SqlParameter("@ProjectName", projectOffice.ProjectName));
                }
                if (!string.IsNullOrEmpty(projectOffice.OtherName))
                {
                    where += "  and p.OtherName like @OtherName";
                    paramet.Add(new SqlParameter("@OtherName", projectOffice.OtherName));
                }
                if (!(new[] { 0, -1 }).Contains(projectOffice.OfficeType ?? -1))
                {
                    where += "  and p.OfficeType = @OfficeType";
                    paramet.Add(new SqlParameter("@OfficeType", projectOffice.OfficeType));
                }
                if (!(new[] { 0, -1 }).Contains(projectOffice.RentSaleType ?? -1))
                {
                    where += "  and p.RentSaleType = @RentSaleType";
                    paramet.Add(new SqlParameter("@RentSaleType", projectOffice.RentSaleType));
                }
...
LEFT JOIN FxtDataCenter.dbo.SYS_Code c WITH (NOLOCK) ON p.CorrelationType = c.Code
LEFT JOIN FxtDataCenter.dbo.SYS_Code c1 WITH (NOLOCK) ON p.TrafficType = c1.Code
LEFT JOIN FxtDataCenter.dbo.SYS_Code c2 WITH (NOLOCK) ON p.OfficeType = c2.Code
