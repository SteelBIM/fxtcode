...
{
    [Authorize]
    public class BusinessAnalysisController : BaseController
    {
        private readonly IDAT_Analysis_City _analy;
        private readonly ILog _log;
        private readonly IDropDownList _dropDownList;

        public BusinessAnalysisController(IDAT_Analysis_City analy, ILog log, IDropDownList dropDownList)
        {
            this._analy = analy;
            this._log = log;
        ...
        public ActionResult Index(DAT_Analysis_City model)
        {
            ...
            var self = true;
            int operate;
            Permission.Check(SYS_Code_Dict.商业数据分类.商业区域分析, SYS_Code_Dict.页面权限.查看自己, SYS_Code_Dict.页面权限.查看全部, out operate);
        ...
        //新建
        [HttpGet]
        [DataCenterActionFilter(SYS_Code_Dict.商业数据分类.商业区域分析, SYS_Code_Dict.页面权限.新增)]
        ...
        public ActionResult Create(DAT_Analysis_City model)
        {
            try
            {
                ...
                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.商业数据分类.商业区域分析, SYS_Code_Dict.操作.新增, "", "", "商业区域分析", RequestHelper.GetIP());
                return this.CloseThickboxToLoad();
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog("Business/BusinessAnalysis/Create", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
        ...
        public ActionResult EdityAnaly(string id)
        {
            ...
            var self = true;
            int operate;
            Permission.Check(SYS_Code_Dict.商业数据分类.商业区域分析, SYS_Code_Dict.页面权限.修改自己, SYS_Code_Dict.页面权限.修改全部, out operate);
            ...
            #endregion
            var result = _analy.GetAnalysisById(Convert.ToInt32(splitArray[1]), SYS_Code_Dict.数据大类.商业, Passport.Current.CityId, Passport.Current.FxtCompanyId);
            BindViewData();
            return View(result);
        }
        ...
        public ActionResult EdityAnaly(DAT_Analysis_City model)
        {
            try
            {
                ...
                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.商业数据分类.商业区域分析, SYS_Code_Dict.操作.修改, "", "", "修改区域分析", RequestHelper.GetIP());
                return this.RefreshParent();
            }
            catch (Exception ex)
            {

                LogHelper.WriteLog("Business/BusinessAnalysis/EdityAnaly", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
