...
{
    [Authorize]
    public class IndustryAnalysisController : BaseController
    {
        private readonly IDAT_Analysis_City _analy;
        private readonly ILog _log;
        private readonly IDropDownList _dropDownList;
        private readonly IIndustrySubArea _industrySubArea;
        public IndustryAnalysisController(IDAT_Analysis_City analy, ILog log, IDropDownList dropDownList, IIndustrySubArea industrySubArea)
        {
            this._analy = analy;
            this._log = log;
        ...
        public ActionResult Index(DAT_Analysis_City analysisCity)
        {
            ...
            var self = true;
            int operate;
            Permission.Check(SYS_Code_Dict.工业数据分类.区域分析, SYS_Code_Dict.页面权限.查看自己, SYS_Code_Dict.页面权限.查看全部, out operate);
            ...
            analysisCity.DataTypeCode = SYS_Code_Dict.数据大类.工业;
        ...
        public ActionResult Edit(string id)
        {
            ...
            var self = true;
            int operate;
            Permission.Check(SYS_Code_Dict.工业数据分类.区域分析, SYS_Code_Dict.页面权限.修改自己, SYS_Code_Dict.页面权限.修改全部, out operate);
            if (operate == (int)PermissionLevel.None)
            {
            ...
            var result = _analy.GetAnalysisById(Convert.ToInt32(splitArray[1]), SYS_Code_Dict.数据大类.工业, Passport.Current.CityId, Passport.Current.FxtCompanyId);
        ...
        public ActionResult Edit(DAT_Analysis_City analysisCity)
        {
            try
            {
                ...
                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.工业数据分类.区域分析, SYS_Code_Dict.操作.修改, "", "", "修改工业区域分析", RequestHelper.GetIP());
                return this.RefreshParent();
            }
            catch (Exception ex)
            {

                LogHelper.WriteLog("Industry/IndustryAnalysis/Edit", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
        ...
        #region 新增
        [HttpGet]
        [DataCenterActionFilter(SYS_Code_Dict.工业数据分类.区域分析, SYS_Code_Dict.页面权限.新增)]
        ...
        public ActionResult Create(DAT_Analysis_City analysisCity)
        {
            try
            {
                ...
                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.工业数据分类.区域分析, SYS_Code_Dict.操作.新增, "", "", "新增工业区域分析", RequestHelper.GetIP());
                return this.CloseThickboxToLoad();
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog("Industry/IndustryAnalysis/Create", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
