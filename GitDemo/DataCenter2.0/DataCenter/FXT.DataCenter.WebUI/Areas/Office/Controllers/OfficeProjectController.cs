...
{
    [Authorize]
    public class OfficeProjectController : BaseController
    {
        ...
        public OfficeProjectController(ILog log, IDropDownList dropDownList, IOfficeProject officeProject, IOfficeSubArea officeSubArea, IImportTask importTask)
        {
            this._log = log;
        ...
        //
        // GET: /Office/OfficeProject/

        public ActionResult Index(DatProjectOffice projectOffice, int? pageIndex, bool? isExternalRequest)
        {
            BindDropDownList(projectOffice.AreaId == 0 ? -1 : projectOffice.AreaId, Passport.Current.FxtCompanyId);
            ...
            {
                return View(new List<DatProjectOffice>().ToPagedList(1, 30));
            }
            ...
            projectOffice.FxtCompanyId = Passport.Current.FxtCompanyId;
            projectOffice.CityId = Passport.Current.CityId;

            var pageSize = 30;
            int totalCount;
            var result = _officeProject.GetProjectOffices(projectOffice, pageIndex ?? 1, pageSize, out totalCount, self).AsQueryable();

            var projectOfficeResult = new PagedList<DatProjectOffice>(result, pageIndex ?? 1, pageSize, totalCount);
            return View("Index", projectOfficeResult);
        }

        #region 编辑
        [HttpPost]
        public JsonResult IsExistProjectOffice(int areaId, long projectId, string projectName)
        {
            return Json(_officeProject.IsExistProjectOffice(areaId, projectId, projectName, Passport.Current.CityId, Passport.Current.FxtCompanyId));
        }
        ...
        public ActionResult Edit(string id)
        {
            ...
            var result = _officeProject.GetProjectNameById(int.Parse(splitArray[1]), int.Parse(splitArray[0]));
        ...
        [HttpPost]
        public ActionResult Edit(DatProjectOffice projectOffice)
        {
            try
            {
                projectOffice.CreateTime = DateTime.Now;
                projectOffice.Creator = Passport.Current.ID;
                projectOffice.SaveDateTime = DateTime.Now;
                projectOffice.SaveUser = Passport.Current.ID;
                projectOffice.Weight = projectOffice.Weight.HasValue ? projectOffice.Weight : 1;
                _officeProject.UpdateProjectOffice(projectOffice, Passport.Current.FxtCompanyId);

                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.功能模块.办公楼盘, SYS_Code_Dict.操作.修改, projectOffice.ProjectId.ToString(), projectOffice.ProjectName, "修改办公楼盘", RequestHelper.GetIP());

                return RedirectToAction("Index");
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog("Office/OfficeProject/Edit", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
        ...
        #region 图片
        //办公楼盘图片
        public ActionResult OfficeProjectPicture(int projectId, int? pageIndex)
        {
            ...
            var result = _officeProject.GetOfficeProjectPhotoes(lnkPhoto, self).AsQueryable();
        ...
        public ActionResult PictureCreate()
        {
            ...
            try
            {
                var virtualPath = "/ProjectPic/Office/OfficeProject" + Passport.Current.CityId + "/" + projectId + "/";
                ...
                if (IsOss)
                {
                    foreach (string f in Request.Files)
                    {
                        ...
                        if (file != null && file.ContentLength > 0)
                        {
                            ...
                            _officeProject.AddOfficeProjectPhoto(lnkPPhoto);
                            count++;
                        }
                    }
                }
                ...
                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.功能模块.办公楼盘, SYS_Code_Dict.操作.新增, "", "", "批量新增办公楼盘项目图片", RequestHelper.GetIP());
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog("Office/OfficeProject/PictureCreate", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
        ...
        public ActionResult PictureEdit(string chValue)
        {
            ...
            this.ViewBag.PhotoTypeName = GetDictById(SYS_Code_Dict._图片类型);
            var picture = _officeProject.GetOfficeProjectPhoto(int.Parse(splitArray[1]), int.Parse(splitArray[0]));
        ...
        public ActionResult PictureEdit(LnkPPhoto lnkPhoto)
        {
            try
            {
                ...
                _officeProject.UpdateOfficeProjectPhoto(lnkPhoto, Passport.Current.FxtCompanyId);

                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.功能模块.办公楼盘, SYS_Code_Dict.操作.修改, lnkPhoto.Id.ToString(), lnkPhoto.PhotoName, "修改办公楼盘图片", RequestHelper.GetIP());

                return this.RefreshParent();
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog("Office/OfficeProject/PictureEdit", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
        ...
        public ActionResult DeletePicture(List<string> ids)
        {
            ...
            try
            {
                ...
                foreach (var item in ids)
                {
                    ...
                    var result = _officeProject.GetOfficeProjectPhoto(int.Parse(array[1]), int.Parse(array[0]));
                    result.SaveDate = DateTime.Now;
                    result.SaveUser = Passport.Current.ID;
                    _officeProject.DeleteOfficeProjectPhoto(result, Passport.Current.FxtCompanyId);
                }

                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.功能模块.办公楼盘, SYS_Code_Dict.操作.删除, string.Join(",", ids), "", "删除办公楼盘图片", RequestHelper.GetIP());

                return Json(failList.Any() ? new { result = true, msg = "有" + failList.Count + "条数据您无权限删除！" } : new { result = true, msg = "" });
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog("Office/OfficeProject/DeletePicture", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
                return Json(new { result = false, msg = "删除失败！" });
            }
        }
        ...
        public ActionResult UploadPicture(int projectId)
        {
            ...
            try
            {
                var filePath = "/ProjectPic/Office/OfficeProject/" + Passport.Current.CityId + "/" + projectId + "/";
            ...
            {
                LogHelper.WriteLog("Office/OfficeProject/UploadPicture", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
        ...
        //新增保存
        [HttpPost]
        public ActionResult Create(DatProjectOffice projectOffice)
        {
            try
            {
                projectOffice.CityId = Passport.Current.CityId;
                ...
                projectOffice.Creator = Passport.Current.ID;
                projectOffice.Weight = projectOffice.Weight.HasValue ? projectOffice.Weight : 1;
                _officeProject.AddProjectOffice(projectOffice);

                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.功能模块.办公楼盘, SYS_Code_Dict.操作.新增, "", "", "新增办公楼盘", RequestHelper.GetIP());
            ...
            {
                LogHelper.WriteLog("Office/OfficeProject/Create", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
                return base.Back("操作失败！");
            }
        ...
        public ActionResult Delete(List<string> ids)
        {
            ...
            try
            {
                ...
                foreach (var item in ids)
                {
                    ...
                    var result = _officeProject.GetProjectNameById(int.Parse(array[1]), int.Parse(array[0]));
                    result.SaveDateTime = DateTime.Now;
                    result.SaveUser = Passport.Current.ID;
                    _officeProject.DeleteProjectOffice(result, Passport.Current.FxtCompanyId);
                }

                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.功能模块.办公楼盘, SYS_Code_Dict.操作.删除, string.Join(",", ids), "", "删除办公楼盘", RequestHelper.GetIP());

                return Json(failList.Any() ? new { result = true, msg = "有" + failList.Count + "条数据您无权限删除！" } : new { result = true, msg = "" });
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog("Office/OfficeProject/Delete", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
                return Json(new { result = false, msg = "删除失败！" });
            }
        }
        ...
        #region 楼盘导出
        [HttpGet]
        public ActionResult Export(int? areaid, int? subareaid, string projectName, string otherName, int? OfficeType, int? RentSaleType)
        {
            var self = true;
            int operate;
            ...
            var projectOffice = new DatProjectOffice
            {
                FxtCompanyId = Passport.Current.FxtCompanyId,
                CityId = Passport.Current.CityId,
                AreaId = areaid ?? 0,
                ...
                OtherName = otherName,
                OfficeType = OfficeType ?? 0,
                RentSaleType = RentSaleType,
            };

            int totalCount;
            var result = _officeProject.GetProjectOffices(projectOffice, 1, int.MaxValue, out totalCount, self);

            System.Web.HttpContext curContext = System.Web.HttpContext.Current;
            curContext.Response.AddHeader("content-disposition",
                                             "attachment;filename*=UTF-8''" +
                                             System.Web.HttpUtility.UrlEncode(Passport.Current.CityName + "_办公_楼盘数据_", System.Text.Encoding.GetEncoding("UTF-8")) + (DateTime.Now).ToString("yyyyMMdd") + ".xlsx");
            curContext.Response.ContentEncoding = System.Text.Encoding.GetEncoding("UTF-8");
            curContext.Response.Charset = "UTF-8";

            //插入日志
            _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.功能模块.办公楼盘, SYS_Code_Dict.操作.导出, "", "", "导出办公楼盘", RequestHelper.GetIP());
        ...
        [HttpPost]
        public ActionResult SelfDefineExport(DatProjectOffice projectOffice, List<string> project)
        {
            try
            {
                var self = true;
                ...
                projectOffice.FxtCompanyId = Passport.Current.FxtCompanyId;
                projectOffice.CityId = Passport.Current.CityId;
                var dt = _officeProject.ProjectSelfDefineExport(projectOffice, project, Passport.Current.CityId, Passport.Current.FxtCompanyId, self);
                if (dt != null && dt.Rows.Count > 0)
                {
                    System.Web.HttpContext curContext = System.Web.HttpContext.Current;
                    curContext.Response.AddHeader("content-disposition",
                                                     "attachment;filename*=UTF-8''" +
                                                     System.Web.HttpUtility.UrlEncode(Passport.Current.CityName + "_办公_自定义导出楼盘_", System.Text.Encoding.GetEncoding("UTF-8")) + (DateTime.Now).ToString("yyyyMMdd") + ".xlsx");
                    curContext.Response.ContentEncoding = System.Text.Encoding.GetEncoding("UTF-8");
                    curContext.Response.Charset = "UTF-8";

                    MemoryStream ms = ExcelHandle.RenderToExcel(dt);
                    _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.功能模块.办公楼盘, SYS_Code_Dict.操作.导出, "", "", "自定义导出办公楼盘", RequestHelper.GetIP());
            ...
            {
                LogHelper.WriteLog("Office/OfficeProject/SelfDefineExport", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
        ...
        {
            var result = _importTask.GetTask(SYS_Code_Dict.批量导入类型.办公楼盘信息, Passport.Current.CityId, Passport.Current.FxtCompanyId);
        ...
        public ActionResult UploadFile(HttpPostedFileBase file, string taskNameHiddenValue)
        {
            ...
            try
            {
                ...
                //操作日志
                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.功能模块.办公楼盘, SYS_Code_Dict.操作.导入, "", "", "导入办公楼盘", RequestHelper.GetIP());
                ...
                Task.Factory.StartNew(() =>
                {
                    ...
                    client.StartAsync(cityId, fxtCompanyId, filePath, userId, taskNameHiddenValue, "OfficeProject");
                    try { client.Close(); }
                    catch { client.Abort(); }
                });
            }
            ...
            {
                LogHelper.WriteLog("Office/OfficeProject/UploadFile", RequestHelper.GetIP(), Passport.Current.ID,
        ...
        //删除导入记录
        public ActionResult DeleteOfficeProjectImportRecord(List<string> ids)
        {
            try
            {
                if (ids == null || ids.Count == 0)
            ...
            {
                LogHelper.WriteLog("Office/OfficeProject/DeleteOfficeProjectImportRecord", RequestHelper.GetIP(), Passport.Current.ID,
                        Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
                return Json(new { result = false, msg = "删除失败！" });
            }
        }
        ...
        #region 统计
        [HttpGet]
        public JsonResult OfficeBuildingCount(int projectId)
        {
            var self = true;
            int operate;
            Permission.Check(SYS_Code_Dict.办公数据分类.办公基础数据, SYS_Code_Dict.页面权限.查看自己, SYS_Code_Dict.页面权限.查看全部, out operate);
            ...
            var result = _officeProject.GetBuildingCounts(projectId, Passport.Current.CityId, Passport.Current.FxtCompanyId, self);
        ...
        private IEnumerable<SelectListItem> GetSubAreaNamesByAreaId(int areaId, int fxtCompanyId, SubAreaOther subAreaOther = null)
        {
            ...
            if (subAreaOther != null && (subAreaOther.areaId == areaId || areaId == -1))
            {
                list.Add(
                    new SelectListItem
                    {
