...
namespace FXT.DataCenter.WebUI.Areas.Office.Controllers
{
    ...
    public class MarketAnalysisController : BaseController
    {
        ...
        public ActionResult Index(DatAnalysisMarket model)
        {
            ...
            var self = true;
            int operate;
            Permission.Check(SYS_Code_Dict.办公数据分类.市场背景分析, SYS_Code_Dict.页面权限.查看自己, SYS_Code_Dict.页面权限.查看全部, out operate);
            if (operate == (int)PermissionLevel.None) return View();
            if (operate == (int)PermissionLevel.All) self = false;
            #endregion

            model.DataTypeCode = SYS_Code_Dict.数据大类.办公;
        ...
        [HttpGet]
        [DataCenterActionFilter(SYS_Code_Dict.办公数据分类.市场背景分析, SYS_Code_Dict.页面权限.新增)]
        ...
        public ActionResult Create(DatAnalysisMarket model)
        {
            try
            {
                ...
                Services.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.办公数据分类.市场背景分析, SYS_Code_Dict.操作.新增, "", "", "办公区域分析", RequestHelper.GetIP());
                return this.CloseThickboxToLoad();
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog("Office/MarketAnalysis/Create", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
        ...
        public ActionResult Edit(string id)
        {
            ...
            var self = true;
            int operate;
            Permission.Check(SYS_Code_Dict.办公数据分类.市场背景分析, SYS_Code_Dict.页面权限.修改自己, SYS_Code_Dict.页面权限.修改全部, out operate);
            ...
            #endregion
            var result = Services.GetAnalysisById(Convert.ToInt32(splitArray[1]), SYS_Code_Dict.数据大类.办公, Passport.Current.CityId, Passport.Current.FxtCompanyId);
        ...
        public ActionResult Edit(DatAnalysisMarket model)
        {
            try
            {
                ...
                Services.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.办公数据分类.市场背景分析, SYS_Code_Dict.操作.修改, "", "", "修改市场背景分析", RequestHelper.GetIP());
                return this.RefreshParent();
            }
            catch (Exception ex)
            {

                LogHelper.WriteLog("Office/MarketAnalysis/Edit", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
        ...
        private List<SelectListItem> GetSubAreaName(int areaId, int fxtCompanyId)
        {
            ...
            var subAreaNames = Services.GetOfficeSubAreaNamesByAreaId(areaId, fxtCompanyId, Passport.Current.CityId);
