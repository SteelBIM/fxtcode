...
namespace FXT.DataCenter.WebUI.Areas.House.Controllers
{
    ...
    public class AnalysisController : BaseController
    {
        ...
        public ActionResult Index(DAT_Analysis_City model)
        {
            ...
            Permission.Check(SYS_Code_Dict.住宅数据分类.住宅区域分析, SYS_Code_Dict.页面权限.查看自己, SYS_Code_Dict.页面权限.查看全部, out operate);
        ...
        [HttpGet]
        [DataCenterActionFilter(SYS_Code_Dict.住宅数据分类.住宅区域分析, SYS_Code_Dict.页面权限.新增)]
        ...
        public ActionResult Create(DAT_Analysis_City model)
        {
            try
            {
                ...
                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.住宅数据分类.住宅区域分析, SYS_Code_Dict.操作.新增, "", "", "住宅区域分析", RequestHelper.GetIP());
                return this.CloseThickboxToLoad();
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog("House/Analysis/Create", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
        ...
        public ActionResult EdityAnaly(string id)
        {
            ...
            var self = true;
            int operate;
            Permission.Check(SYS_Code_Dict.住宅数据分类.住宅区域分析, SYS_Code_Dict.页面权限.修改自己, SYS_Code_Dict.页面权限.修改全部, out operate);
            ...
            #endregion
            var result = _analy.GetAnalysisById(Convert.ToInt32(splitArray[1]), SYS_Code_Dict.数据大类.住宅, Passport.Current.CityId, Passport.Current.FxtCompanyId);
            return View(result);
        }
        ...
        public ActionResult EdityAnaly(DAT_Analysis_City model)
        {
            try
            {
                ...
                _log.InsertLog(Passport.Current.CityId, Passport.Current.FxtCompanyId, Passport.Current.ID, Passport.Current.ID, SYS_Code_Dict.住宅数据分类.住宅区域分析, SYS_Code_Dict.操作.修改, "", "", "修改区域分析", RequestHelper.GetIP());
                return this.RefreshParent();
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog("House/Analysis/EdityAnaly", RequestHelper.GetIP(), Passport.Current.ID, Passport.Current.CityId, Passport.Current.FxtCompanyId, ex);
