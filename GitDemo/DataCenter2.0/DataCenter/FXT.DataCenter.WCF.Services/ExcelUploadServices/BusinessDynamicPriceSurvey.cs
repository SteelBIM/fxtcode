...
namespace FXT.DataCenter.WCF.Services
{
    ...
    {
        public void BusinessDynamicPriceSurveyExcelUpload(int cityid, int fxtcompanyid, string filePath, string userid,
            string taskName)
        {
            var taskId = 0;
            ...
            try
            {
                ...
                {
                    TaskName = taskName,
                    ImportType = SYS_Code_Dict.批量导入类型.商业动态价格调查,
                    CityID = cityid,
                ...
                List<Dat_P_B_Price_Biz> listTrue;//正确数据
                ...
                {
                    var fileName = DateTime.Now.ToString("yyyyMMddHHmmss") + "商业动态价格调查错误数据.xlsx";
                ...
                //正确数据添加到表中
                //listTrue.ForEach(m => _dynamicPriceSurvey.AddDynamicPriceSurvey(m));
                foreach (var item in listTrue)
                {
                    var id = _dynamicPriceSurvey.GetDynamicPriceSurveyId(item.ProjectId, item.BuildingId, item.CityId, item.FxtCompanyId);
                    if (id > 0)
                    {
                        item.Id = id;
                        var modifyResult = _dynamicPriceSurvey.UpdateDynamicPriceSurvey(item);
                        if (modifyResult <= 0) failureNum = failureNum + 1;
                    }
                    else
                    {
                        var insertResult = _dynamicPriceSurvey.AddDynamicPriceSurvey(item);
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.WriteLog("BusinessDynamicPriceSurveyExcelUploadTask", "", userid, cityid, fxtcompanyid, ex);
            }
        }

        private void DataFilter(string userId,int cityId, int fxtCompanyId, out List<Dat_P_B_Price_Biz> listTrue, out DataTable dtError, DataTable dt)
        {

            listTrue = new List<Dat_P_B_Price_Biz>();
            ...
            for (var i = 0; i < dt.Rows.Count; i++)
            {
                ...
                //{
                //    isError = true;
                //    dr["商业楼栋"] = projectName + "#error";
                //}


                var avgRentStr = dt.Rows[i]["*平均租金_元/平方米*日"].ToString().Trim();
                var avgRent = TryParseHelper.StrToDecimal(avgRentStr, -1);
                dr["*平均租金_元/平方米*日"] = avgRentStr;
                dynamicPriceSurvey.AvgRent = avgRent;
                if (string.IsNullOrEmpty(avgRentStr))
                {
                    isError = true;
                    dr["*平均租金_元/平方米*日"] = avgRentStr + "#error";
                }
                ...
                dynamicPriceSurvey.RentTypeCode = rentTypeCode;
                ...
                var vacantRate = dt.Rows[i]["空置率"].ToString().Trim();
                ...
                var managerPrice = dt.Rows[i]["管理费_元/平方米*月"].ToString().Trim();
                dynamicPriceSurvey.ManagerPrice = (decimal?)TryParseHelper.StrToDecimal(managerPrice);
                dr["管理费_元/平方米*月"] = managerPrice;
                if (!string.IsNullOrEmpty(managerPrice) && TryParseHelper.StrToDecimal(managerPrice) == null)
                {
                    isError = true;
                    dr["管理费_元/平方米*月"] = managerPrice + "#error";
                }


                var surveyDate = dt.Rows[i]["调查时间"].ToString().Trim();
                dynamicPriceSurvey.SurveyDate = TryParseHelper.StrToDateTime(surveyDate) == null ? (DateTime)SqlDateTime.MinValue : (DateTime)TryParseHelper.StrToDateTime(surveyDate);
                dr["调查时间"] = surveyDate;
                if (!string.IsNullOrEmpty(surveyDate) && TryParseHelper.StrToDateTime(surveyDate) == null)
                {
                    isError = true;
                ...
                dynamicPriceSurvey.CityId = cityId;
                dynamicPriceSurvey.FxtCompanyId = fxtCompanyId;
                dynamicPriceSurvey.Creator = userId;
                dynamicPriceSurvey.SaveUser = userId;
