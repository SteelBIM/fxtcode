...
namespace FXT.DataCenter.WCF.Services
{
    ...
    {
        public void OfficeDynamicPriceExcelUpload(int cityid, int fxtcompanyid, string filePath, string userid,
            string taskName)
        {
            var taskId = 0;
            ...
            try
            {
                ...
                {
                    TaskName = taskName,
                    ImportType = SYS_Code_Dict.批量导入类型.办公动态价格,
                    CityID = cityid,
                ...
                List<DatPbPriceOffice> listTrue;//正确数据
                ...
                {
                    var fileName = DateTime.Now.ToString("yyyyMMddHHmmss") + "办公动态价格调查错误数据.xlsx";
                ...
                foreach (var item in listTrue)
                {
                    ...
                    var insertResult = _officeDynamicPrice.AddDynamicPriceSurvey(item);
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.WriteLog("OfficeDynamicPriceExcelUploadTask", "", userid, cityid, fxtcompanyid, ex);
            }
        }

        private void DataFilter(int cityId, int fxtCompanyId, out List<DatPbPriceOffice> listTrue, out DataTable dtError, DataTable dt)
        {

            listTrue = new List<DatPbPriceOffice>();
