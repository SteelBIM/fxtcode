...
namespace FXT.DataCenter.WCF.Services
{
    ...
    {
        public void OfficeHouseExcelUpload(int cityid, int fxtcompanyid, string filePath, string userid,
            string taskName)
        {
            var taskId = 0;
            ...
            try
            {
                ...
                {
                    TaskName = taskName,
                    ImportType = SYS_Code_Dict.批量导入类型.办公房号信息,
                    CityID = cityid,
                ...
                List<DatHouseOffice> listTrue;//正确数据
                ...
                {
                    var fileName = DateTime.Now.ToString("yyyyMMddHHmmss") + "办公房号错误数据.xlsx";
                ...
                {
                    var houseId = _officeHouse.GetOfficeHouseId(item.BuildingId, item.HouseName, item.CityId, item.FxtCompanyId);
                    if (houseId <= 0)
                    {
                        item.Creator = userid;
                        _officeHouse.AddOfficeHouse(item);
                    }
                    else
                    {
                        item.SaveUser = userid;
                        item.HouseId = houseId;
                        _officeHouse.UpdateOfficeHouse(item, fxtcompanyid);
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.WriteLog("OfficeHouseExcelUploadTask", "", userid, cityid, fxtcompanyid, ex);
            }
        }

        private void DataFilter(int cityId, int fxtCompanyId, out List<DatHouseOffice> listTrue, out DataTable dtError, DataTable dt)
        {
            listTrue = new List<DatHouseOffice>();
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                datHouseOffice.CityId = cityId;
                datHouseOffice.FxtCompanyId = fxtCompanyId;
                ...
                datHouseOffice.ProjectId = projectId;
                dr["*办公楼盘"] = projectName;
                ...
                datHouseOffice.BuildingId = buildingId;
                ...
                datHouseOffice.FloorNo = TryParseHelper.StrToInt32(floorNo, -1);
                dr["*物理层"] = floorNo;
                ...
                datHouseOffice.FloorNum = floorNum;
                dr["*实际层"] = floorNum;
                ...
                datHouseOffice.HouseNo = houseNo;
                dr["*室号"] = houseNo;
                ...
                datHouseOffice.HouseName = houseName;
                dr["*房号名称"] = houseName;
                ...
                datHouseOffice.PurposeCode = purposeCode;
                dr["证载用途"] = purposeName;
                ...
                datHouseOffice.SJPurposeCode = sjPurposeCode;
                ...
                datHouseOffice.BuildingArea = (decimal?)TryParseHelper.StrToDecimal(buildingArea);
                ...
                datHouseOffice.InnerBuildingArea = (decimal?)TryParseHelper.StrToDecimal(innerBuildingArea);
                ...
                datHouseOffice.FrontCode = frontCode;
                dr["朝向"] = frontName;
                ...
                datHouseOffice.SightCode = sightCode;
                dr["景观"] = sightName;
                ...
                datHouseOffice.UnitPrice = (decimal?)TryParseHelper.StrToDecimal(unitPrice);
                ...
                datHouseOffice.Weight = (decimal?)TryParseHelper.StrToDecimal(weight);
                dr["价格系数"] = weight;
                ...
                datHouseOffice.IsEValue = isEvalue == "否" ? 0 : 1;
                dr["是否可估"] = isEvalue;
                ...
                datHouseOffice.Remarks = remarks;
                dr["备注"] = remarks;
