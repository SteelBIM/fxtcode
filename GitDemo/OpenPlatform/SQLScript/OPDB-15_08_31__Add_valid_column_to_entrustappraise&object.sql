...
-- 在entrustobject表添加valid字段
ALTER TABLE entrustobject ADD Valid BIT(1) DEFAULT 1;
...
CREATE DEFINER=`tyoung`@`%` PROCEDURE `process_entrust_data_prc`(
...
CREATE DEFINER=`tyoung`@`%` PROCEDURE `process_entrustobject_data_prc`(
