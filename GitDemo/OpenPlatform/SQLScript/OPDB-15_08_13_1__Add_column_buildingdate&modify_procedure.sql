/*
Date: 20150813
Description: 添加字段建筑年代
*/
USE openplatform;

ALTER TABLE EntrustObject ADD BuildingDate VARCHAR(50) COMMENT '建筑年代';
...
CREATE PROCEDURE `process_entrustobject_data_prc`(IN `v_gjbobjid` BIGINT(20)    -- 估价宝委估对象ID
    ...
    , IN `v_issurvey` BIT(1)    -- 是否现场查勘(0:否, IN `v_surveybegintime` DATETIME    -- 查勘开始时间
    , IN `v_surveyendtime` DATETIME    -- 查勘结束时间
    
    , IN `v_financingpurpose` VARCHAR(100)    -- 融资目的
    , IN `v_usage` VARCHAR(50)    -- 使用情况(1: 自住, IN `2:` 出租或空置)
    ...
    , IN BuildingDate VARCHAR(50)   -- 建筑年代
    )
	LANGUAGE SQL
	NOT DETERMINISTIC
	MODIFIES SQL DATA
    ...
    */
    ...
    IF v_count > 0
        THEN
            ...
            COMMIT;
        ELSE
            INSERT INTO propertycertificate(pcid
