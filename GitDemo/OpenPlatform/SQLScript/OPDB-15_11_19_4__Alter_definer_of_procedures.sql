...
CREATE DEFINER = 'root'@'localhost' PROCEDURE `process_buyer_data_prc`(
...
CREATE DEFINER = 'root'@'localhost' PROCEDURE `process_entrustobject_data_prc`(
...
CREATE DEFINER = 'root'@'localhost' PROCEDURE `process_files_data_prc`(
    IN `v_gjbobjid` BIGINT    -- 估价宝委估对象Id
...
CREATE DEFINER = 'root'@'localhost' PROCEDURE `process_person_data_prc`(
...
CREATE DEFINER = 'root'@'localhost' PROCEDURE `process_property_data_prc`(
     IN `v_gjbobjid` BIGINT   -- 估价宝委估对象Id
    ,IN `v_personname` VARCHAR(20)    -- 姓名
    ,IN `v_idnum` VARCHAR(20) -- 身份证
