...
TabPanel = function (config) {
    ...
    */
    ...
    this.onlyone = config.onlyone;
    this.contextMenu = true;
    if (config.menu == false) this.contextMenu = false;
    this.inited = false;
    this.init();
};
...
TabPanel.prototype = {
    ...
    init: function () {
        ...
        this.render.width(this.render.width() - hwFix);
        ...
        if (this.active >= 0) {
            ...
            var that = this;
            setTimeout(function () { that.resize(); }, 10);
        }
    },
    //scroll left
    moveLeft: function () {
    ...
    */
    addTab: function (tabitem) {
        ...
        if ($('#' + tabitem.id).length > 0) {
            ...
            title.appendTo(tab);
            if (tabitem.url)
                tabitem.html = " <iframe onresize='initLayout(this)' id='" + tab.attr("id") + "Frame' src='" + "" + "' style='width:100%;height:100%;border:0;' frameborder=0></iframe>";
            ...
            if (tabitem.closable) {
                ...
                //添加右键 kevin
                var menu = [
                  { text: '刷新', icon: "iconrefresh", callback: function () { tabEntity.refresh(tab.attr("id")); } },
                  { text: '关闭', icon: "iconclose", callback: function () { tabEntity.kill(tab.attr("id")); } },
                  { text: "-" },
                  { text: '关闭其他', icon: "iconcloseother", callback: function () { tabEntity.killother(tab.attr("id")); } },
                  { text: '关闭所有', icon: "iconcloseall", callback: function () { tabEntity.killall(); } }
                ];
    ...
    */
    ...
    show: function (position, notExecuteMoveSee) {
        ...
        if (this.scrollFinish) {
            ...
            //这里改为隐藏/显示来控制切换，原作者用的是ZINDEX，会出现滚动条BUG，所以修改 kevin
            //如果只有一个对象，不切换 kevin
            for (var i = 0; i < this.tabs.length; i++) {
                if (this.onlyone && i == 0) continue;
                this.tabs[i].content.hide();
            }
            if (!this.onlyone) {
                this.tabs[position].content.show(); //.css('z-index', ++this.maxzindex);
            }
            else {
                this.tabs[0].content.show(); //.css('z-index', ++this.maxzindex);
            }
            if (this.onShow) {
                this.onShow(this.tabs[position].extdata);
            }
            ...
            if (this.inited) {
                ...
                if (iframes.length > 0) {
                    ...
                    if (src == "")
                        $("#" + frameId).attr('src', this.tabs[position].url);
                }
            }
    ...
    */
    ...
    killall: function () {
        ...
        for (var i = 0; i < this.tabs.length; i++) {
            if (this.tabs[i].closable) {
                ...
                tmptabs.push(this.tabs[i]);
            }
        }
        this.tabs = tmptabs;
        this.reIndex(0);
    },
    killother: function (position) {
        var tmptabs = [];
        for (var i = 0; i < this.tabs.length; i++) {
            if (this.tabs[i].closable && i != this.getTabPosision(position)) {
                this.kill(i, true);
            }
            else {
                tmptabs.push(this.tabs[i]);
    ...
    kill: function (position, mul) {
        ...
        this.tabs[position].content.find("iframe").remove();
        this.tabs[position].content.remove();

        if (!mul) {
            //remove from tabs 
            this.tabs.splice(position, 1);
            var cur = this.getActiveTab();
            if (cur != null) preTabId = cur.id;
            this.reIndex(preTabId);
        }
    },
    reIndex: function (preTabId) {
        ...
        this.showScroll();
        //show last 
        if (!preTabId) preTabId = this.tabs[0].id;
    ...
    */
    ...
    resize: function () {
        ...
        if (this.widthResizable) {
            ...
            this.tabpanel.width(this.render.width() - hwFix);
            this.tabpanel_tab_content.width(this.render.width() - hwFix);
            this.tabpanel_content.width(this.render.width() - hwFix);
        }
        ...
        var iframe = $(".html_content").find('iframe');
        if (iframe.size() > 0) {
            $(iframe[0].contentWindow.document.body).caslayout();
        }
    },
