    ...
    public class ExcelHelpers
    {
        ...
        public static void MergeRow(GridView gvw, int sCol, int eCol)//gvw 需要合并的GridView，sCol要合并开始列（从0开始），eCol要合并的结束列 
        {
            for (int i = sCol; i <= eCol; i++)
            {
                ...
                for (int rowIndex = 0; rowIndex <= gvw.Rows.Count - 2; rowIndex++)
                {
                    if (!match)
                    {
                        currentMatchRow = gvw.Rows[rowIndex];
                    }
                    GridViewRow nextRow = gvw.Rows[rowIndex + 1];
                    ...
                    {
                        // 
                        if (currentMatchRow.Cells[i].Text == nextRow.Cells[i].Text && nextRow.Cells[0].Text != _separatorGuid)
                        {
                            rowSpan++;
                            nextRow.Cells[i].Visible = false;
                            match = true;
                            if (rowIndex == gvw.Rows.Count - 2)
                            {
                                //最后一行
