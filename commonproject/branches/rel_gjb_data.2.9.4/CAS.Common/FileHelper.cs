...
namespace CAS.Common
{
    public class FileHelper
    {
        ...
        /// <param name="file"></param>
        /// <param name="content"></param>
        public static void FileCreate(string file, string content)
        {
        ...
        /// <summary>
        /// 压缩一组文件(文件名为：文件名称 fileList,filenameList顺序要对应 )
        /// hell 2014-12-04
        /// </summary>
        /// <param name="fileList">文件路径数组</param>
        /// <param name="filenameList">文件路径对应名称数组</param>
        /// <param name="outputCompressedFilePath">压缩后输出的压缩文件路径(包含文件名)</param>
        public static void CompressFileContainFileName(string[] fileList, string[] filenameList, string outputCompressedFilePath)
        {
            using (ZipOutputStream s = new ZipOutputStream(File.Create(outputCompressedFilePath)))
            {
                s.UseZip64 = UseZip64.On;
                ...
                foreach (string file in fileList)
                {
                    ...
                    ZipEntry entry = new ZipEntry(filenameList[curindex++]);
    ...
    public class ImageUtil
    {
        ...
        public static int MakeThumbnail(string originalImagePath, string thumbnailPath, int width, int height, ThumbnailCompressType mode, string type)
        {
            ...
            else if (null != originalImage)
            {
                ...
                //动态处理压缩质量 kevin
                if (ow < 800 || oh < 600) flag = 70;
                else if (ow < 1400 || oh < 800) flag = 60;
                //新建一个bmp图片
                ...
                try
                {
                    ...
                    {
                        bitmap.Save(thumbnailPath, tFormat);
                    }
                }
                catch (Exception ex)
                {
        ...
        public static bool isImage(string filename)
        {
            ...
            if (ext == ".jpg" || ext == ".jpeg" || ext == ".png" || ext == ".bmp" || ext == ".tif") return true;
            return false;
        }
        /***cas项目引用***/
        /// <summary>
        ...
        /// <returns></returns>
        public static bool CompressImage(string sFile)
        {
            Image iSource = null;
            Graphics g = null;
            Bitmap ob = null;
            try
            {
                ...
                else
                {
                    ...
                    sH = tem_size.Height;
                    //动态处理压缩质量 kevin
                    if (sW < 800 || sH < 600) flag = 70;
                    else if (sW < 1400 || sH < 800) flag = 60;
                }
                ob = new Bitmap(sW, sH);
                g = Graphics.FromImage(ob);
                g.Clear(Color.WhiteSmoke);
                g.CompositingQuality = CompositingQuality.HighQuality;
                g.SmoothingMode = SmoothingMode.HighQuality;
                g.InterpolationMode = InterpolationMode.HighQualityBicubic;
            ...
            {
                if (iSource != null) iSource.Dispose();
                if (ob != null) ob.Dispose();
                if (g != null) g.Dispose();
            }
        }
