	    ...
	    appendNodes: function (setting, level, nodes, parentNode, initFlag, openFlag) {
	        ...
	        for (var i = 0, l = nodes.length; i < l; i++) {
	            ...
	            if (openFlag) {
	                ...
	                html.push("<li " + (node.locked ? "style='padding:0; background:none;'" : "") + " id='", node.tId, "' class='level", node.level, "' tabindex='0' hidefocus='true' treenode>",
						"<span " + (node.locked ? "style='visibility:hidden; width:1px;'" : "") + " id='", node.tId, consts.id.SWITCH,
						"' title='' class='", view.makeNodeLineClass(setting, node), "' treeNode", consts.id.SWITCH, "></span>");
	                data.getBeforeA(setting, node, html);
	                html.push("<a id='", node.tId, consts.id.A, "' class='" + (node.selected ? "curSelectedNode " : "") + "level", node.level, "' treeNode", consts.id.A, " onclick=\"", (node.click || ''),
	            ...
	            if (node.selected) {
	                view.selectNode(setting, node, 1);
	            }
	        }
	        return html;
	    },
