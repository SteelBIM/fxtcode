...
namespace CAS.Common.Aspose
{
    public class WordHelper
    {
        ...
        public void ConvertExcelToWordTable(List<string> openTargetExcelFile, List<string> insertImageExcelFile, string sheetName, string[] prefixRegionName, string imgRegionName)
        {
                ...
                foreach (string targetExcelFile in openTargetExcelFile)
                {
                    ...
                    if (workbook.Worksheets.Names.Count > 0)
                    {
                        ...
                        foreach (Bookmark bookmark in word.Range.Bookmarks)
                        {
                            try
                            {
                                string[] name = bookmark.Name.Split('_');
                                if (name.Length < 2) continue;
                                if (prefixRegionName.Contains(name[0]))
                                {
                                    AsposeExcel.Range r = workbook.Worksheets.GetRangeByName(name[0] + "_" + name[1]);
                                        ...
                                        for (int i = 0; i < r.RowCount; i++)
                                        {
                                            for (int j = 0; j < r.ColumnCount; j++)
                                            {
                                                ...
                                                curCell.CellFormat.VerticalAlignment =
                                                    ConvertExcelVerticalAlignmentToWord(style.VerticalAlignment);
                                                    //垂直居中对齐
            ...
            if (insertImageExcelFile != null)
                GetExcelImageData(insertImageExcelFile);
        ...
        /// <param name="insertImageExcelFile">图片集合</param>
        /// <param name="sheetName"></param>
        public void GetExcelImageData(List<string> insertImageExcelFile)
        {
            List<byte[]> imgList = new List<byte[]>();
            var bookMarks = word.Range.Bookmarks;
            for (int i = 0; i < bookMarks.Count; i++)
            {
                string bookname = bookMarks[i].Name;
                if (bookname.Contains("可比实例"))
                {
                    if (bookname == "可比实例照片")
                    {
                        if (builder.MoveToBookmark(bookname))
                        {
                            bookMarks[i].Text = "";
                            bookMarks[i].Remove();
                            builder.StartBookmark(bookname);
                            //特殊处理当为可比实例照片生成其他三种所有的照片
                            foreach (string imageExcelFile in insertImageExcelFile)
                            {
                                ExcelHelper eh = new ExcelHelper(imageExcelFile);
                                int kbcount = eh.workbook.Worksheets.Where(c => c.Name.Contains("可比实例") && c.Name.Contains("照片")).Count();
                                for (int j = 0; j < kbcount; j++)
                                {
                                    imgList.AddRange(eh.GetPictureData("可比实例" + (j + 1) + "照片"));
                                }
                                InsertImgToWord(imgList);
                                imgList.Clear();
                            }
                            builder.EndBookmark(bookname);
                        }
                    }
                    else if (bookname.Contains("可比实例") && bookname.Contains("照片"))
                    {
                        if (builder.MoveToBookmark(bookname))
                        {
                            bookMarks[i].Text = "";
                            ...
                            {
                                ExcelHelper eh = new ExcelHelper(imageExcelFile);
                                imgList.AddRange(eh.GetPictureData(bookname));
                                InsertImgToWord(imgList);
                                imgList.Clear();
                            }
                        }
            ...
            foreach (Match m in rg.Matches(text))
            {
                ...
                try
                {
                    ...
                    else if (changeBackground)
                    {
                        ...
                        compositenode.Range.Replace(new Regex(RegexReplace(m.Value)), new ReplaceAndInsertText(bookMakeName, "", isCreatBookMake, false), false);
                }
                catch (Exception ex)
                {
        ...
        public void DataInsertToBookmark(Dictionary<string, string> dicExcel,int index,string flat)
        {
            ...
            {
                if (strList.Contains("," + bookmark.Name + ",")) continue;
                strList.Add("," + bookmark.Name + ",");
                ...
                else if ("区位因素分析,估价方法描述,最高最佳使用方法描述,市场背景分析".Contains(name) && dicExcel.Keys.Contains(name))
                {
                    ...
                    try
                    {
                        if (builder.MoveToBookmark(bookmark.Name))
                        {
                            ...
                            builder.InsertHtml(dicExcel[name], true);
                            builder.EndBookmark(bookmarkname);
                        }
                    }
                    catch (Exception ex)
                    {
                       AsposeHelper.Info(ex + "字段名称：" + name + "******字段内容：" + dicExcel[name]);
                    }
                        ...
                        if (bookmark.Name.Contains("_")&&flat!="")
                        {
                            ...
                            if (!string.IsNullOrEmpty(key) && dicExcel.Keys.Contains(key))
                            {
                                if (arrStrings[2] == flat)
                                {
                                    if (builder.MoveToBookmark(bookmark.Name))
                                    {
                                        if (dicExcel[key].Contains("\r") || dicExcel[key].Contains("\a") ||
                                            dicExcel[key].Contains("\n") ||
                                            dicExcel[key].ToLower().Contains("^p"))
                                        {
                                            //因为要把数据插入到书签里面才能进行替换所以只能删除该书签了在重新插入书签
                                            string bookmarkname = bookmark.Name;
                                            bookmark.Text = "";
                                            ...
                                            if (dicExcel[key].ToLower().Contains("^p"))
                                            {
                                                string[] str = dicExcel[key].Split(new string[2] {"^p", "^P"},StringSplitOptions.None);
                        ...
                        //如果文本框中没有包括｛｝就继续查找
                    {
                        ...
                        if (dicExcel.Keys.Contains(textType) && File.Exists(dicExcel[textType]))
                        {
                            if (isScaling)
                            {
                                ...
                                Image image = AsposeHelper.ScalingImage(dicExcel[textType], shape.Width, shape.Height,
                                    true);
                                if (image != null)
                                {
                                    builder.MoveTo(run);
