...
{
    /// <summary>
    /// APNS通知通道 - 不是线程安全的 - 只用于专门的情景。
    /// 使用的NotificationService或NotificationConnection。
    /// </summary>
    /// <remarks>
    /// 的NotificationChannel的目的是提供直接访问APNS流
    /// 没有线程安全和没有排队在内存中的通知。
    /// 
    ...
    /// 
    /// 如果使用NotificationService或的NotificationConnection类，如果应用程序
    /// 失败通知时，排队和发送通知时之间，
    /// 该通知将丢失。
    /// 
    /// 在此消息的情况下，这些通知，而不是排队MSMQ或东西
    /// 类似的，该队列时，将需要被立即发送出去，以确保
    /// 迷失在一个内存中的队列没有通知。
    /// </remarks>
