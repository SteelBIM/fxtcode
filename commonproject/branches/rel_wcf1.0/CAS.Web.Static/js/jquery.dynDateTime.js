	...
	$.fn.dynDateTime = function(options) {
		...
		return this.each(function() {
			...
			triggerEl["on" + opts.eventName] = function() {
				...
				if (!(cal && opts.cache)) {
				    window.calendar = cal = new Calendar(triggerEl, opts.firstDay,
									     opts.date,
...
Calendar = function (obj, firstDayOfWeek, dateStr, onSelected, onClose) {
    // member variables
    this.obj = obj;
    this.activeDiv = null;
...
Calendar.dayMouseOver = function(ev) {
	...
	if (el.navtype != 300 && el.navtype!=400) {
...
Calendar.prototype.create = function (_par) {
    ...
    hh("CLS", 1, 400).ttip = Calendar._TT["INFO"];
    ...
    this._nav_py = hh("&#x00ab;年", 1, -2);
    this._nav_py.ttip = Calendar._TT["PREV_YEAR"];

    this._nav_pm = hh("&#x2039;月", 1, -1);
    this._nav_pm.ttip = Calendar._TT["PREV_MONTH"];

    this._nav_now = hh(Calendar._TT["TODAY"], this.weekNumbers ? 4 : 3, 0);
    this._nav_now.ttip = Calendar._TT["GO_TODAY"];

    this._nav_nm = hh("月&#x203a;", 1, 1);
    this._nav_nm.ttip = Calendar._TT["NEXT_MONTH"];

    this._nav_ny = hh("年&#x00bb;", 1, 2);
...
Calendar._TT["DAY_FIRST"] = "以 %s 为第一天";
