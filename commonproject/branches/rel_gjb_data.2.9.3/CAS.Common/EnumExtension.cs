    ...
    public static class EnumExtension
    {
        ...
        public static Dictionary<string,string> GetEnumDesToDictionaryJson<T>(string dataType = "int")
        {
            ...
            foreach (T item in Enum.GetValues(typeof(T)))
            {
                ...
                if (string.IsNullOrEmpty(enumDes))
                {
                    continue;
                }
        ...
        /// <summary>
        /// 获取枚举上需定义Description属性
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <returns>返回Description属性数组</returns>
        public static string[] GetEnumDescriptionToArray<T>()
        {
            string description = Enum.GetValues(typeof(T)).Cast<T>().Aggregate("", (current, item) => current + (current == "" ? GetEnumDescription((Enum)(object)item, false) : "," + GetEnumDescription((Enum)(object)item, false)));
            return description.Split(',').ToArray();
        }
    }
}
