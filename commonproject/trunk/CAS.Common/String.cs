...
namespace CAS.Common
{
    public class StringHelper
    {
        ...
        public static string LetterReplace(string letter)
        {
            ...
            dic["A"] = "原购价"; dic["B"] = "现评估总值"; dic["C"] = "建筑面积"; dic["D"] = "增值税";
        ...
        /// <returns></returns>
        public static string NumericToPrice(string strBumeric,bool isShowDigits=false)
        {
            return NumericToPrice(strBumeric, true, isShowDigits);
        }

        /// <summary>
        /// 数字转千分为货币数字（caoq 2013-8-21）
        /// 潘锦发 修改 20150915
        /// 添加参数是否显示原始小数位, 由于不清楚原参数containDecimal(是否保留小数) 是否涉及到一些不需要显示原始小数位的,所以加了一个可选参数
        /// 是否显示原始小数位 :即要求保留多少位就是多少位, 而不是
        /// </summary>
        /// <param name="strBumeric"></param>
        /// <param name="containDecimal">是否保留小数</param>
        /// <param name="isShowDigits">是否显示原始小数位</param>
        /// <returns></returns>
        public static string NumericToPrice(string strBumeric, bool containDecimal, bool isShowDigits=false)
        {
            ...
            if (strBumeric.IndexOf(".") != -1 && containDecimal)
            {
                if (isShowDigits)
                {
                    num = num + "." +strBumeric.Split('.')[1]; //默认保留所有位数
                }
                else
                {
        ...
        public static List<long> ConvertToLongList(string str)
        {
            ...
            return ConvertToLongList(str.Split(','));
        }
        /// <summary>
        /// 将字符串转换成list(逗号分隔)
        /// </summary>
        ...
        /// <summary>
        /// 比较两个数组是否相等，注意：长度为0等于数组为null
        /// </summary>
        /// <param name="str1"></param>
        /// <param name="str2"></param>
        /// <returns></returns>
        public static bool CompareArray(string[] str1, string[] str2)
        {
            ...
            if(str1.Where(_s=>!str2.Contains(_s)).Count()>0)
            {
                return false;
            }
            return true;
       ...
       /// <returns></returns>
        ...
        /// 获取时间段,20160708,rock
        /// </summary>
        /// <param name="dateType">d:按天，m：按月，j：按季度，y：按年</param>
        /// <param name="startDate"></param>
        /// <param name="endDate"></param>
        /// <returns></returns>
        public static List<string> GetDateTimeSlot(string dateType, DateTime startDate, DateTime endDate)
        {
            ...
            switch (dateType)
            {
                case "d":
                    ...
                    start = Convert.ToDateTime(startDate.ToString("yyyy-MM-dd"));
                    end = Convert.ToDateTime(endDate.ToString("yyyy-MM-dd 23:59:59"));
                    ...
                    break;
                case "m":
                    start = Convert.ToDateTime(startDate.ToString("yyyy-MM-01"));
                    end = Convert.ToDateTime(endDate.ToString("yyyy-MM-01 23:59:59")).AddMonths(1).AddDays(-1);
                    for (; start <= end; start = start.AddMonths(1))
                    {
                        dateList.Add(start.ToString("yyyy-MM-01"));
                    }
                    break;
                case "j":
                    start = Convert.ToDateTime(startDate.ToString("yyyy-MM-01"));
                    end = Convert.ToDateTime(endDate.ToString("yyyy-MM-01 23:59:59")).AddMonths(1).AddDays(-1);
                    for (; start <= end; start = start.AddMonths(3))
                    {
                        dateList.Add(start.ToString("yyyy-MM-01"));
                    }
                    break;
                case "y":
                    start = Convert.ToDateTime(startDate.ToString("yyyy-01-01"));
                    end = Convert.ToDateTime(endDate.ToString("yyyy-12-30 23:59:59"));
                    for (; start <= end; start = start.AddYears(1))
                    {
                        dateList.Add(start.ToString("yyyy-MM-01"));
                    }
                    break;
