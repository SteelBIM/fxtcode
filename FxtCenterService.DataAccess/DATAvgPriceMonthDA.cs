...
namespace FxtCenterService.DataAccess
{
    public class DATAvgPriceMonthDA : Base
    {
        ...
        public static DataSet GetAvgPriceList(SearchBase search, DateTime datefrom, DateTime dateto)
        {
            SqlCommand cmd = new SqlCommand();
            string sql = SQL.Project.AvgPriceList;
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@cityid", search.CityId, SqlDbType.Int));
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@datefrom", datefrom, SqlDbType.DateTime));
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@dateto", dateto, SqlDbType.DateTime));
