...
if (@max_x > 0 and @min_x > 0 and @max_y> 0 and @min_y > 0)
	...
	insert into @tbl_prolist (projectid,projectname,cityid,distince,projectx,projecty)	values (@projectid,@cityid,@projectname,0,@x,@y)--包括自己
	...
	if(select count(*) from @tbl_prolist)>0
		begin
			...
			--上月均价
			,(select top 1 avgprice from dbo.DAT_ProjectAvgPrice pre with(nolock) where pre.projectid=t.projectid and pre.cityid=t.cityid and pre.avgpricedate = @preavgdate and t.PurposeType=@purposetype and ISNULL(t.BuildingAreaType,0)=0 and ISNULL(t.BuildingTypeCode,0)=0 and daterange=@daterange order by pre.id desc) as preavgprice
			from dbo.DAT_ProjectAvgPrice t with(nolock)
			inner join @tbl_prolist p on p.projectid = t.projectid and t.cityid=p.cityid
			inner join (
