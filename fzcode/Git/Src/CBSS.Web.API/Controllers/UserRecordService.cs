...
namespace CBSS.Web.API
{
    ...
    public class UserRecordService : ICBSSService
    {
        private static object o = new object();
        static string ss = "Local_Addr";
        private static string serverid = HttpContext.Current.Request.ServerVariables.Get(ss).ToString();
           ...
           APIResponse requestdata = apidate.JudgeRequestData(Key, Info, FunName, FunWay);
            ...
            try
            {
                ...
                if (requestinfo != null)
                {
                    ...
                    #region 接口分发
                    APIResponse DistributeUrl = tbxService.GetAPIDistributeUrl(Flag.UserID, Flag.ModelID);
                    if(DistributeUrl.Success)//接口分发
                    {
                        string url = DistributeUrl.Data + "/uc/active";
                        APIResponse response =HttpHelper.HttpPost<APIResponse>(url, requestdate, SerializationType.Json);
                        return response;
                    }                    
                    #endregion
                    ...
                    Type objectCon = typeof(UserRecordController);
