        ...
        */
        public static string Run(string body, string total_fee, string auth_code)
        {
            ...
            if (!result.IsSet("return_code") || result.GetValue("return_code").ToString() == "FAIL")
		    {
                ...
                Log4NetHelper.Error(LoggerType.ApiExceptionLog, "Micropay API interface call failure, result : " + result.ToXml(), null);
		    ...
		    {
                Log4NetHelper.Error(LoggerType.ApiExceptionLog, "micropay API interface call success, business failure, result : " + result.ToXml(), null);
                return result.ToPrintStr();
		    }
		    ...
		    while(queryTimes-- > 0)
		    {
                ...
                {
                    Log4NetHelper.Error(LoggerType.ApiExceptionLog, "Micropay failure, return micropay result : " + result.ToXml(), null);
                    return result.ToPrintStr();
			    }
		    }

            //确认失败，则撤销订单
            Log4NetHelper.Error(LoggerType.ApiExceptionLog, "Micropay failure, Reverse order is processing...", null);
		    if(!Cancel(out_trade_no))
		    {
                Log4NetHelper.Error(LoggerType.ApiExceptionLog, "Reverse order failure", null);
