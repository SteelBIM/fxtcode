...
namespace CBSS.Web
{
    public class CookieContext : IAuthCookie
    {
        ...
        public virtual string VerifyCode
        {
            get
            {
                var verifyCode = SecurityHelper.Decode(Cookie.GetValue(KeyPrefix + "VerifyCode"));

                //获取完Cookie后马上过期，重新生成新的验证码
                Cookie.Save(KeyPrefix + "VerifyCode", SecurityHelper.Encode(DateTime.Now.Ticks.ToString()), 1);

                return verifyCode;
            }
            set
            {
                Cookie.Save(KeyPrefix + "VerifyCode", SecurityHelper.Encode(value), 1);
            }
        }
