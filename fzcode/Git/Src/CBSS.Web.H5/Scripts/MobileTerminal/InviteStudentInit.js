...
var InviteStudentInit = function () {
    ...
    $("#getCheckCode").click(myclick = function () {
        ...
        $.post("sendCode", obj, function (data) {
            if (data) {
    ...
    $("#next").click(function () {
        ...
        var data = {
            Telephone: $.trim($("#telephone").val()), Type: 26, Code: $.trim($('#verifyCode').val()),
            PKey: "", RTime: Common.DateNow()
        };
        var obj = { FunName: "GetClassByTelePhone", Info: $.toJSON(data), FunWay: "0", Flag: "", Key: "" }
        $.post("http://192.168.3.1:8026/dc/active", $.toJSON(obj), function (data) {
            var result = eval("(" + data + ")");
            if (result.Success) {
                if (result.UserType == "12") {
                    ...
                    window.location.href = Constant.classInfo_Url + "Class/JoinClass.html?UserID=" + Current.UserID + "&StudentID=" + result.UserID + "&AppID=" + Current.APPID;
                }
            } else {
                popup(result.Msg);
                return false;
