...
var TeacherInfoInit = function () {
    ...
    this.UpdateUserInfo = function () {
        ...
        if (name == '') {
            ...
            //    time--;
            //    if (time == 0) {
            window.location.href = "TeacherInfo.html?UserID=" + Current.UserID;
            //    }
            //}, 1000)
        } else {
            var data = { UserID: Current.UserID, TrueName: name, SchoolID: Current.SchoolID, SchoolName: Current.SchoolName, PKey: "", RTime: Common.DateNow() };
            var obj = { FunName: "UpdateUserInfobyLocal", Info: $.toJSON(data), FunWay: "0", Flag: "", Key: "" }
            $.post("http://192.168.3.1:8024/dc/active", $.toJSON(obj), function (data) {
                if (data) {
                    var result = eval("(" + data + ")");
                    if (result.Success) {
