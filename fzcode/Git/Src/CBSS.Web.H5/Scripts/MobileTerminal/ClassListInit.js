...
var ClassListInit = function () {
    ...
    this.GetClassList = function () {
        ...
        var data = {
            UserID: UserID: current.UserID, SubjectID: 3,
            ...
            if (data) {
                ...
                if (result.Success) {
                    ...
                    if (classList.length <= 0) {
                        window.location.href = "CreateClass.html?UserID=" + current.UserID;
                        return;
                    }
                    ...
                    for (var i = 0, length = classList.length; i < length; i++) {
                        ...
                        html += '<img id="' + classList[i].Id + '" src="../../Content/AppTheme/images/xuan1.png" alt="" />';
                        html += '</div>';
                        html += '<div class="line-normal-wrapper">';
                        html += '<a class="a_a" href="ClassDetail.html?ClassID=' + classList[i].Id + '&ClassName=' + encodeURI(encodeURI(classList[i].ClassName)) + '&UserID=' + current.UserID + '&AppID=' + current.APPID + '">';
                        html += '<div class="line-normal-left-wrapper">';
                        html += '<div class="line-normal-avatar-wrapper">';
                        html += '<img src="../../Content/AppTheme/images/tou.png" />';
                        html += '</div>';
                        ...
                        html += '<img src="../../Content/AppTheme/images/zongpeo.png" /><span>' + classList[i].StudentNum + '人</span>'; //<b>新增0人<img src="images/tishi.png" alt="" /></b>
                        html += '</div></a>';
                        html += '</div>';
                        html += '<div class="line-btn-delete">';
                        html += '<a id="' + classList[i].Id + '"><img src="../../Content/AppTheme/images/delete.png" alt="" /><p>删除</p></a>';
                        html += '</div>';
                        html += '</div>';
                        html += '</div>';
                    }
                    ...
                    window.location.href = "CreateClass.html?UserID=" + current.UserID + "&AppID=" + current.APPID;
                    //}
                }
            } else {
                popup("发送请求失败，请重新发送");
    ...
    this.DeleteClass = function () {
        if (current.DeleteIDStr.length > 0) {
            ...
            var data = {
                IDStr: idStr, UserID: current.UserID, SubjectID: 3 ,
                PKey: "", RTime: Common.DateNow()
            };
            var obj = { FunName: "DeleteClassList", Info: $.toJSON(data), FunWay: "0", Flag: "", Key: "" }
            $.post("http://192.168.3.1:8026/dc/active", $.toJSON(obj), function (data) {
                if (data) {
    ...
    $("#me").click(function () {
        window.location.href = Constant.classInfo_Url +"Account/TeacherInfo.html?UserID=" + current.UserID + "&AppID=" + current.APPID;
    });
            ...
            if (data) {
                ...
                if (result.Success) {
                    ...
                    if (result.UserInfo.SchoolID != null && result.UserInfo.SchoolID !== "" && result.UserInfo.SchoolID != 0) {
                        ...
                        window.location.href = Constant.classInfo_Url +"Account/AddInformation.html?UserID=" + current.UserID;
                    }
                }
            }
        });
