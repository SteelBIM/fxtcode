...
var ChooseClassInit = function () {
    ...
    $("#nextStep").click(function () {
        ...
        var data = {
            UserID: Current.UserID, SchoolID: Current.SchoolID, ClassStr: classStr,
            PKey: "", RTime: Common.DateNow()
        };
        var obj = { FunName: "TeaBindClass", Info: $.toJSON(data), FunWay: "0", Flag: "", Key: "" }
        $.post("http://192.168.3.1:8026/dc/active", $.toJSON(obj), function (data) {
            if (data) {
                ...
                if (result.Success) {
                    ...
                    setTimeout(window.location.href = "ClassList.html?UserID=" + Current.UserID + "&SchoolID=" + Current.SchoolID,1000);
                } else {
                    popup(result.Msg);
                    $(".main2 .show").css("display", "none");
                    return false;
