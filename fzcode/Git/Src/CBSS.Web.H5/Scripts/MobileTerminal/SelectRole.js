        ...
        $("#queding").click(function () {
            ...
            window.WebViewJavascriptBridge.callHandler(
                'startWebPage', data, function (responseData) {
                    ...
                    var data = {
                        UserID: sc.UserId, AppID: sc.AppId ,
                        PKey: "", RTime: Common.DateNow()
                    };
                    var obj = { FunName: "GetUserTypebyId", Info: $.toJSON(data), FunWay: "0", Flag: "", Key: "" }
                        ...
                        if (result.Success) {
                            if (current.UserType == "12") {
                                if (result.UserType == current.UserType) {
                                    window.location.href = "ClassList.html?UserID=" + sc.UserId + "&AppID=" + sc.AppId;
                                } else {
