...
function loadSkinCss() {
    ...
    if (cookie == "" || cookie == undefined) {
        loadStyleCss("simple"); //如果cookie中没有值，则调用默认皮肤样式表
    }
    else {
...
$.fn.ruleHtmlSelect = function () {
    var ruleHtmlSelect = function (selectObj) {
        ...
        $(itemObj).children("ul").css({ "width": allW + 38 }); //slect下拉框宽度*/
        ...
        selectObj.find("option").each(function (i) {
            ...
            liObj.click(function () {
                ...
                //将点击事件公布出去，以进行其它的操作
                optionSelected(selectObj.parent("div"), selectObj.find("option:selected").attr("id"), selectObj.find("option:selected").text());
            });
        });
...
function optionSelected(curObj, selectedIndex, selectedText) {
    ...
    //    alert(curObj.context.id);
    //    if ($(curObj).attr("id") == "selectSstage") {//联动刷新
    //        $("#selectSstage").val(selectedIndex);
    //        alert($("#selectSstage").val(selectedIndex));
    //    }
    //    
    if (curObj.context.id == "selectEdition" || curObj.context.id == "selectSstage" || curObj.context.id == "selectOrder") {
        if (curObj.context.id == "selectEdition") {
            LoadEditionSStage(selectedIndex);
            Check();
        }
        if (curObj.context.id == "selectOrder") {
            resourceSetInit.orderBy = selectedIndex;
            resourceSetInit.InitResource();
        }
...
function selectAllMessages(obj) {
    ...
    if ($(obj).prop("checked")) {//勾选
        $("input:checkbox").not(obj).each(function (n) {
            var isuse = "0";
            isuse = $(this).parent().next().text();
            if (!$(this).prop("checked") && isuse == "0") { 
                $(this).prop("checked", true);
                $(this).prev().remove(); //清空checkbox皮肤
                $(this).ruleSingleHtmlCheckbox(); //重新加载checkbox皮肤
                idArr += "," + $(this).attr("id");
