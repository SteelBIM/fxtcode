using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using CBSS.Tbx.Contract;
using CBSS.Tbx.DAL;
using CBSS.Core.Utility;
using System.Data.Objects;
using CBSS.Framework.Contract;
using EntityFramework.Extensions;
using CBSS.Core.Cache;
using CBSS.Framework.DAL;

namespace CBSS.Tbx.BLL
{
    public class TbxService : ITbxService
    {
        Repository repository = new Repository();
        #region Article CURD
        public Article GetArticle(int id)
        {
            return repository.GetByID<Article>(id);
        }

        public IEnumerable<Article> GetArticleList(ArticleRequest request = null)
        {
            request = request ?? new ArticleRequest();

            var articles = repository.ListAll<Article>();

            if (!string.IsNullOrEmpty(request.Title))
                articles = articles.Where(u => u.Title.Contains(request.Title));

            if (request.ChannelId > 0)
                articles = articles.Where(u => u.ChannelId == request.ChannelId);

            if (request.IsActive != null)
                articles = articles.Where(u => u.IsActive == request.IsActive);

            return articles.OrderByDescending(u => u.ID).Skip((request.PageIndex - 1) - request.PageSize).Take(request.PageSize);

        }

        public void SaveArticle(Article article)
        {

            var tags = new List<Tag>();

            foreach (var tag in article.Tags)
            {
                var existTag = repository.SelectSearch<Tag>(o=>true).FirstOrDefault(t => t.Name == tag.Name);
                if (existTag != null)
                    existTag.Hits++;
                tags.Add(existTag ?? tag);
            }

            if (article.ID > 0)
            {
                article.TagString = string.Empty;             
               // dbContext.Entry(article).Collection(m => m.Tags).Load();
                //article.Tags = tags;
                //dbContext.SaveChanges();
                repository.Update<Article>(article);
            }
            else
            {
                article.Tags = tags;
                repository.Insert<Article>(article);
            }

        }

        public void DeleteArticle(List<int> ids)
        {
            //using (var dbContext = new TbxDbContext())
            //{
            //    dbContext.Articles.Include("Tags").Where(u => ids.Contains(u.ID)).ToList().ForEach(a => { a.Tags.Clear(); dbContext.Articles.Remove(a); });
            //    dbContext.SaveChanges();
            //}
        }
        #endregion


        #region Channel CURD
        public Channel GetChannel(int id)
        {
            //using (var dbContext = new TbxDbContext())
            //{
            //    return dbContext.Find<Channel>(id);
            //}
            return null;
        }

        public IEnumerable<Channel> GetChannelList(ChannelRequest request = null)
        {
            request = request ?? new ChannelRequest();

            var channels = repository.ListAll<Channel>();

                if (!string.IsNullOrEmpty(request.Name))
                    channels = channels.Where(u => u.Name.Contains(request.Name));

                if (request.IsActive != null)
                    channels = channels.Where(u => u.IsActive == request.IsActive);

                return channels.OrderByDescending(u => u.ID).Skip(request.PageSize*--request.PageIndex).Take(request.PageSize);
            
        }

        public void SaveChannel(Channel channel)
        {
            //using (var dbContext = new TbxDbContext())
            //{
            //    if (channel.ID > 0)
            //    {
            //        dbContext.Update<Channel>(channel);
            //    }
            //    else
            //    {
            //        dbContext.Insert<Channel>(channel);
            //    }
            //}
        }

        public void DeleteChannel(List<int> ids)
        {
            //using (var dbContext = new TbxDbContext())
            //{
            //    dbContext.Channels.Where(u => ids.Contains(u.ID)).Delete();
            //}
        }
        #endregion

        public IEnumerable<Tag> GetTagList(TagRequest request = null)
        {
            request = request ?? new TagRequest();
           
              var tags = repository.ListAll<Tag>();

                if (request.Orderby == Orderby.Hits)
                    return tags.OrderByDescending(u => u.Hits).Skip(request.PageSize*--request.PageIndex).Take(request.PageSize);
                else
                    return tags.OrderByDescending(u => u.ID).Skip(request.PageSize * --request.PageIndex).Take(request.PageSize);
            
        }
    }
}
