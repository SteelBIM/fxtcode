...
namespace CBSS.Cfgmanager.BLL
{
    public partial class CfgmanagerService : ICfgmanagerService
    {
        ...
        {
            return repository.SelectSearch<Sys_ApiFunction>("ApiFunctionID in(" + ApiFunctionIDs + ")").OrderBy(a=>a.SystemCode).ToList();
        }
        public List<Sys_ApiFunction> GetApiFunctionList(string ApiFunctionName, int SystemCode)
        {
            if (SystemCode>0)
            {
                return repository.SelectAppointField<Sys_ApiFunction>(a => a.ApiFunctionName.Contains(ApiFunctionName) && a.SystemCode == SystemCode, "ApiFunctionID").ToList();
            }
            return repository.SelectAppointField<Sys_ApiFunction>(a => a.ApiFunctionName.Contains(ApiFunctionName), "ApiFunctionID").ToList();
        }
        /// <summary>
        ...
        public IEnumerable<Sys_ApiFunction> GetAllSys_ApiFunction(out int totalcount, ApiFunctionRequest request = null)
        {
            ...
            pageParameter.OrderColumns = p => p.ApiFunctionID;
