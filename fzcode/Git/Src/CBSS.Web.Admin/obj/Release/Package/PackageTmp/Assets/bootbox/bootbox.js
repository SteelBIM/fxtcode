...
function addcloud() {//增加页面遮罩
    ...
    var bodyHeight = window.screen.availHeight;// document.documentElement.clientHeight; //Math.max(document.documentElement.clientHeight, document.body.scrollHeight);
    ...
    loadingObj.style.background = "#000000";
    loadingObj.style.filter = "progid:DXImageTransform.Microsoft.Alpha(style=3,opacity=25,finishOpacity=75";
    loadingObj.style.opacity = "0.1";
    loadingObj.style.position = "fixed";
    loadingObj.style.top = (bodyHeight-106) / 2 + "px";
    loadingObj.style.left = (bodyWidth-206) / 2 + "px";
    //loadingObj.style.background = "white";//url(/images/loading.gif)
    loadingObj.innerHTML = "<img style='width:36px;' src=\"/Content/Images/loading.gif\" /><p style='color:white; float:right;   margin-top:8px;'>" + "Loading pages..." + "</P>";//showName
    loadingObj.style.padding = "30px";
    loadingObj.style.width = "206px";
    loadingObj.style.height = "46px";
...
///////////////////////////
function addcloudNew(showName) {//增加页面遮罩
    var bodyWidth = document.documentElement.clientWidth;
    ...
    //loadingObj.style.background = "white";//url(/images/loading.gif)
    loadingObj.innerHTML = "<img style='width:36px;'  src=\"/Content/Images/loading.gif\" /><p style='color:white'>" + showName + "</P>";//showName
    loadingObj.style.width = "63px";
    loadingObj.style.height = "36px";
    loadingObj.style.zIndex = "10000";
    document.body.appendChild(loadingObj); //添加loading动画-
}
function removecloudNew() {//移除页面遮罩
    $("#loadingDiv").remove();
    $("#bgDiv").remove();
}
//获取url
...
//字符串截取
function SubStr(str, number, com) {
    if (str != null && str != "") {
        if (str.length > number) {
            return str.substr(0, number) + com;
        }
    }
    return str;
}
function TrimEnd(str, character) {
    if (str != null && str != "" && str.length > 1) {
        str = $.trim(str);
        if (str.substr(str.length - 1, str.length) == character) {
            return str.substr(0, str.length - 1);
        }
    }
}
function TrimEndChar(str, character) {
    if (str != null && str != "" && str.length > 1) {
        str = $.trim(str);
        if (str.substr(str.length - 1, str.length) == character) {
            return str.substr(0, str.length - 1);
...
function TrimStateChar(str, character) {
    if (str != null && str != "" && str.length > 1) {
        str = $.trim(str);
...
//数字转成日期类型 前面加描述 
function numbercontime(timenumber, comment) {
    if (timenumber != null && timenumber != "" && timenumber > 0) {
        var newDate = new Date();
        newDate.setTime(timenumber * 1000);
        return AddMonesDay(newDate.toLocaleDateString()) + comment;
    }
    return "";
}

function AddMonesDay(time) {
...
function DataToFixed(data) {//取小数位数
