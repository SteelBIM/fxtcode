...
(function () {
    ...
    if (locationHref.indexOf('locationHref') == -1)
    {
        locationHref = locationHref.replace("ModelImgLibrary", "Models").replace("Tbx/Module/ModuleManage", "Tbx/Module/Index").replace("AppVersion", "App").replace("AppModule", "App").replace("GoodPrice", "Good").replace("TableFiledListView","Index/")
    }
            ...
            $(this).next("ul").each(function () {
                $("a", $(this)).each(function () {
                    if (locationHref.indexOf($(this).attr("href")) > 0) {
                        ...
                        $("#navigation .breadcrumb li:eq(2) a").attr("href", locationHref);
                        document.title = $(this).text() + " - " + document.title;
