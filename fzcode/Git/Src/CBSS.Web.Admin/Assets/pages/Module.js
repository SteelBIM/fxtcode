...
var ModuleTableInit = function () {
    ...
    oTableInit.Init = function (edit) {
        $('#tb_booktable').bootstrapTable({
            url: '/Tbx/Module/GetModulePage',         //请求后台的URL（*）
            method: 'post',                     //请求方式（*）
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    if (edit == "True")
                        return "<a class='btn mini green thickbox' title='编辑模块' href=\"/Tbx/Module/ModuleManage/" + row.MarketBookID + "\"     ><i class='icon-edit'> </i> 编辑模块  </a >";
                    return "";
                }
            }]
        });
...
$("#searchbtn").click(function () {
    $('#tb_booktable').bootstrapTable('selectPage', 1);
})
...
//    var selects = $('#tb_booktable').bootstrapTable('getSelections');
//    var ids = '';
//    for (var i = 0; i < selects.length; i++) {
//        if (ids) {
//            ids += "," + selects[i].MarketBookID;
//        } else {
//            ids += selects[i].MarketBookID;
//        }
//    }
