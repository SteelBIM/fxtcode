...
namespace CBSS.Framework.Web
{
    public class ControllerBase : Controller
    {
        ...
        public ContentResult Alert(string alert = null)
        {
            var script = string.Format("<script>{0};</script>", string.IsNullOrEmpty(alert) ? string.Empty : "alert('" + alert + "')");
        ...
        {
            var script = string.Format("<script>parent.location.reload(1)</script>");
            return this.Content(script);
        }
#pragma warning disable CS0109 // 成员“ControllerBase.RefreshParentTab(string)”不会隐藏可访问成员。不需要关键字 new。
        public new ContentResult RefreshParentTab(string alert = null)
#pragma warning restore CS0109 // 成员“ControllerBase.RefreshParentTab(string)”不会隐藏可访问成员。不需要关键字 new。
        {
