...
namespace CBSS.IBS.BLL
{
    public class MOD2IBS_SchInfoBLL :CommonBLL, IIBS_MOD_SchInfoBLL
    {
        ...
        public bool Add(IBS_SchClassRelation SchInfo)
        {
            ...
            {
                Log4NetHelper.Error(LoggerType.ServiceExceptionLog, "学校Add接口异常，SchInfo=" + SchInfo.ToJson(), ex);
                //新增到异常处理消息队列
        ...
        public bool Update(IBS_SchClassRelation SchInfo)
        {
            ...
            {
                Log4NetHelper.Error(LoggerType.ServiceExceptionLog, "学校Update接口异常，SchInfo=" + SchInfo.ToJson(),ex);
                //新增到异常处理消息队列
                MOD2IBS_PushDataException excep = new MOD2IBS_PushDataException();
                excep.DataType = 3;
                excep.ChangeType = 1;
                excep.Json = SchInfo.ToJson().ToString();
        ...
        public bool Delete(IBS_SchClassRelation SchInfo)
        {
            ...
            {
                Log4NetHelper.Error(LoggerType.ServiceExceptionLog,"学校DEL接口异常，SchInfo=" + SchInfo.ToJson(),ex);
                //新增到异常处理消息队列
                MOD2IBS_PushDataException excep = new MOD2IBS_PushDataException();
                excep.DataType = 3;
                excep.ChangeType = 3;
