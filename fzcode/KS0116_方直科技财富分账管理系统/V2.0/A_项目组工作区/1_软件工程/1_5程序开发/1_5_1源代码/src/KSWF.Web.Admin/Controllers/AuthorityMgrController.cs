...
namespace KSWF.Web.Admin.Controllers
{
    ...
    public class AuthorityMgrController : BaseController
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult AuthorityMgr_Add(string groupname, int dataauthority, string actions)
        {
            ...
            com_group subdata = new com_group() { groupname = groupname, dataauthority = dataauthority, creatername = masterinfo.mastername, description = "", delflg = 0, createtime = DateTime.Now.ToString() };
        ...
        public JsonResult AuthorityMgr_View(int pagesize, int pageindex)
        {
            ...
            pageParameter.PageSize = pageindex;
            if (masterinfo.agentid == Core.Utility.PublicHelp.OrgId)
            {
                pageParameter.Where = t1 => t1.delflg == 0;//根据权限加载的参数
            }
            else
            {
                pageParameter.Where = t1 => (t1.delflg == 0 && t1.creatername == "系统默认");//根据权限加载的参数
            }
            pageParameter.OrderColumns = t1 => t1.groupid;
            int total;
            var rows = base.Manage.SelectPage<com_group>(pageParameter, out total);
