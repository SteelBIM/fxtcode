...
var channelmanager = decodeURI(getQueryString("q"));
...
$(function () {
    ...
    $("#channelmanager").html(channelmanager);
    $("#papername").html(channelmanager);
        ...
        if (data != null && data != "") {
            ...
            $.post("/CommissionPay/GetRecentlySettledTime", { mastername: $.trim(mastername) }, function (data) {
                if (data != null && data != "" && data > 0) {
                    ...
                    //$("#spanstarttime").html(numbercontime(data, ""));
                }
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_ments').bootstrapTable({
            url: '/CommissionPay/AgentidOrdreInfo_View',         //请求后台的URL（*）
            method: 'post',                     //请求方式（*）
...
function EmployeeTotal() {
    $.post("/CommissionPay/GetAgentOrderTotal", {
        mastername: $.trim(mastername),
        endtime: addDate($("#enddate").val(), 1)
    }, function (data) {
        if (data != null && data != "") {
...
function Export() {
    ...
    $form.attr('action', "/CommissionPay/AgentExport");
    ...
    $.post("/CommissionPay/GetAgentOrderTotal", {
        ...
        if (data != null && data != "") {
            ...
            if (jsonData.o_bonus > 0) {
                ...
                $.post("/CommissionPay/GetAgentBasicInformation", { mastername: $.trim(mastername) }, function (data) {
                    if (data != null && data != "") {
                        var jsondata = eval(data);
                        if (jsondata[0].agent_tel != null)
                            $("#agent_tel").html(jsondata[0].agent_tel);
                        if (jsondata[0].agent_fax != null)
                            $("#agent_fax").html(jsondata[0].agent_fax);
                        $("#deptnameagent").html(jsondata[0].deptname);
                        if (jsondata[0].agent_addr != null)
                            $("#address").html(jsondata[0].agent_addr);
                        $("#agenttruename").html(jsondata[0].truename);
                        if (jsondata[0].mobile != null)
                            $("#iphone").html(jsondata[0].mobile);
                        $("#agentname").html(jsondata[0].agentname);
                    }
                });
...
var TableKont = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_kont').bootstrapTable({
            url: '/CommissionPay/AgentKontInformationDisplay',         //请求后台的URL（*）
            method: 'post',                     //请求方式（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
...
$("#btn_settlement").click(function () {
    ...
    addcloud();
    $.post("/CommissionPay/AgentSetbonus_Add", { os_no: $("#KontNumbering").html(), endtime: addDate($("#enddate").val(), 1), mastername_t: mastername, adjust_amount: adjust_amount, adjust_reason: adjust_reason }, function (reust) {
        if (reust > 0) {
            removecloud();
            bootbox.setDefaults("locale", "zh_CN");
            bootbox.alert("结算成功~", function (result) {
                location.href = "/CommissionSingle/Index";
            });
        } else {
            removecloud();
