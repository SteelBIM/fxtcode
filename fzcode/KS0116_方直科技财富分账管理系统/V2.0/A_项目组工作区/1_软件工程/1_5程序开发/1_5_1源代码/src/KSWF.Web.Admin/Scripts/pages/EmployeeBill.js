...
$(function () {
    if (type != null) {
        ...
        //$("#ReturnPreviousLayer").html("<input type=\"button\" onclick=\"location.href='/CommissionSingle/Index?t=1'\" class=\"btn btn-default\" value=\" 返回上一层\" />");
        $("#ReturnPreviousLayer").attr("onclick", "location.href='/CommissionSingle/Index?t=1'");
    }
    ...
    $.post("/CommissionSingle/GetEmployeeBillDetailed", { os_no: $.trim(os_no) }, function (data) {
        if (data != null && data != "") {
            var jsondata = eval(data);
            ...
            $("#enployeename").html(masterlist[0].truename);
...
function Export() {
    ...
    $form.attr('action', "/CommissionSingle/Employee_Export");
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_ments').bootstrapTable({
            url: '/CommissionSingle/EmployeeOrdreInfo_View',         //请求后台的URL（*）
            method: 'post',                     //请求方式（*）
...
var TableKont = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_kont').bootstrapTable({
            url: '/CommissionSingle/KontInformationDisplay',         //请求后台的URL（*）
            method: 'post',                     //请求方式（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
