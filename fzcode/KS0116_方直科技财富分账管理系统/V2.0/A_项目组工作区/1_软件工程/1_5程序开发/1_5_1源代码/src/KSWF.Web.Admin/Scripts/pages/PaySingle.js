$(function () {
    //$(".nav-pills li:eq(0)").click();
    $.post("/PaySingle/GetcurrentAction", function (data) {
        if (data != null && data != "") {
...
function Statistics(pannelType) {
    ...
    $.post("/PaySingle/Statistics", { pannelType: pannelType, sTime: sTime, eTime: eTime }, function (data) {
        if (data) {
            if (pannelType == 0) {
                $("#agentTotalBonus").html(DataToFixed(data.totalBonus));
            } else if (pannelType == 1) {
                $("#employeeTotalBonus").html(DataToFixed(data.totalBonus));
            } else {
                $("#departmentTotalBonus").html(DataToFixed(data.totalBonus));
            }
            removecloud();
        }
        else {
...
var oTableInitEmployee = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_employee').bootstrapTable({
            url: '/PaySingle/GetPageList',      //请求后台的URL（*）
            method: 'post',                     //请求方式（*）
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    var action = "";
                    action += " <a  href=\"/CommissionSingle/EmployeeBill?os_no=" + row.os_no + "&n=" + row.mastername_t + "&t=1" + "\"> 查看 <a> "
                    return action;
                }
            }]
        });
...
function employeeexportbtn() {
    ...
    $form.attr('action', "/PaySingle/ExportOrderXls");
    for (var key in obj) {
    ...
    oTableInit.Init = function () {
        $('#tb_department').bootstrapTable({
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    var action = "";
                    action += " <a  href=\"/CommissionSingle/DeptBill?os_no=" + row.os_no + "&n=" + row.mastername_t + "&t=2" + "\"> 查看 <a> ";
                    return action;
                }
            }]
        });
    ...
    oTableInit.Init = function () {
        $('#tb_agent').bootstrapTable({
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    var action = "";
                    action += " <a  href=\"/CommissionSingle/ViewBill?os_no=" + row.os_no + "&n=" + row.mastername_t + "&t=0" + "\"> 查看 <a> "
                    return action;
                }
            }]
        });
