...
namespace Kingsun.SynchronousStudy.Pay.lib
{
    ...
    {
        private static string _szWxAppid = WebConfigurationManager.AppSettings["sz_wx_appid"];
        private static string _szWxMchid = WebConfigurationManager.AppSettings["sz_wx_mchid"];
        private static string _szWxKey = WebConfigurationManager.AppSettings["sz_wx_key"];
        private static string _bjWxAppid = WebConfigurationManager.AppSettings["bj_wx_appid"];
        private static string _bjWxMchid = WebConfigurationManager.AppSettings["bj_wx_mchid"];
        private static string _bjWxKey = WebConfigurationManager.AppSettings["bj_wx_key"];
        private static string _shbdAppid = WebConfigurationManager.AppSettings["shbd_appid"];
        private static string _shbdMchid = WebConfigurationManager.AppSettings["shbd_mchid"];
        private static string _shbdKey = WebConfigurationManager.AppSettings["shbd_key"];
        private static string _shqgWxAppid = WebConfigurationManager.AppSettings["shqg_wx_appid"];
        private static string _shqgWxMchid = WebConfigurationManager.AppSettings["shqg_wx_mchid"];
        private static string _shqgWxKey = WebConfigurationManager.AppSettings["shqg_wx_key"];
        private static string _gdAppid = WebConfigurationManager.AppSettings["gd_appid"];
        private static string _gdMchid = WebConfigurationManager.AppSettings["gd_mchid"];
        private static string _gdKey = WebConfigurationManager.AppSettings["gd_key"];
        private static string _gzAppid = WebConfigurationManager.AppSettings["gz_appid"];
        private static string _gzMchid = WebConfigurationManager.AppSettings["gz_mchid"];
        private static string _gzKey = WebConfigurationManager.AppSettings["gz_key"];
        private static string _rjpepWxAppid = WebConfigurationManager.AppSettings["rjpep_wx_appid"];
        private static string _rjpepWxMchid = WebConfigurationManager.AppSettings["rjpep_wx_mchid"];
        private static string _rjpepWxKey = WebConfigurationManager.AppSettings["rjpep_wx_key"];
        ...
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
        {
            ...
            {
                Log4Net.LogHelper.Error("缺少统一支付接口必填参数out_trade_no！");
                throw new WxPayException("缺少统一支付接口必填参数out_trade_no！");
            }
            else if (!inputObj.IsSet("body"))
            {
                Log4Net.LogHelper.Error("缺少统一支付接口必填参数body！");
                throw new WxPayException("缺少统一支付接口必填参数body！");
            }
            else if (!inputObj.IsSet("total_fee"))
            {
                Log4Net.LogHelper.Error("缺少统一支付接口必填参数total_fee！");
                throw new WxPayException("缺少统一支付接口必填参数total_fee！");
            }
            else if (!inputObj.IsSet("trade_type"))
            {
                Log4Net.LogHelper.Error("缺少统一支付接口必填参数trade_type！");
            ...
            {
                Log4Net.LogHelper.Error("统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！！");
                throw new WxPayException("统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！");
            }
            if (inputObj.GetValue("trade_type").ToString() == "NATIVE" && !inputObj.IsSet("product_id"))
            {
                Log4Net.LogHelper.Error("统一支付接口中，缺少必填参数product_id！trade_type为JSAPI时，product_id为必填参数！！");
	    ...
	    */
        ...
        public static WxPayData GetPrePayInfo(string orderid, int totalmoney, string body, string packageName)
        {
            ...
            Log4Net.LogHelper.Info("统一下单返回的结果值=====：" + data.ToXml());
