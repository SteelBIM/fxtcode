        ...
        public static string GetText(string FromUserName, string ToUserName, string Content)
        {
            ...
            switch (Content)
            {
                case "关键字":
                    ...
                    break;
                case "测试":
                    XML = ReText(FromUserName, ToUserName, "测试");
                    break;
                case "关注成功":
                    XML = ReText(FromUserName, ToUserName, "回复测试");
                    break;
                case "单图文":
                    XML = ReArticle(FromUserName, ToUserName, "测试标题", "测试详情——百度搜索链接", "", "http://www.baidu.com");
                    break;
                case "取消关注成功":
                    LoginOut(FromUserName);
                    break;
                default:
                    //XML = ReText(FromUserName, ToUserName, "欢迎");
                    break;
            }
            return XML;
        }

        private static void LoginOut(string openid)
        {
            string sql = string.Format(@"DELETE FROM [dbo].[TB_UserOpenID] WHERE OpenID='{0}'", openid);
