...
{
    public class IBSHearResourceBLL:IIBSHearResourceBLL
    {
        ...
        /// <returns></returns>
        public bool InitializeClass(string connectionstring)
        {
            try
            {
                int totalcount = 0;
                ...
                for (int i = 0; i < pageSize; i++)
                {
                    ...
                    using (var Redis = RedisManager.GetClient(0))
                    {
                        ...
                        foreach (var item in uvi)
                        {
                            ...
                            {
                                Log4Net.LogHelper.Error(ex, "错误：初始化Rds_StudyReport_Class和Rds_StudyReport_Book失败，数据记录为" + Extension.ToJson(item));
                            }
                        }
                        Redis.Dispose();
                    }
                ...
                for (int i = 0; i < pageSize; i++)
                {
                    ...
                    using (var Redis = RedisManager.GetClient(0))
                    {
                        ...
                        foreach (var item in uvi)
                        {
                            ...
                            {
                                Log4Net.LogHelper.Error(ex, "错误：初始化Rds_StudyReport_Module失败，数据记录为" + Extension.ToJson(item));
                            }
                        }
                        Redis.Dispose();
                    }
