...
namespace Kingsun.InterestDubbingGame.Common
{
    public class InterestDubbingBaseManagement
    {
        ...
        {
            IDBManage dbManage = DBFactory.CreateDBManage(SqlHelper.InterestDubbingGameConnectionStr);
        ...
        public DataSet ExecuteSql(string sqlstr)
        {
            ...
            _operatorError = dbManage.ErrorMsg;
            if (!string.IsNullOrWhiteSpace(dbManage.ErrorMsg))
            {
                Log4Net.LogHelper.Info("方法ExecuteSql出错" + dbManage.ErrorMsg);
            }
            return ds;
        }
