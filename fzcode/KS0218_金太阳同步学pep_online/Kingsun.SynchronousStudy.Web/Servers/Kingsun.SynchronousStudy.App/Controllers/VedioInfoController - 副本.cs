...
namespace Kingsun.SynchronousStudy.App.Controllers
{
    public class VedioInfoController : ApiController
    {
        ...
        {
            string strJson =
                "{\"FirstTitleID\":\"276194\",\"FirstModularID\":\"2\",\"BookID\":\"168\",\"SecondTitleID\":\"276201\"}";
            string where = "";
        ...
        public HttpResponseMessage InsertVideoInfo([FromBody]KingRequest request)
        {
            ...
            try
            {
                int versionId = 0;// = Convert.ToInt32(SqlHelper.ExecuteScalar(SqlHelper.ConnectionString, CommandType.Text, sql));
                string path = System.Web.Hosting.HostingEnvironment.MapPath("/XmlFiles/APPManagement.xml");
        ...
        {
            string strJson = "{\"AppID\":\"241ea176-fce7-4bd7-a65f-a7978aac1cd2\",\"BookID\":\"168\",\"DialogueScore\":0.0,\"IsBool\":0,\"IsEnableOss\":1,\"NumberOfOraise\":0,\"State\":1,\"TotalScore\":0.0,\"UserId\":\"637159003\",\"VersionType\":2,\"VideoFileId\":\"2017/08/22/481ed194-c4cf-458d-899a-c7218eb93996.mp4\",\"VideoID\":\"21299\",\"VideoImageAddress\":\"2017/08/22/17a63104-de4e-4b0b-842f-61e4ca9d5921.jpg\",\"VideoNumber\":\"3\",\"VideoReleaseAddress\":\"241ea176-fce7-4bd7-a65f-a7978aac1cd2\",\"VideoType\":0,\"children\":[{\"DialogueNumber\":0,\"DialogueScore\":0.0},{\"DialogueNumber\":0,\"DialogueScore\":0.0},{\"DialogueNumber\":0,\"DialogueScore\":0.0},{\"DialogueNumber\":0,\"DialogueScore\":0.0},{\"DialogueNumber\":0,\"DialogueScore\":0.0},{\"DialogueNumber\":0,\"DialogueScore\":0.0}],\"isChoose\":false,\"isVisable\":false}";
            JavaScriptSerializer js = new JavaScriptSerializer();
            ...
            //if (string.IsNullOrEmpty(submitData.VideoNumber) && !string.IsNullOrEmpty(submitData.VideoID))
            //{
            //    string strSql = @"SELECT BookID,VideoNumber FROM dbo.TB_VideoDetails WHERE id='" + submitData.VideoID + "'";
            //    DataSet ds = SqlHelper.ExecuteDataset(SqlHelper.ConnectionString, CommandType.Text, strSql);
            //    if (ds.Tables[0].Rows.Count > 0)
            //    {
            //        submitData.BookId = ds.Tables[0].Rows[0]["BookID"].ToString();
        ...
        public HttpResponseMessage PublishVedioInfor([FromBody]KingRequest request)
        {
            ...
            try
            {
                string sql = @"UPDATE dbo.TB_UserVideoDetails SET State=1 WHERE ID=" + submitData.ID;
        ...
        {
            //UserVideoDetails submitData = new UserVideoDetails();
            UserVideoDetails submitData = JsonHelper.DecodeJson<UserVideoDetails>(request.Data);
            JavaScriptSerializer js = new JavaScriptSerializer();
            ...
            try
            {
                if (submitData.Type == 1)
                {
                    where = "a.State = '" + submitData.State + "' and a.VideoID='" + submitData.VideoID + "'";
                }
                else
                {
                    if (string.IsNullOrEmpty(submitData.AppID))
                ...
                //ps[0].Value = submitData.PageIndex;
                //ps[1].Value = submitData.PageCount;
                ps[0].Value = string.IsNullOrWhiteSpace(where) ? "1=1" : where;
                where = string.IsNullOrWhiteSpace(where) ? "1=1" : where;
                string sql = string.Format(@"select ID,TotalScore,VideoFileID,UserName,UserImage,NickName,UserID,VideoTitle,CreateTime,IsEnableOss,TrueName,UIsEnableOss ,ISNULL(p.NumberOfOraise,0) NumberOfOraise from 
(	
SELECT top 50	 ROW_NUMBER() OVER (ORDER BY a.ID DESC) num,a.ID,a.TotalScore ,a.VideoFileID ,a.State,b.TrueName,a.UserID,
									b.UserName ,b.UserImage,b.NickName,c.VideoTitle,a.VideoNumber,a.CreateTime,a.IsEnableOss,b.IsEnableOss as UIsEnableOss
									FROM      [FZ_InterestDubbing].[dbo].[TB_UserVideoDetails] a with(nolock)
									LEFT JOIN dbo.Tb_UserInfo b with(nolock) ON b.UserID = a.UserID 
									inner JOIN [FZ_InterestDubbing].[dbo].[TB_VideoDetails] c with(nolock) ON a.BookID=c.BookID AND a.VideoNumber=c.VideoNumber
									 where  " + where + @"
) T left join (   select UserVideoID,COUNT(UserID)NumberOfOraise  from TB_PraiseStatistics with(nolock)  group by UserVideoID) p on t.ID=p.UserVideoID where t.num between {0} and {1}", submitData.PageIndex, (submitData.PageIndex + submitData.PageCount));

                DataSet ds = SqlHelper.ExecuteDataset(SqlHelper.ConnectionString, CommandType.Text, sql.Replace("\r", " ").Replace("\n", " ").Replace("\t", " "), ps);
                ...
                if (vri != null)
                {
                    vri = vri.OrderByDescending(i => i.CreateTime).ToList();
                    foreach (var item in vri)
                    {
                        string img = "";
                        string fileid = "";
                        if (submitData.IsEnableOss == 0)
...
SELECT top 50	 ROW_NUMBER() OVER (ORDER BY a.ID DESC) num,a.ID,a.TotalScore ,a.VideoFileID ,a.State,b.TrueName,a.UserID,
        ...
        public HttpResponseMessage VideoAchievementInfo([FromBody]KingRequest request)
        {
            ...
            sql = string.Format(@"SELECT  a.ID ,
                                        ...
                                        a.VideoFileID ,
                                        a.State ,
                                       (SELECT  COUNT(1)  FROM   TB_PraiseStatistics  WHERE  UserVideoID = '{0}') NumberOfOraise ,
                                        b.UserName ,
                                        b.NickName ,
                                        b.TrueName ,
                                        b.UserImage ,
            ...
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    sql = string.Format(@"SELECT TOP 1 VideoTitle FROM [FZ_InterestDubbing].[dbo].[TB_VideoDetails] WHERE BookID={0} AND VideoNumber={1} ", ds.Tables[0].Rows[0]["BookID"].ToString(), ds.Tables[0].Rows[0]["VideoNumber"].ToString());
            ...
            if (ds.Tables[0].Rows.Count > 0 && null != ds.Tables[0])
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    json.Append("{\"ID\": \"" + ds.Tables[0].Rows[i]["ID"] + "\",\"UserID\": \"" + ds.Tables[0].Rows[i]["UserID"] + "\",\"CreateTime\": \"" + ds.Tables[0].Rows[i]["CreateTime"] + "\",\"NumberOfOraise\": \"" + ds.Tables[0].Rows[i]["NumberOfOraise"] + "\",");
                    json.Append("\"TotalScore\": \"" + ds.Tables[0].Rows[i]["TotalScore"] + "\",\"UserName\": \"" + ds.Tables[0].Rows[i]["TrueName"] + "\",\"VideoTitle\": \"" + Title + "\",");
                    json.Append("\"VideoType\": \"" + ds.Tables[0].Rows[i]["VideoType"] + "\",\"UserImage\": \"" + img + "\",\"NickName\": \"" + ds.Tables[0].Rows[i]["TrueName"] + "\",\"State\": \"" + ds.Tables[0].Rows[i]["State"] + "\",\"IsBool\": \"" + IsBool + "\",\"VideoFileId\": \"" + fileid + "\",");
                    json.Append("\"children\": [");
        ...
        public HttpResponseMessage VideoAchievementInfoTest()
        {
            ...
            //    return ObjectToJson.GetErrorResult("用户ID不能为0！");
            //}
            submitData.UserID = "913937032";
            submitData.ID = "686177";
            string Title = "";
            string sql = string.Empty;
            //try
            //{
            ...
            try
            {
                ...
                if (submitData.State == "1")
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (DateTime.Now.Day - Convert.ToDateTime(dsClass.Tables[0].Rows[0]["CreateDate"]).Day <= 7)
                            {
