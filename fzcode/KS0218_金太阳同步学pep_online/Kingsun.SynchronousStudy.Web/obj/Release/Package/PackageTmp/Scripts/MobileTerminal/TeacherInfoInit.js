...
var TeacherInfoInit = function () {
    ...
    $("#ma").click(function () {
        window.location.href = "ClassList.aspx?UserID=" + Current.UserID + "&AppID=" + Current.APPID;
    });

    $("#xiugai").click(function () {
        window.location.href = "UpdateUserInfo.aspx?UserID=" + Current.UserID + "&AppID=" + Current.APPID;
    });
    ...
    $("#log").click(function () {
        var obj = { OpenID: $("#divOpenId").html() };
        $.post("../Handler/WeChatHandler.ashx?queryKey=logOut", obj, function (data) {
            if (data) {
                var result = JSON.parse(data); //eval("(" + data + ")");
                if (result.Success) {
                    //window.location.href = "Login.aspx?Type=3";
