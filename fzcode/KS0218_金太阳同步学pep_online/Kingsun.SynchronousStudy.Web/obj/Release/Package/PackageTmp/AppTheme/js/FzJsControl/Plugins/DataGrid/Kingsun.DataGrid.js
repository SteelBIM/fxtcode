...
Kingsun.DataGrid = function (struct, setting) {
    ...
    this.Options = { //默认配置
        ...
        ClassNames: { TableClass: "MainTable TdNoBorder", PageClass: "FontTable font_page", FirstPage: "FirstPage", PrevPage: "PrevPage", NextPage: "NextPage", LastPage: "LastPage" }, //数据列表相关CSS
    ...
    this.discussPanel = function () {
        $(".discussTegether").hide();
        $(".recAbout h2").bind("click", function () {
            if ($(this).parent().parent().parent().next(".discussTegether")) {
                $(".discussTegether").not($(this).parent().parent().parent().next(".discussTegether")).slideUp();
                $(this).parent().parent().parent().next(".discussTegether").slideToggle();
            }
        });
        $(".discussTegether a.up").bind("click", function () {
            $(this).parent().slideUp();
        });
        $(".sideMenu li a").not($(".childList a")).bind("click", function () {
            if ($(this).next(".childList")) {
                $(".sideMenu .childList").not($(this).next(".childList")).slideUp();
                $(this).next(".childList").slideToggle();
            }
        });
        if (typeof($(".discussList").val()) !="undefined") {
            $(".discussList").slide({ mainCell: "ul", vis: 3, prevCell: ".preButton", nextCell: ".nextButton", autoPage: true, effect: "left", pnLoop: false });
        }
    }
    ...
    this.GetPageHTML = function () {//获取分页HTML
        ...
        var left = "", right = "";
        var pageHTML = this.GetTagHTML({ tagName: this.TagNames.Table, attrs: { width: "100%", className: this.Options.ClassNames.PageClass} });
