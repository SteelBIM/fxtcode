...
namespace Kingsun.SynchronousStudy.Web.HearResourcesShareResult
{
    public partial class HearResourcesShareResult : System.Web.UI.Page
    {
        ...
        private void GetListenSpeakAchievement()
        {
            try
            {
                ...
                List<HearResourcesInfo> resourcesList = new List<HearResourcesInfo>();
                if (hearResourcesList != null && hearResourcesList.Count > 0)
                {
                    ...
                    for (int i = 0, length = hearResourcesList.Count; i < length; i++)
                    {
                        ...
                        questString += " ORDER BY CreateTime DESC";
                        ...
                        if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count != 0)
                        {
                            ...
                            HearResourcesInfo resourcesInfo = new HearResourcesInfo();
                            ...
                            resourcesInfo.IsEnableOss = hearResourInfo.IsEnableOss;
                            resourcesList.Add(resourcesInfo);
                        }
                    }
                    resourcesList.Sort();
                }
                Response.Write(JsonHelper.EncodeJson(new { ResourcesList = resourcesList, UserInfo = tbuser, Success = true }));

            }
            catch (Exception)
            {
                Response.Write(JsonHelper.EncodeJson(new { Msg = "信息异常", Success = false }));
            }
            finally
            {
                Response.End();
    ...
    class HearResourcesInfo : IComparable<HearResourcesInfo>
    {
        ...
        //排序
        public int CompareTo(HearResourcesInfo obj)
        {
