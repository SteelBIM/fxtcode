...
var ClassDetailInit = function () {
        ...
        $.post("../Handler/WeChatHandler.ashx?queryKey=getStuListByClassID", obj, function (data) {
            ...
            if (result.Success) {
                ...
                for (var i = 0, length = studentList.length; i < length; i++) {
                    ...
                    html += '<img id="' + studentList[i].UserId + '" src="../AppTheme/images/xuan1.png" alt="" /></div>';
                    html += '<div class="line-normal-wrapper">';
                    html += '<div class="line-normal-left-wrapper">';
                    html += '<div class="line-normal-avatar-wrapper">';
                    html += '<img src="' + (studentList[i].AvatarUrl == "00000000-0000-0000-0000-000000000000" ? "../AppTheme/images/tou.png" : studentList[i].AvatarUrl) + '" />';
                    html += '</div>';
                    ...
                    html += '<a id="' + studentList[i].UserId + '"><img src="../AppTheme/images/delete.png" alt="" /><p>删除</p></a>';
                    html += '</div>';
                    html += '</div>';
                    html += '</div>';
                }
    ...
    this.DeleteStudent = function () {
        ...
        var obj = { IDStr: idStr, ClassID: Current.ClassID };
        $.post("../Handler/WeChatHandler.ashx?queryKey=deleteStudentList", obj, function (data) {
            if (data) {
                var result = eval("(" + data + ")")
                if (result.Success) {
