var PushSetMgr = function () {
    var Current = this;
    //初始化数据
    this.Init = function () {
        $('#tb_activatetype').bootstrapTable({
            url: '/MessageMgr/GetPushSetList',         //请求后台的URL（*）
            method: 'post',                     //请求方式（*）
            ...
            uniqueId: "pushid",                 //每一行的唯一标识，一般为主键列
            ...
            columns: [
                ...
                },
              {
                    ...
                    formatter: function (value, row, index) {
                        ...
                        action += "<a onclick=\"deleteActivate(" + row.pushid + ")\"> 删除 <a> "
...
function deleteActivate(pushid) {
    bootbox.setDefaults("locale", "zh_CN");
    bootbox.confirm("确定删除？", function (result) {
        if (result) {
            $.post("/MessageMgr/Delete", { "pushid": pushid }, function (data) {
                if (data.Success) {
