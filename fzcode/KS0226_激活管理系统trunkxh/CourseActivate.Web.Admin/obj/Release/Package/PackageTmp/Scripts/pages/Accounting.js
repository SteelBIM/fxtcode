    ...
    $.post("/CommissionPay/GetcurrentAction", function (data) {
        if (data != null && data != "") {
            ...
            if (jsonData.Export)//导出
                dataaction += "<button  onclick=\"Export()\"   type=\"button\" class=\"btn btn-default\"><span class=\"glyphicon-export\" aria-hidden=\"true\"></span>导出 </button>"
            //if (jsonData.Kont)
                dataaction += "<button onclick= \"Kont()\"    type=\"button\" class=\"btn btn-default\"><span class=\"glyphicon glyphicon-plus\" aria-hidden=\"true\"></span>结算 </button>"
    ...
    $.post("/CommissionPay/GetEmployeeOrderTotal", {
        ...
        if (data != null && data != "") {
            ...
            if (jsonData.o_bonus > 0) {
                $("#ordertotal").html(jsonData.o_bonus);
                var kontime = "";
                if ($("#startdate").val() != "") {
        ...
        $('#tb_kont').bootstrapTable({
            ...
            , {
                ...
                formatter: function (value, row, index) {
                    return (row.basis_bonus + row.p_class_bonus).toFixed(3);
                }
            }]
        });
    };
