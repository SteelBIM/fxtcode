var ActivateManagement = function () {
    var Current = this;
    //初始化数据
    this.Init = function () {
        $('#tb_activatetype').bootstrapTable({
            url: '/ActivateCreateMgr/GetBatchList',         //请求后台的URL（*）
            method: 'post',                     //请求方式（*）
            ...
            columns: [
         ...
         
                {
                    ...
                    formatter: function (value, row, index) {
                        ...
                        action += "<a onclick=\"deleteActivate(" + row.batchid + ")\"> 删除 <a> ";
                        if (row.status == 0 || row.status == 2)
                            action += "<a onclick=\"updateStatus(" + param + ")\"> 启用 <a> ";
                        if (row.status == 1)
    ...
    this.queryParams = function (params) {
        var temp = {   //这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
            ...
            activatetypeid: $("#SelectActivateType").val() || -1,
            state: $("#state").val() || -1,
...
function updateStatus(batchId, status) {
    $.post("/ActivateCreateMgr/UpdateState", { "batchId": batchId, "status": status }, function (data) {
        if (data.Success) {
...
function deleteActivate(batchId) {
    bootbox.setDefaults("locale", "zh_CN");
    bootbox.confirm("确定删除？", function (result) {
        if (result) {
            $.post("/ActivateCreateMgr/BatchDelete", { "batchId": batchId }, function (data) {
                if (data.Success) {
                    $('#tb_activatetype').bootstrapTable("refresh");
                }
                else {
