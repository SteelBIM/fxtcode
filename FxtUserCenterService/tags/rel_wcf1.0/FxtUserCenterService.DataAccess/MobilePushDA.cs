...
namespace FxtUserCenterService.DataAccess
{
    ...
    public class MobilePushDA : Base
    {
        ...
        public static int UpdatePush(DatMobilePush model)
        {
            ...
            string sql = @" update dbo.Dat_MobilePush set IosPhshUserid=@IosPhshUserid,AndPhshUserid=@phshuserid,channelid=@channelid where 
			                              UserName=@username and  ProductTypeCode=@producttypecode ";
            cmd.CommandText = sql;
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@IosPhshUserid", model.iosphshuserid, SqlDbType.NVarChar));
            cmd.Parameters.Add(SqlHelper.GetSqlParameter("@AndPhshUserid", model.andphshuserid, SqlDbType.NVarChar));
