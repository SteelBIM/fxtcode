...
namespace FxtUserCenterService.Actualize
{
    public class SecurityVerify
    {
        ...
        public static WCFJsonData ApiBaseVerify(string sinfo, string info)
        {
            ...
            {
                jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "非法访问(10003)");
            }
            else
            {
...
#else
                        jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "非法访问(10004)");
#endif
                    }
                    else
                    {
...
#else
                                    jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "非法访问(10005)");
#endif
                                    break;
                                case 0:
#if DEBUG
...
#else
                           jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "非法访问(10006)");
#endif
                ...
                {
                    LogHelper.Error(ex);
                    jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "非法访问(10007)");
                    return jsonData;
                }
            }
            return jsonData;
        ...
        public static WCFJsonData ApiLoginVerify(string sinfo, string info)
        {
            ...
            if (string.IsNullOrEmpty(sinfo) || string.IsNullOrEmpty(info))
            {
                jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10011)");
            }
            else
            {
                string code = string.Empty;//加密后参数
                ...
                try
                {
                    ...
                    if (errList.Count > 0)
                    {
                        ...
                        jsonData = JSONHelper.GetWcfJson(null, (int)EnumHelper.Status.None, "必传参数缺失", errList);
#else
                        jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10012)");
#endif
                    }
                    else
                    {
                        ...
                        switch (dateVeify)
                        {
                            case -1:
                                ...
                                jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.None, "加密字符串不匹配");
#else
                                jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10013)");
#endif
                                break;
                            case 0:
#if DEBUG
                ...
                {
                    LogHelper.Error(ex);
                    jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10005)");
                    return jsonData;
                }
            }
            return jsonData;
        ...
        /// <param name="funargs">功能参数</param>
        /// <returns></returns>
        public static WCFJsonData CmbApiLoginVerify(string sinfo, string info)
        {
            WCFJsonData jsonData = new WCFJsonData();
            //sinfomd5加密key  
            if (string.IsNullOrEmpty(sinfo) || string.IsNullOrEmpty(info))
            {
                jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10006)");
            }
            else
            {
                string code = string.Empty;//加密后参数
                ...
                try
                {
                    ...
                    if (errList.Count > 0)
                    {
                        ...
                        jsonData = JSONHelper.GetWcfJson(null, (int)EnumHelper.Status.None, "必传参数缺失", errList);
#else
                        jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10007)");
#endif
                    }
                    else
                    {
                        ...
                        switch (dateVeify)
                        {
                            case -1:
                                ...
                                jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.None, "加密字符串不匹配");
#else
                                jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10008)");
#endif
                                break;
                            case 0:
#if DEBUG
                            ...
                            case 1:
                                if (paramInfo.funinfo == null) { paramInfo.funinfo = new FunInfo(); paramInfo.funinfo.weburl = ""; }
                                jsonData = UserBL.CmbGetUserAndAppInfo(null, paramInfo.uinfo.password, StringHelper.TryGetInt(paramInfo.appinfo.systypecode), paramInfo.uinfo.username, paramInfo.appinfo.stype, paramInfo.funinfo.weburl, paramInfo.appinfo.splatype, paramInfo.appinfo.channel);
                                break;
                        }
                ...
                {
                    LogHelper.Error(ex);
                    jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10009)");
                    return jsonData;
                }
            }
            return jsonData;
        ...
        /// <returns></returns>
        public static WCFJsonData VerifyPassWordIsSimple(string systypecode, string password, string username)
        {
            WCFJsonData jsonData = new WCFJsonData();
            ...
            try
            {
                ...
                string m = EncryptHelper.GetMd5(username + ConstCommon.WcfUserNameMd5Key);
                string d = EncryptHelper.TextToPassword(username);
                string um = EncryptHelper.TextToPassword(password);
                ...
                if (ignore.Length > 0)
                {
                    for (int i = 0; i < ignore.Length; i++)
                    {
                        ...
                        if (ignore[i] == systypecode)
                        {
                            if (oldpwd.Length < 6 || oldpwd.Length > 20 || !Regex.IsMatch(oldpwd, pattern))
                            {
                                return JSONHelper.GetWcfJson((int)EnumHelper.Status.SimplePassWord, ConstCommon.WcfSimpleTip);
                            }
                            else
                            {
            ...
            {
                LogHelper.Error(ex);
                jsonData = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10010)");
                return jsonData;
            }
            return jsonData;
        }
