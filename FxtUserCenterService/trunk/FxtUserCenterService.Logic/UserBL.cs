...
namespace FxtUserCenterService.Logic
{
    public class UserBL
    {
...
#else
                jsondata = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10001)");
#endif
...
#else
                        jsondata = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10002)");
#endif
        ...
        /// <param name="userName">用户名</param>
        /// <returns></returns>
        public static WCFJsonData CmbGetUserAndAppInfo(SearchBase search, string pwd, int productTypeCode, string userName, string appabbreviation, string weburl, string splatype = null, string channel = null)
        {
            WCFJsonData jsondata = new WCFJsonData();

            InheritUserInfo inheri = UserDA.GetUserAndAppInfo(search, productTypeCode, userName, appabbreviation, weburl);
            //只对@cmb用户开放
            if (inheri == null || userName.IndexOf("@cmb") < 0)
            {
#if DEBUG
                jsondata = JSONHelper.GetWcfJson((int)EnumHelper.Status.LoginFailure, "用户不存在");
#else
                jsondata = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10003)");
#endif
            ...
            else
            {
                ...
                {
                    string password = EncryptHelper.GetMd5(ConstCommon.CmbWcfPassWordMd5Key);
                    ...
                    jsondata = JSONHelper.GetWcfJson((int)EnumHelper.Status.LoginFailure, "密码错误");
#else
                        jsondata = JSONHelper.GetWcfJson((int)EnumHelper.Status.Failure, "用户或密码错误(10004)");
#endif
        ...
        {
            return UserDA.GetUserInfoByUserNames(username);
        }
        ...
        {
            return UserDA.GetUserListByUserName(search, companyid, username);
        }
        ...
        {
            return UserDA.GetUserListByUserNameOrTrueName(search, companyid, username);
        }
    }
