...
using System.Data;
namespace FxtUserCenterService.API
{
    /// <summary>
    /// 一般处理程序基类
    ...
    public class HttpHandlerBase : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public SearchBase InitData()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(key))
                    search.Key = key;
                //查勘类型
                int SurveyTypeCode = StringHelper.TryGetInt(GetRequest("SurveyTypeCode"));
                search.SurveyTypeCode = SurveyTypeCode;
                return search;
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        public bool CheckMustRequestBeforeLogin()
        {
            HttpContext con = HttpContext.Current;
            Dictionary<string, string> errList = new Dictionary<string, string>();
            string[] request = new string[] { "userid","password","systypecode", "strcode", "strdate" };
            for (int i = 0; i < request.Length; i++)
            {
                if (string.IsNullOrEmpty(con.Request[request[i]]))
                {
        ...
        /// <param name="request"></param>
        /// <returns></returns>
        public bool CheckMustRequestAfterLogin()
        {
            HttpContext con = HttpContext.Current;
            Dictionary<string, string> errList = new Dictionary<string, string>();
            //string[] request = new string[] { "userid", "systypecode", "strcode", "strdate", "fxtcompanyid", "cityid","FxtUserCenterServicetypecode" };
            string[] request = new string[] {"strcode", "strdate"};//用户中心接口，每次必须验证加密code
            for (int i = 0; i < request.Length; i++)
            {
