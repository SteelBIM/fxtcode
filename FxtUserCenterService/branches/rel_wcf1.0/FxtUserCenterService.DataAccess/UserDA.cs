...
namespace FxtUserCenterService.DataAccess
{
    public class UserDA : Base
    {
        ...
        {
            string sql = @"select a.username,a.companyid,a.valid as uservalid,a.emailstr,a.mobile,a.wxopenid, b.valid as companyvalid,b.businessdb,b.companycode,b.companyname
                from dbo.userinfo a with(nolock) 
                inner join dbo.companyinfo b with(nolock) on a.companyid=b.companyid
                where a.username=@username";
            List<SqlParameter> parameters = new List<SqlParameter>();
        ...
        public static int EditUser(FxtUserCenterService.Entity.UserInfo userinfo, string password)
        {
            ...
            if (!string.IsNullOrWhiteSpace(userinfo.emailstr))
            {
        ...
        public static List<UserCheck> GetUserList(SearchBase search, int companyid, string companycode, string[] userids)
        {
            List<SqlParameter> parameters = new List<SqlParameter>();
            //UserPwd  不获取用户密码
            string sql = @"select UserName, CompanyId, CreateDate, Valid, EmailStr, Mobile, wxopenid, UpdateDate,TrueName 
                from dbo.UserInfo a with(nolock)
                inner join dbo.CompanyInfo b with(nolock) on a.CompanyId=b.CompanyId
                where 1=1";
        ...
        /// <returns></returns>
        public static InheritUserInfo GetUserAndAppInfo(SearchBase search, int productTypeCode, string userName, string appabbreviation)
        {
            try
            {
                List<SqlParameter> parameters = new List<SqlParameter>();
                ...
                //}
                //if (!string.IsNullOrEmpty(splatype))
                //{
                //    sql += "and caf.SplaType = @SplaType ";
        ...
        public static List<InheritUserInfo> GetApps(SearchBase search, int productTypeCode, string userName, string appabbreviation)
        {
            try
            {
                ...
                sql += "group by cpa.AppId,cpa.AppKey,cpa.AppPwd,cpa.ApiUrl";
