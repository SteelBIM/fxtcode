...
namespace FxtCenterService.Logic
{
    public class SYSAreaBL
    {
        ...
        /// <returns></returns>
        public static List<SYSArea> GetSYSAreaList(SearchBase search, int[] areaid, string zipcodeStr, string areaName)
        {
            string areaidstr = (areaid == null || areaid.Length == 0) ? null : string.Join(",", areaid.Select(i => i.ToString()).ToArray());
            return SYSAreaDA.GetSYSAreaList(search, areaidstr, zipcodeStr, areaName);
        }
        /// <summary>
        /// 获取城市列表
        /// </summary>
        ...
        {
            return SYSAreaDA.GetSYSCityList(provinceid, zipcode,cityName, cityIDs);
        }
        /// <summary>
        /// 获取城市列表(根据省份zipcode)
        /// </summary>
        ...
        {
            if (string.IsNullOrWhiteSpace(provinceName)&& provinceIDs.Count == 0)
            {
                return SYSAreaDA.GetProvinceList();
            }
            else
            {
                return SYSAreaDA.GetProvinceList(provinceName, provinceIDs);
            }
        }
